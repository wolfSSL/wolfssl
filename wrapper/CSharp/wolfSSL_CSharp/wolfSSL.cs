...
namespace wolfSSL.CSharp
{
    ...
    private const int WC_WAIT = 6000000;
        ...
        //private static wolfcrypt.loggingCbEx internal_log_ex         = null; /* keep reference to prevent GC         */
        //private static wolfcrypt.loggingCbEx internal_bridge_cb      = null; /* helper when using original loggingCb */
#endif
        ...
        /// <returns>1 on success</returns>
        public static int get_ciphers(ref string list, int sz)
        {
            try
            {
                int ret;
#if WindowsCE && !PocketPC
                ret = wolfSSL_get_ciphers(list, sz);
#else
                StringBuilder sb = new StringBuilder(sz);
                ret = wolfSSL_get_ciphers(sb, sz);
                if (ret >= 0) {
                    list = sb.ToString();
                }
#endif
