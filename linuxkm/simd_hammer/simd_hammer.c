...
static int wc_simd_report_fn(void *unused)
{
    ...
    while (!kthread_should_stop()) {
        ...
        pr_info("simd_hammer summary: probes=%lu simd=%lu NO_SIMD=%lu ERRS=%lu(last=%d) "
                "late avg=%lluus max=%lluus | last %us: +%lu NO_SIMD (%lu.%02lu/min), "
                "+%lu ERRS (%lu.%02lu/min)%s\n",
                t.p, t.s, t.n, t.e, t.last_err,
                late_avg, t.late_max, report_secs,
...
static int __init wc_simd_init(void)
{
    ...
    for_each_online_cpu(cpu) {
        ...
        c->desc = kmalloc(sizeof(*c->desc) +
                          crypto_shash_descsize(wc_simd_probe_tfm), GFP_KERNEL);
        if (!c->pbuf || !c->desc) {
            ret = -ENOMEM;
            goto err;
        }
    ...
    pr_info("simd_hammer loaded: hammer=%s%s%s probe=%s(%s) %u B every %u jiffies, "
            "%u cpus, HZ=%d\n",
            hammer_mode,
            wc_simd_raw_mode ? "" : " alg=", wc_simd_raw_mode ? "" : hammer_alg,
            probe_alg, probe_ctx, probe_bytes, probe_interval,
            num_online_cpus(), HZ);
    return 0;
...
static void __exit wc_simd_exit(void)
{
    ...
    pr_info("simd_hammer unloaded: final probes=%lu simd=%lu NO_SIMD=%lu ERRS=%lu "
            "(last=%d) late max=%lluus\n",
            t.p, t.s, t.n, t.e, t.last_err, t.late_max);
}
