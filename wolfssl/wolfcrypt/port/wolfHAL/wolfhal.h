 ...
 *
 * This header deliberately pulls in no wolfHAL headers, so wc_port.c can
 * reach wc_wolfHAL_RegisterDevice() without the BSP on its include path. The
 * wolfHAL types are needed only by wolfhal.c, which gets them from board.h.
 *
 * board.h is supplied by the application, on the include path, in the same
 * way settings.h expects a user_settings.h. It must provide:
 *
 ...
 *
 * Only the modes board.h names a device for are offloaded; the rest fall back to
 * wolfCrypt's software implementations. WC_WOLFHAL_RNG_DEV is the exception and
 * has no fallback; see wolfhal.c.
 */
