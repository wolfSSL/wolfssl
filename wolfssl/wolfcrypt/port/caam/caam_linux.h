...
/* The driver core serializes ring submission and the shared staging pool with
 * this mutex; a single threaded build has one caller, so the locks are no-ops
 * and pthread is not linked. */
#ifdef SINGLE_THREADED
    #define CAAM_MUTEX int
    #define CAAM_INIT_MUTEX(x) (void)(x)
    #define CAAM_FREE_MUTEX(x) (void)(x)
    #define CAAM_LOCK_MUTEX(x) (void)(x)
    #define CAAM_UNLOCK_MUTEX(x) (void)(x)
#else
    #include <pthread.h>
