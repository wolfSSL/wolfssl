 ...
 * copied). seed is the 256-bit HKDF input; wrapped is the private scalar
 * AES-wrapped under the HUK-derived key (length a multiple of 16, <=
 * WC_RTL8735B_MAX_WRAPPED); plainLen is the real scalar size (e.g. 32 P-256).
 *
 * By default the sign runs in software after the HUK ECB-unwrap. Set useHwEngine
 ...
 * and never enters software -- seed/wrapped/plainLen are then unused and may be
 * zero/NULL. (The HW-engine INPUT/HUK-wrapped path is validated on silicon; the
 * OTP-resident path is implemented but unexercised -- it needs an OTP key.) */
