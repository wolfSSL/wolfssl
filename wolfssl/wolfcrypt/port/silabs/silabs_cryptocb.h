...
/* Declare the wrapped-key API on exactly the parts that compile it (Vault
 * High), so a Vault Mid build fails at the call site with an undeclared
 * function rather than at link time. WOLFSSL_SILABS_NO_VAULT_KEYS suppresses
 * the API on a Vault High part. */
...
/* Register / unregister the Secure Element device. wolfCrypt_Init registers it
 * at WOLFSSL_SILABS_DEVID automatically; call these only for a different device
 * id or an early teardown. 0 on success, negative wolfCrypt error otherwise. */
 ...
 * application never sees the material. A built-in key lives in an SE slot and
 * is named rather than supplied. Both bind to an ordinary Aes or ecc_key,
 * after which the normal wolfCrypt calls run on the SE against that key.
 *
 * The descriptor references the caller's wrapped buffer without copying, so
 * that buffer must outlive the bound Aes / ecc_key. Wrapped keys need Vault
 * High; built-in slots do not. 0 on success, negative wolfCrypt error. */
