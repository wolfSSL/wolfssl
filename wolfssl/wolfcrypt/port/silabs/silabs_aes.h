...
typedef struct {
  ...
  /* Non-zero once wc_SilabsSe_AesUseWrappedKey() or ...UseBuiltInKey() has
   * bound a resident key, after which the callback port leaves the descriptor
   * alone instead of rebuilding it from a plaintext key. */
  byte keySet;
} silabs_aes_t;
...
/* Raw-status forms of the above: negative is a wolfCrypt argument error,
 * anything else is the SE status unchanged, so an unsupported command can be
 * told from a hardware failure. The callback port uses these. */
