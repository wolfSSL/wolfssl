...
/* Host compile-test stand-in for the slice of the Silicon Labs SE Manager the
 * port references. Compiled ONLY under WOLFSSL_SILABS_HOST_TEST, it lets CI
 * build the port's dispatch and compile-time guards with no Simplicity SDK
 * installed. On target the real SE Manager headers are used instead.
 *
 * Every stub returns SL_STATUS_NOT_SUPPORTED and does NO crypto, which the
 * port maps to CRYPTOCB_UNAVAILABLE, so this build also exercises every
 * engine's decline-and-fall-back path. Correctness is validated on EFR32
 * silicon, never through these stubs.
 *
 * Signatures mirror Simplicity SDK
 * platform/security/sl_component/se_manager/inc/. Add a stub here when the
 * port starts calling a new sl_se_* function. The device is modeled as
 * Series 2 Config 5 (EFR32xG25) with Secure Vault High.
 */
...
/* Vault High by default. WOLFSSL_SILABS_HOST_TEST_VAULT_MID models a Vault
 * Mid part, compile-testing that every Vault-only path (SHA-384/512,
 * P-384/P-521, ChaCha20-Poly1305, SE KDFs, wrapped keys) drops out. */
