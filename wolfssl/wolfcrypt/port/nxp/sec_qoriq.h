...
/*
 * NXP QorIQ SEC (CAAM) engine on the T-series PowerPC parts. Verified on
 * T2080 (SEC 5.2) and T1040 (SEC 5.0): both report era 6 in CCBVID (their
 * device trees claim 5), place the SEC at CCSR + 0x300000 with four job
 * rings, and declare "fsl,sec-v4.0", so one descriptor format covers both.
 *
 * Only security-enabled ("E") part numbers carry the SEC;
 ...
 *    index 0..31 returns DECO error 0x82. Passing the parameters explicitly
 *    costs a few descriptor words and gains any prime curve wolfCrypt knows
 *    rather than the handful built in.
 *  - The "message representative is already hashed" PROTINFO flag, refused
 *    with DECO error 0x81. Left clear the engine takes the representative
 *    as given, which is what wolfCrypt passes anyway. */
 ...
 *    The counters are bumped in the callback router before the ring mutex is
 *    taken, so concurrent callers lose updates: approximate under threads,
 ...
 *    re-read RDSTA (side-effect free) in case another agent instantiated the
 *    handle later, and never program the TRNG.
 ...
 * stages data through a pinned arena (Linux) sees every buffer. There is
 * deliberately no way to append a raw physical address.
 *
 * wc_SecQoriqDescAddBuf() infers direction from the command (FIFO STORE and
 * STORE are engine writes, everything else reads); wc_SecQoriqDescAddPdbPtr()
 * appends a bare PDB pointer word, whose command carries no direction, so
...
/* While set, DMA allocations report physical addresses above 4 GB, the way
 * a 36-bit Linux system hands out high pages. */
WOLFSSL_SECQ_API void wc_SecQoriqSimHighPhysAlloc(int on);
/* While set, MCFGR reads report MCFGR[LONG_PTR] (64-bit pointer mode). */
WOLFSSL_SECQ_API void wc_SecQoriqSimLongPtr(int on);
