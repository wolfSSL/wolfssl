 ...
 *  - rngReady and the RNG4 programming behind it are not serialised. Call
 *    wc_SecQoriqRngInit() once at start-up rather than relying on the lazy
 *    path in wc_SecQoriqRandomBlock().
...
WOLFSSL_LOCAL word32 wc_SecQoriqRead(const byte* base, word32 off);
WOLFSSL_LOCAL void   wc_SecQoriqWrite(byte* base, word32 off, word32 val);

/* Accessor for the singleton device, NULL before wc_SecQoriqInit(). */
WOLFSSL_LOCAL SecQoriqDev* wc_SecQoriqGetDev(void);
...
WOLFSSL_LOCAL int  wc_SecQoriqRegisterCryptoCb(void);
WOLFSSL_LOCAL void wc_SecQoriqUnregisterCryptoCb(void);
#endif

/* Descriptor assembly. Each returns 0 on success or a negative error. */
WOLFSSL_LOCAL int wc_SecQoriqDescInit(SecQoriqDesc* desc);
WOLFSSL_LOCAL int wc_SecQoriqDescAddWord(SecQoriqDesc* desc, word32 in);
WOLFSSL_LOCAL int wc_SecQoriqDescAddPtr(SecQoriqDesc* desc, word64 phys);
WOLFSSL_LOCAL int wc_SecQoriqDescAddBuf(SecQoriqDesc* desc, word32 cmd,
    const byte* buf, word32 bufSz);

/* Submit a descriptor to the job ring and wait for it to retire. */
WOLFSSL_LOCAL int wc_SecQoriqRun(SecQoriqDev* dev, SecQoriqDesc* desc);
...
WOLFSSL_LOCAL int wc_SecQoriqRunEx(SecQoriqDev* dev, SecQoriqDesc* desc,
    word32* statusOut);

/* Decode a job ring status word into a wolfSSL error and, when logging is on,
 * a human readable reason. */
WOLFSSL_LOCAL int wc_SecQoriqParseError(word32 status);
...
WOLFSSL_LOCAL int wc_SecQoriqHash(word32 algo, word32 digestSz,
    const byte* in, word32 inSz, byte* out);
WOLFSSL_LOCAL int wc_SecQoriqSha1(const byte* in, word32 inSz, byte* out);
WOLFSSL_LOCAL int wc_SecQoriqSha224(const byte* in, word32 inSz, byte* out);
WOLFSSL_LOCAL int wc_SecQoriqSha256(const byte* in, word32 inSz, byte* out);
WOLFSSL_LOCAL int wc_SecQoriqSha384(const byte* in, word32 inSz, byte* out);
WOLFSSL_LOCAL int wc_SecQoriqSha512(const byte* in, word32 inSz, byte* out);
...
WOLFSSL_LOCAL int wc_SecQoriqAes(word32 mode, int encrypt, const byte* key,
    word32 keySz, byte* iv, const byte* in, word32 inSz, byte* out);
WOLFSSL_LOCAL int wc_SecQoriqAesCbcEncrypt(const byte* key, word32 keySz,
    byte* iv, const byte* in, word32 inSz, byte* out);
WOLFSSL_LOCAL int wc_SecQoriqAesCbcDecrypt(const byte* key, word32 keySz,
    byte* iv, const byte* in, word32 inSz, byte* out);
WOLFSSL_LOCAL int wc_SecQoriqAesCtrEncrypt(const byte* key, word32 keySz,
    byte* iv, const byte* in, word32 inSz, byte* out);
WOLFSSL_LOCAL int wc_SecQoriqAesEcbEncrypt(const byte* key, word32 keySz,
    const byte* in, word32 inSz, byte* out);
WOLFSSL_LOCAL int wc_SecQoriqAesEcbDecrypt(const byte* key, word32 keySz,
    const byte* in, word32 inSz, byte* out);
#endif /* !NO_AES */
...
/******************************************************************************
  ...
  uninstantiated, so wc_SecQoriqInit() records that and this has to run
  before any random data can be drawn.
  ****************************************************************************/

WOLFSSL_LOCAL int wc_SecQoriqRngInit(void);
WOLFSSL_LOCAL int wc_SecQoriqRandomBlock(byte* out, word32 sz);
#endif /* !WC_NO_RNG */
...
WOLFSSL_LOCAL int wc_SecQoriqAesGcm(int encrypt, const byte* key, word32 keySz,
    const byte* iv, word32 ivSz, const byte* aad, word32 aadSz,
    const byte* in, word32 inSz, byte* out, byte* tag, word32 tagSz);
WOLFSSL_LOCAL int wc_SecQoriqAesGcmEncrypt(const byte* key, word32 keySz,
    const byte* iv, word32 ivSz, const byte* aad, word32 aadSz,
    const byte* in, word32 inSz, byte* out, byte* tag, word32 tagSz);
WOLFSSL_LOCAL int wc_SecQoriqAesGcmDecrypt(const byte* key, word32 keySz,
    const byte* iv, word32 ivSz, const byte* aad, word32 aadSz,
...
WOLFSSL_LOCAL int wc_SecQoriqEccSupported(int curveId, word32 keySz);
 ...
 * internally from RNG4, which this brings up if it is not already running. */
WOLFSSL_LOCAL int wc_SecQoriqEccSign(int curveId, const byte* priv,
...
WOLFSSL_LOCAL int wc_SecQoriqEccVerify(int curveId, const byte* pubXY,
...
WOLFSSL_LOCAL int wc_SecQoriqEcdh(int curveId, const byte* priv,
...
WOLFSSL_LOCAL int wc_SecQoriqRsaPublic(const byte* in, word32 inSz,
...
WOLFSSL_LOCAL int wc_SecQoriqModExp(const byte* in, const byte* d, word32 dSz,
...
WOLFSSL_LOCAL int   wc_SecQoriqMapRegs(byte** regsOut);
WOLFSSL_LOCAL void  wc_SecQoriqUnmapRegs(byte* regs);
...
WOLFSSL_LOCAL void* wc_SecQoriqDmaAlloc(word32 sz, word64* physOut);
WOLFSSL_LOCAL void  wc_SecQoriqDmaFree(void* virt, word64 phys, word32 sz);
 ...
 * Every translation goes through this: there is deliberately no unchecked
 * single-address form to reach for. */
WOLFSSL_LOCAL word64 wc_SecQoriqVirtToPhysLen(void* virt, word32 len);

/* Cache maintenance around every descriptor and data buffer. */
WOLFSSL_LOCAL int wc_SecQoriqCacheFlush(void* virt, word32 sz);
WOLFSSL_LOCAL int wc_SecQoriqCacheInval(void* virt, word32 sz);
 ...
 * scalar, say) in DRAM. */
WOLFSSL_LOCAL void wc_SecQoriqForceZeroDma(void* buf, word32 sz);

/* Yield briefly inside the completion poll loop. */
WOLFSSL_LOCAL void wc_SecQoriqCpuRelax(void);

/* Read the SVR so the caller can confirm this is a security enabled part. */
WOLFSSL_LOCAL int wc_SecQoriqGetSvr(word32* svrOut);
