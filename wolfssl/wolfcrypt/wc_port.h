 ...
 *
 * The hold covers a session that already has the buffer. Taking the hold is a
 * read of the context's pointer followed by the increment, so a reload running
 * at the same moment as wolfSSL_new() can still pull the buffer out in
 * between. Nor is the rest of what wolfSSL_new() takes from the context
 * counted. Reloading has to be kept clear of session creation either way.
 *
