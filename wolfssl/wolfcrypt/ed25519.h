...
/*!
    ...
    \retrun BAD_FUNC_ARG Returned if rng or key evaluate to NULL, or if the specified key size is not 32 bytes (ed25519 has 32 byte keys)
    \retrun MEMORY_E Returned if there is an error allocating memory during function execution
...
/*!
    ...
    \return BAD_FUNC_ARG Returned any of the input parameters evaluate to NULL, or if the output buffer is too small to store the generated signature
    \return MEMORY_E Returned if there is an error allocating memory during function execution
    ...
    \param key pointer to a private ed25519_key with which to generate the signature
...
/*!
    ...
    \return 1 Returned if verification completes, but the signature generated does not match the signature provided
     ...
     \param key pointer to a public ed25519 key with which to verify the signature
...
/*!
    ...
    \return BAD_FUNC_ARG Returned if in or key evaluate to NULL, or inLen is less than the size of an ed25519 key
    
    \param in pointer to the buffer containing the public key
    \param inLen length of the buffer containing the public key
    \param key pointer to the ed25519_key object in which to store the public key
...
/*!
    ...
    \return 0 Returned on successfully importing the ed25519_key
    \return BAD_FUNC_ARG Returned if in or key evaluate to NULL, or if either privSz or pubSz are less than the size of an ed25519 key
    ...
    \param key pointer to the ed25519_key object in which to store the imported private/public key pair
...
/*!
    ...
    \return BUFFER_E Returned if the buffer provided is not large enough to store the private key. Upon returning this error, the function sets the size required in outLen
    
    \param key pointer to an ed25519_key structure from which to export the public key
...
/*!
    ...
    \return BUFFER_E Returned if the buffer provided is not large enough to store the private key
    
    \param key pointer to an ed25519_key structure from which to export the private key
...
/*!
    ...
    \param outLen Max length of output, set to the length of the exported private key.
...
/*!
    ...
    \return 0 Success
    \return BAD_FUNC_ARG: Returns if any argument is null.
    \return BUFFER_E: Returns if outLen is less than ED25519_PRV_KEY_SIZE or ED25519_PUB_KEY_SIZE
...
/*!
    ...
    \param key pointer to an ed25519_key structure for which to get the key size
...
/*!
    ...
    \param key pointer to an ed25519_key structure for which to get the signature size
