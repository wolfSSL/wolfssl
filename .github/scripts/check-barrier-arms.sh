...
need_arg() { [ $# -ge 2 ] || { echo "check-barrier-arms: $1 needs an argument" >&2; exit 2; }; }
while [ $# -gt 0 ]; do
    case $1 in
...
# Like check_arm, but greps the fully macro-expanded call (via -E, not -dM),
# so a difference nested inside XASM_VOLATILE actually shows up.
...
check_expansion() {
    ...
    case "$_pp" in
        *"$_want"*)
            case "$_pp" in
                *"${_notwant:-\255no-such-string\255}"*)
...
# XASM_VOLATILE_MB keeps the "memory" clobber for both. Check the actual
# expanded keyword (via -E, invoking the macro), not just that
# XASM_VOLATILE_MB's own body is unconditional -- that part can't differ per
# compiler. (Not tested via XFENCE(): a real gcc/clang picks the
# __atomic_thread_fence()/stdatomic arm ahead of the asm one.)
...
# XASM_VOLATILE_NO_CLOBBER opts out of the "memory" clobber for a toolchain
# that rejects it.
