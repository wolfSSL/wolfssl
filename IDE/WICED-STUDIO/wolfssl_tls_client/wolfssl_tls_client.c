 ...
 *      Quick Start Guide
 *   8. Setup instructions can be found at IDE/WICED_STUDIO in the wolfSSL
 *      library.
 *
 ...
 * printed on the serial console and the message to the server from the client
 * is printed in the terminal running the server.
 *
 ...
 *
 *  If you're using the RSA certificate, the example server will load the
 *  correct certificate by default. If you're using the ECC cert, you will
 *  need to load the correct certificate and key into the server.
...
void application_start(void)
{
...
#if defined(WOLFSSL_WICED_USE_RSA_CERT) && defined(WOLFSSL_WICED_STATIC_CA)
    /* RSA certs */
    const char unsigned*    CaCert = ca_cert_der_2048;
    const int               CaCertLen = sizeof_ca_cert_der_2048;
#endif
#if defined(WOLFSSL_WICED_USE_ECC_CERT) && defined(WOLFSSL_WICED_STATIC_CA)
    /* ECC certs */
    ...
    WPRINT_APP_INFO(("Connecting to the remote TCP server every "
        "%d seconds...\n", TCP_CLIENT_INTERVAL));
}
