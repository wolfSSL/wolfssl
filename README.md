...
also no longer enables static key cipher suites with PSK, RSA, or ECDH.
This means if you plan to use TLS cipher suites you must enable DH
(DH is on by default), or enable ECC (ECC is on by default), or you must enable
static key cipher suites with one or more of the following defines:
...
WOLFSSL_STATIC_PSK
```
Static key cipher suites lower your security by removing perfect forward
secrecy (PFS). Static cipher suites leverage the same long term private key for
sharing a session secret instead of using (PFS), which is safer and is the
default in wolfSSL. PFS uses a newly generated ephemeral key to exchange the
primary secret used to derive session keys.
