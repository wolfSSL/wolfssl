...
static int x509AddCertDir(WOLFSSL_BY_DIR *ctx, const char *argc, long argl)
{
    ...
    int i, num;
    const char* c;
    WC_DECLARE_VAR(buf, char, MAX_FILENAME_SZ);
    ...
    int req)
{
    ...
    WC_DECLARE_VAR(der, byte, X509_BUFFER_SZ);
...
int wolfSSL_i2d_X509_NAME_canon(WOLFSSL_X509_NAME* name, unsigned char** out)
{
    ...
    WC_DECLARE_VAR(names, EncodedName, MAX_NAME_ENTRIES);
                    ...
                    unsigned int flags, char **peername)
{
    int         ret;
    size_t      i;
    WC_DECLARE_VAR(dCert, DecodedCert, 1);
    ...
    const unsigned char* buf, int sz, int format, void * heap)
{
    ...
    WOLFSSL_X509_ACERT * x509 = NULL;
    DerBuffer *          der = NULL;
    WC_DECLARE_VAR(acert, DecodedAcert, 1);
