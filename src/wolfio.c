    ...
    while (p){
        if (p->len) {
          if ((p->len - skipLen) > (size - totalLen)) { /* buffer full */
                XMEMCPY(&buff[totalLen],
                    (const char *)&(((char *)(p->payload))[skipLen]),
                    size - totalLen
                );
                totalLen = size;
                break;
...
static err_t wolfSSL_LwIP_recv_cb(void *cb, struct tcp_pcb *pcb, struct pbuf *p,
    s8_t err)
{
    struct pbuf *next;
    WOLFSSL *ssl;
    ...
    if (ssl->lwipCtx.recv_fn)
        return ssl->lwipCtx.recv_fn(ssl->lwipCtx.arg, pcb, p, err);
                                                      /* user callback */
    return ERR_OK;
}

static err_t wolfSSL_LwIP_sent_cb(void *cb, struct tcp_pcb *pcb, u16_t err)
{
    WOLFSSL* ssl = (WOLFSSL*)cb;
    DBG_PRINTF_CB("wolfSSL_LwIP_write_cb, err=%d\n", err);
    if (ssl->lwipCtx.sent_fn)
        return ssl->lwipCtx.sent_fn(ssl->lwipCtx.arg, pcb, err);
                                                      /* user callback */
    return ERR_OK;
}
