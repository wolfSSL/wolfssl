    ...
    DecodedCert* cert)
{
    ...
    /* Don't check if no SSL object verification is disabled for SSL context. */
...
static int ProcessBufferCert(WOLFSSL_CTX* ctx, WOLFSSL* ssl, DerBuffer* der)
{
...
#endif
    {
        ...
        if (ret == 0) {
            ...
            ret = ProcessBufferCertPublicKey(ctx, ssl, cert);
        }
    }
    ...
    int verify, int flags, int* failCount, int* successCount)
{
    ...
    if (ret != 1) {
        ...
        /* Try loading as a trusted peer certificate instead. */
    ...
    word32 flags, int verify, int successCount)
{
    ...
    if (ret == 1) {
        ...
        #if defined(WOLFSSL_QT)
            /* Compliant with OpenSSL when flag set. */
            /* TODO: condition on if statement isn't zero files found. */
    ...
    long sz, int format)
{
    ...
    ret = ProcessBuffer(ctx, in, sz, format, PRIVATEKEY_TYPE, NULL, NULL, 0,
        GET_VERIFY_SETTING_CTX(ctx));
    ...
    unsigned char* g, int gSz)
{
    ...
    if ((ret == 1) && (((word16)pSz < ctx->minDhKeySz) ||
            ((word16)pSz > ctx->maxDhKeySz))) {
        ret = DH_KEY_SIZE_E;
    }
    ...
    const unsigned char* buf, long sz, int format)
{
    ...
    if (res == 1) {
        /* Get the heap to use for allocations. */
        if (ssl != NULL) {
            heap = ssl->heap;
        }
        else {
    ...
    if (res == 1) {
        if (format == WOLFSSL_FILETYPE_PEM) {
            ...
            if (ret < 0) {
                /* Otherwise, try converting X9.43 format. */
                ret = PemToDer(buf, sz, X942_PARAM_TYPE, &der, heap, NULL,
                    NULL);
            }
#ifdef WOLFSSL_WPAS
    #ifndef NO_DSA
            if (ret < 0) {
                /* Otherwise, try converting DSA format. */
        ...
        if ((res == 1) && (wc_DhParamsLoad(der->buffer, der->length, p, &pSz, g,
                &gSz) < 0)) {
            res = WOLFSSL_BAD_FILETYPE;
        }
        if (res == 1) {
