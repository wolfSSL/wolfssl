...
static int QSH_GenerateSerCliSecret(WOLFSSL* ssl, byte isServer)
{
    ...
    word32 tmpSz = 0;
    buffer* buf;
    QSHKey* current = ssl->peerNtruKey;
    ...
    int SendClientKeyExchange(WOLFSSL* ssl)
    {
        ...
        if (ret == 0) {
            ...
            if (ssl->peerNtruKeyPresent) {
                ...
                QSH_KeyExchangeWrite(ssl, 0);

                /* extension type */
                c16toa(WOLFSSL_QSH, output + idx);
                idx += OPAQUE16_LEN;

                TLSX_QSHPK_Write(ssl->QSH_secret->list, output + idx);
