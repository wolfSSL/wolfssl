...
int
IsAtLeastTLSv1_2 (const WOLFSSL* ssl)
{
...
int
IsAtLeastTLSv1_3 (const ProtocolVersion pv)
{
  return (pv.major == SSLv3_MAJOR && pv.minor >= TLSv1_3_MINOR);
}

static INLINE int
IsEncryptionOn (WOLFSSL* ssl, int isSend)
{
  (void) isSend;
...
static word32
GetEntropy (unsigned char* out, word32 num_bytes)
{
  int ret = 0;
...
static void*
myAlloc (void* opaque, unsigned int item, unsigned int size)
{
  (void) opaque;
...
static void
myFree (void* opaque, void* memory)
{
  (void) opaque;
...
static int
myCompress (WOLFSSL* ssl, byte* in, int inSz, byte* out, int outSz)
{
  int err;
...
static int
myDeCompress (WOLFSSL* ssl, byte* in, int inSz, byte* out, int outSz)
{
  int err;
...
static int
ExportCipherSpecState (WOLFSSL* ssl, byte* exp, word32 len, byte ver)
{
  word32 idx = 0;
  CipherSpecs* specs;
...
static int
ExportKeyState (WOLFSSL* ssl, byte* exp, word32 len, byte ver)
{
  word32 idx = 0;
  byte sz;
  Keys* keys;
...
static int
ImportCipherSpecState (WOLFSSL* ssl, byte* exp, word32 len, byte ver)
{
  word32 idx = 0;
  CipherSpecs* specs;
...
static int
ImportKeyState (WOLFSSL* ssl, byte* exp, word32 len, byte ver)
{
  word32 idx = 0;
  byte sz;
  Keys* keys;
...
static int
dtls_export_new (WOLFSSL* ssl, byte* exp, word32 len, byte ver)
{
  int idx = 0;
  word16 zero = 0;
 ...
 * On success returns size of buffer used on failure returns a negative value */
static int
dtls_export_load (WOLFSSL* ssl, byte* exp, word32 len, byte ver)
{
  int idx = 0;
  Options* options = &ssl->options;
...
static int
ExportPeerInfo (WOLFSSL* ssl, byte* exp, word32 len, byte ver)
{
  int idx = 0;
...
static int
ImportPeerInfo (WOLFSSL* ssl, byte* buf, word32 len, byte ver)
{
  word16 idx = 0;
  word16 ipSz;
  word16 fam;
...
int
wolfSSL_dtls_export_internal (WOLFSSL* ssl, byte* buf, word32 sz)
{
  int ret;
  word32 idx = 0;
  word32 totalLen = 0;
...
int
wolfSSL_dtls_import_internal (WOLFSSL* ssl, byte* buf, word32 sz)
{
  word32 idx = 0;
  word16 length = 0;
  int version;
  ...
  idx += DTLS_EXPORT_LEN;
  if (length > sz - DTLS_EXPORT_LEN)
    { /* subtract 2 for protocol */
      return BUFFER_E;
    }
...
void
InitSSL_Method (WOLFSSL_METHOD* method, ProtocolVersion pv)
{
  method->version = pv;
...
int
InitSSL_Ctx (WOLFSSL_CTX* ctx, WOLFSSL_METHOD* method, void* heap)
{
  int ret = 0;
...
void
SSL_CtxResourceFree (WOLFSSL_CTX* ctx)
{
...
void
InitCipherSpecs (CipherSpecs* cs)
{
...
void
InitSuitesHashSigAlgo (Suites* suites, int haveECDSAsig, int haveRSAsig,
...
void
InitSuites (Suites* suites, ProtocolVersion pv, int keySz, word16 haveRSA,
 ...
 */
static INLINE void
DecodeSigAlg (const byte* input, byte* hashAlgo, byte* hsType)
{
  switch (input[0])
    {
    case NEW_SA_MAJOR:
...
void
InitX509Name (WOLFSSL_X509_NAME* name, int dynamicFlag)
{
  (void) dynamicFlag;
...
void
FreeX509Name (WOLFSSL_X509_NAME* name, void* heap)
{
  if (name != NULL)
    {
      if (name->dynamicName)
...
void
InitX509 (WOLFSSL_X509* x509, int dynamicFlag, void* heap)
{
  if (x509 == NULL)
    {
 ...
 */
static INLINE void
EncodeSigAlg (byte hashAlgo, byte hsType, byte* output)
{
  switch (hsType)
    {
#ifdef HAVE_ECC
...
static void
SetDigest (WOLFSSL* ssl, int hashAlgo)
{
  switch (hashAlgo)
    {
#ifndef NO_SHA
...
int
ConvertHashPss (int hashAlgo, enum wc_HashType* hashType, int* mgf)
{
  switch (hashAlgo)
    {
#ifdef WOLFSSL_SHA512
...
int
RsaSign (WOLFSSL* ssl, const byte* in, word32 inSz, byte* out,
...
int
RsaVerify (WOLFSSL* ssl, byte* in, word32 inSz, byte** out, int sigAlgo,
...
int
VerifyRsaSign (WOLFSSL* ssl, byte* verifySig, word32 sigSz,
...
int
RsaDec (WOLFSSL* ssl, byte* in, word32 inSz, byte** out, word32* outSz,
...
int
RsaEnc (WOLFSSL* ssl, const byte* in, word32 inSz, byte* out, word32* outSz,
        RsaKey* key, const byte* keyBuf, word32 keySz, void* ctx)
{
  int ret;
...
int
EccSign (WOLFSSL* ssl, const byte* in, word32 inSz, byte* out,
...
int
EccVerify (WOLFSSL* ssl, const byte* in, word32 inSz, const byte* out,
 ...
 */
static int
EccGetKey (WOLFSSL* ssl, ecc_key** otherKey)
{
  int ret = NO_PEER_KEY;
  ecc_key* tmpKey = NULL;
...
int
EccSharedSecret (WOLFSSL* ssl, ecc_key* priv_key, ecc_key* pub_key,
...
int
EccMakeKey (WOLFSSL* ssl, ecc_key* key, ecc_key* peer)
{
  int ret = 0;
  int keySz = 0;
 ...
 */
int
Ed25519Sign (WOLFSSL* ssl, const byte* in, word32 inSz, byte* out,
 ...
 */
int
Ed25519Verify (WOLFSSL* ssl, const byte* in, word32 inSz, const byte* msg,
 ...
 */
static int
X25519GetKey (WOLFSSL* ssl, curve25519_key** otherKey)
{
  int ret = NO_PEER_KEY;
...
static int
X25519SharedSecret (WOLFSSL* ssl, curve25519_key* priv_key,
...
static int
X25519MakeKey (WOLFSSL* ssl, curve25519_key* key,
               curve25519_key* peer)
{
  int ret = 0;
...
int
DhGenKeyPair (WOLFSSL* ssl, DhKey* dhKey,
...
int
DhAgree (WOLFSSL* ssl, DhKey* dhKey,
...
int
SetSSL_CTX (WOLFSSL* ssl, WOLFSSL_CTX* ctx, int writeDup)
{
  byte havePSK = 0;
  byte haveAnon = 0;
  byte newSSL;
...
int
InitHandshakeHashes (WOLFSSL* ssl)
{
  int ret;
...
void
FreeHandshakeHashes (WOLFSSL* ssl)
{
  if (ssl->hsHashes)
    {
#ifndef NO_OLD_TLS
...
/* init everything to 0, NULL, default values before calling anything that may
   ...
   0 on success */
int
InitSSL (WOLFSSL* ssl, WOLFSSL_CTX* ctx, int writeDup)
{
  int ret;
...
void
FreeArrays (WOLFSSL* ssl, int keep)
{
  if (ssl->arrays)
    {
      if (keep)
...
void
FreeKey (WOLFSSL* ssl, int type, void** pKey)
{
  if (ssl && pKey && *pKey)
    {
      switch (type)
...
int
AllocKey (WOLFSSL* ssl, int type, void** pKey)
{
...
static int
ReuseKey (WOLFSSL* ssl, int type, void* pKey)
{
  int ret = 0;
...
void
FreeHandshakeResources (WOLFSSL* ssl)
{
...
static INLINE void
GetSEQIncrement (WOLFSSL* ssl, int verify, word32 seq[2])
{
  if (verify)
    {
...
static INLINE void
DtlsGetSEQ (WOLFSSL* ssl, int order, word32 seq[2])
{
  if (order == PREV_ORDER)
    {
      /* Previous epoch case */
...
static INLINE void
DtlsSEQIncrement (WOLFSSL* ssl, int order)
{
  word32 seq;
...
static INLINE void
WriteSEQ (WOLFSSL* ssl, int verifyOrder, byte* out)
{
  word32 seq[2] = {0, 0};
...
DtlsMsg*
DtlsMsgNew (word32 sz, void* heap)
{
  DtlsMsg* msg = NULL;
...
void
DtlsMsgDelete (DtlsMsg* item, void* heap)
{
  (void) heap;
...
void
DtlsMsgListDelete (DtlsMsg* head, void* heap)
{
  DtlsMsg* next;
  while (head)
    {
...
static DtlsFrag*
CreateFragment (word32* begin, word32 end, const byte* data,
...
int
DtlsMsgSet (DtlsMsg* msg, word32 seq, const byte* data, byte type,
...
DtlsMsg*
DtlsMsgFind (DtlsMsg* head, word32 seq)
{
...
void
DtlsMsgStore (WOLFSSL* ssl, word32 seq, const byte* data,
...
DtlsMsg*
DtlsMsgInsert (DtlsMsg* head, DtlsMsg* item)
{
...
int
DtlsMsgPoolSave (WOLFSSL* ssl, const byte* data, word32 dataSz)
{
  DtlsMsg* item;
  int ret = 0;
...
int
DtlsMsgPoolTimeout (WOLFSSL* ssl)
{
  int result = -1;
...
int
VerifyForDtlsMsgPoolSend (WOLFSSL* ssl, byte type, word32 fragOffset)
{
  /**
...
int
DtlsMsgPoolSend (WOLFSSL* ssl, int sendOnlyFirstPacket)
{
  int ret = 0;
...
int
HashOutputRaw (WOLFSSL* ssl, const byte* output, int sz)
{
  int ret = 0;
...
int
HashOutput (WOLFSSL* ssl, const byte* output, int sz, int ivSz)
{
  int ret = 0;
  const byte* adj;
...
int
HashInput (WOLFSSL* ssl, const byte* input, int sz)
{
  int ret = 0;
  const byte* adj;
...
static void
AddRecordHeader (byte* output, word32 length, byte type, WOLFSSL* ssl)
{
...
static void
AddHandShakeHeader (byte* output, word32 length,
...
static void
AddHeaders (byte* output, word32 length, byte type, WOLFSSL* ssl)
{
...
static void
AddFragHeaders (byte* output, word32 fragSz, word32 fragOffset,
...
static int
Receive (WOLFSSL* ssl, byte* buf, word32 sz)
{
  int recvd;
...
void
ShrinkOutputBuffer (WOLFSSL* ssl)
{
...
void
ShrinkInputBuffer (WOLFSSL* ssl, int forcedFree)
{
...
static INLINE int
GrowOutputBuffer (WOLFSSL* ssl, int size)
{
  byte* tmp;
...
int
GrowInputBuffer (WOLFSSL* ssl, int size, int usedLength)
{
  byte* tmp;
...
int
CheckAvailableSize (WOLFSSL *ssl, int size)
{
  if (size < 0)
    {
...
static int
GetRecordHeader (WOLFSSL* ssl, const byte* input, word32* inOutIdx,
...
static int
GetHandShakeHeader (WOLFSSL* ssl, const byte* input, word32* inOutIdx,
...
static int
GetDtlsHandShakeHeader (WOLFSSL* ssl, const byte* input,
...
static const byte PAD1[PAD_MD5] ={0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36,
  0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36,
  0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36,
  0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36,
  0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36,
  0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36};
static const byte PAD2[PAD_MD5] ={0x5c, 0x5c, 0x5c, 0x5c, 0x5c, 0x5c, 0x5c, 0x5c,
...
static int
BuildMD5 (WOLFSSL* ssl, Hashes* hashes, const byte* sender)
{
  int ret;
...
static int
BuildSHA (WOLFSSL* ssl, Hashes* hashes, const byte* sender)
{
  int ret;
...
static int
BuildFinished (WOLFSSL* ssl, Hashes* hashes, const byte* sender)
{
  int ret = 0;
#ifdef WOLFSSL_SHA384
#ifdef WOLFSSL_SMALL_STACK
...
static int
CipherRequires (byte first, byte second, int requirement)
{
 ...
 *.z.com matches y.z.com but not x.y.z.com
   ...
   return 1 on success */
int
MatchDomainName (const char* pattern, int len, const char* str)
{
  char p, s;
...
int
CheckAltNames (DecodedCert* dCert, char* domain)
{
  int match = 0;
 ...
 */
static int
CheckForAltNames (DecodedCert* dCert, char* domain, int* checkCN)
{
  int match;
  DNS_entry* altName = NULL;
 ...
 */
int
CheckHostName (DecodedCert* dCert, char *domainName, size_t domainNameLen)
{
  int checkCN;
...
static void
AddSessionCertToChain (WOLFSSL_X509_CHAIN* chain,
...
int
CopyDecodedToX509 (WOLFSSL_X509* x509, DecodedCert* dCert)
{
  int ret = 0;
...
static void
FreeProcPeerCertArgs (WOLFSSL* ssl, void* pArgs)
{
...
int
ProcessPeerCerts (WOLFSSL* ssl, byte* input, word32* inOutIdx,
                  word32 totalSz)
{
  int ret = 0;
#ifdef WOLFSSL_ASYNC_CRYPT
            ...
            if (args->certIdx == 0)
              {
                ...
                if (args->dCert->extAuthKeyIdSet)
                  ...
                  { /* if the cert has no SKID try to match by name */
                    tp = GetTrustedPeer (ssl->ctx->cm,
...
static int
DoCertificate (WOLFSSL* ssl, byte* input, word32* inOutIdx,
               word32 size)
{
...
static int
DoCertificateStatus (WOLFSSL* ssl, byte* input, word32* inOutIdx,
                     word32 size)
{
  int ret = 0;
  byte status_type;
...
static int
DoHelloRequest (WOLFSSL* ssl, const byte* input, word32* inOutIdx,
...
int
DoFinished (WOLFSSL* ssl, const byte* input, word32* inOutIdx, word32 size,
...
static int
SanityCheckMsgReceived (WOLFSSL* ssl, byte type)
{
...
static int
DoHandShakeMsgType (WOLFSSL* ssl, byte* input, word32* inOutIdx,
                    byte type, word32 size, word32 totalSz)
{
  int ret = 0;
  word32 expectedIdx;
...
static int
DoHandShakeMsg (WOLFSSL* ssl, byte* input, word32* inOutIdx,
                word32 totalSz)
{
  int ret = 0;
  word32 inputLength;
...
UpdateHighwaterMark (word32 cur, word32 first,
...
static int
DoDtlsHandShakeMsg (WOLFSSL* ssl, byte* input, word32* inOutIdx,
                    word32 totalSz)
{
  byte type;
  word32 size;
...
static INLINE void
AeadIncrementExpIV (WOLFSSL* ssl)
{
  int i;
...
static int
Poly1305TagOld (WOLFSSL* ssl, byte* additional, const byte* out,
 ...
 */
static int
ChachaAEADEncrypt (WOLFSSL* ssl, byte* out, const byte* input,
                   word16 sz)
{
 ...
 */
static int
ChachaAEADDecrypt (WOLFSSL* ssl, byte* plain, const byte* input,
                   word16 sz)
{
  byte add[AEAD_AUTH_DATA_SZ];
  byte nonce[CHACHA20_NONCE_SZ];
...
static INLINE int
EncryptDo (WOLFSSL* ssl, byte* out, const byte* input,
...
static INLINE int
Encrypt (WOLFSSL* ssl, byte* out, const byte* input, word16 sz,
         int asyncOkay)
{
  int ret = 0;
...
static INLINE int
DecryptDo (WOLFSSL* ssl, byte* plain, const byte* input,
           word16 sz)
{
  int ret = 0;
...
static INLINE int
Decrypt (WOLFSSL* ssl, byte* plain, const byte* input,
         word16 sz)
{
  int ret = 0;
...
static int
SanityCheckCipherText (WOLFSSL* ssl, word32 encryptSz)
{
#ifdef HAVE_TRUNCATED_HMAC
...
static INLINE void
Md5Rounds (int rounds, const byte* data, int sz)
{
  wc_Md5 md5;
  int i;
...
static INLINE void
ShaRounds (int rounds, const byte* data, int sz)
{
  wc_Sha sha;
  int i;
...
static INLINE void
Sha256Rounds (int rounds, const byte* data, int sz)
{
  wc_Sha256 sha256;
  int i;
...
static INLINE void
Sha384Rounds (int rounds, const byte* data, int sz)
{
  wc_Sha384 sha384;
  int i;
...
static INLINE void
Sha512Rounds (int rounds, const byte* data, int sz)
{
  wc_Sha512 sha512;
  int i;
...
static INLINE void
RmdRounds (int rounds, const byte* data, int sz)
{
  RipeMd ripemd;
  int i;
...
/* Do dummy rounds */
static INLINE void
DoRounds (int type, int rounds, const byte* data, int sz)
{
  (void) rounds;
  (void) data;
  (void) sz;
...
static INLINE void
CompressRounds (WOLFSSL* ssl, int rounds, const byte* dummy)
{
  if (rounds)
...
static int
PadCheck (const byte* a, byte pad, int length)
{
  int i;
  int compareSum = 0;
...
static INLINE int
GetRounds (int pLen, int padLen, int t)
{
...
static int
TimingPadVerify (WOLFSSL* ssl, const byte* input, int padLen, int t,
...
int
DoApplicationData (WOLFSSL* ssl, byte* input, word32* inOutIdx)
{
...
static int
DoAlert (WOLFSSL* ssl, byte* input, word32* inOutIdx, int* type,
         word32 totalSz)
{
  byte level;
  byte code;
...
static int
GetInputData (WOLFSSL *ssl, word32 size)
{
  int in;
  int inSz;
  int maxLength;
...
static INLINE int
VerifyMac (WOLFSSL* ssl, const byte* input, word32 msgSz,
...
static int
SSL_hmac (WOLFSSL* ssl, byte* digest, const byte* in, word32 sz,
          int content, int verify)
{
...
static int
BuildMD5_CertVerify (WOLFSSL* ssl, byte* digest)
{
  int ret;
  byte md5_result[WC_MD5_DIGEST_SIZE];
#ifdef WOLFSSL_SMALL_STACK
...
static int
BuildSHA_CertVerify (WOLFSSL* ssl, byte* digest)
{
  int ret;
  byte sha_result[WC_SHA_DIGEST_SIZE];
#ifdef WOLFSSL_SMALL_STACK
...
int
BuildCertHashes (WOLFSSL* ssl, Hashes* hashes)
{
  int ret = 0;
...
static void
FreeBuildMsgArgs (WOLFSSL* ssl, void* pArgs)
{
...
int
BuildMessage (WOLFSSL* ssl, byte* output, int outSz, const byte* input,
      ...
      if (IsEncryptionOn (ssl, 1))
        {
          ...
          if (inputSz > 0)
            { /* clang thinks could be zero, let's help */
...
int
SendCertificateRequest (WOLFSSL* ssl)
{
  byte *output;
  int ret;
  int sendSz;
...
static int
BuildCertificateStatus (WOLFSSL* ssl, byte type, buffer* status,
                        byte count)
{
  byte* output = NULL;
...
int
SendCertificateStatus (WOLFSSL* ssl)
{
  int ret = 0;
  byte status_type = 0;
...
int
SendData (WOLFSSL* ssl, const void* data, int sz)
{
...
/* process input data */
int
ReceiveData (WOLFSSL* ssl, byte* output, int sz, int peek)
{
  int size;
...
/* send alert message */
int
SendAlert (WOLFSSL* ssl, int severity, int type)
{
...
void
SetErrorString (int error, char* str)
{
...
const char*
GetCipherNameInternal (const char* cipherName, int cipherSuite)
{
...
const char*
wolfSSL_get_cipher_name_internal (WOLFSSL* ssl)
{
  if (ssl == NULL)
    {
      WOLFSSL_MSG ("Bad argument");
...
const char*
wolfSSL_get_cipher_name_from_suite (const unsigned char cipherSuite,
 ...
 */
int
SetCipherList (WOLFSSL_CTX* ctx, Suites* suites, const char* list)
{
  int ret = 0;
  int idx = 0;
  int haveRSAsig = 0;
...
void
PickHashSigAlgo (WOLFSSL* ssl, const byte* hashSigAlgo,
...
void
InitHandShakeInfo (HandShakeInfo* info, WOLFSSL* ssl)
{
  int i;
...
void
FinishHandShakeInfo (HandShakeInfo* info)
{
  int i;
...
void
AddPacketName (const char* name, HandShakeInfo* info)
{
...
void
InitTimeoutInfo (TimeoutInfo* info)
{
  int i;
...
/* Free TimeoutInfo */
void
FreeTimeoutInfo (TimeoutInfo* info, void* heap)
{
  int i;
  (void) heap;
  for (i = 0; i < MAX_PACKETS_HANDSHAKE; i++)
...
void
AddPacketInfo (const char* name, TimeoutInfo* info, const byte* data,
               int sz, void* heap)
{
...
void
AddLateName (const char* name, TimeoutInfo* info)
{
...
void
AddLateRecordHeader (const RecordLayerHeader* rl, TimeoutInfo* info)
{
  /* make sure we have a valid previous one */
  if (info->numberPackets > 0 && info->numberPackets <
      MAX_PACKETS_HANDSHAKE)
...
static int
DoHelloVerifyRequest (WOLFSSL* ssl, const byte* input,
...
static INLINE int
DSH_CheckSessionId (WOLFSSL* ssl)
{
  int ret = 0;
 ...
 */
int
CheckVersion (WOLFSSL *ssl, ProtocolVersion pv)
{
#ifdef WOLFSSL_TLS13
...
int
DoServerHello (WOLFSSL* ssl, const byte* input, word32* inOutIdx,
               word32 helloSz)
{
...
static int
DoCertificateRequest (WOLFSSL* ssl, const byte* input, word32*
...
static void
FreeDskeArgs (WOLFSSL* ssl, void* pArgs)
{
...
static int
DoServerKeyExchange (WOLFSSL* ssl, const byte* input,
                     word32* inOutIdx, word32 size)
{
  int ret = 0;
#ifdef WOLFSSL_ASYNC_CRYPT
 ...
 */
static int
NtruSecretEncrypt (QSHKey* key, byte* bufIn, word32 inSz,
...
static int
NtruSecretDecrypt (QSHKey* key, byte* bufIn, word32 inSz,
                   byte* bufOut, word16* outSz)
{
  int ret;
  DRBG_HANDLE drbg;
...
static int
QSH_Encrypt (QSHKey* key, byte* in, word32 szIn,
...
int
QSH_Decrypt (QSHKey* key, byte* in, word32 szIn, byte* out, word16* szOut)
{
  int ret = 0;
  word16 size = *szOut;
 ...
 */
static int
QSH_GenerateSerCliSecret (WOLFSSL* ssl, byte isServer)
{
  int sz = 0;
 ...
 */
static word32
QSH_KeyExchangeWrite (WOLFSSL* ssl, byte isServer)
{
  int ret = 0;
...
static void
FreeSckeArgs (WOLFSSL* ssl, void* pArgs)
{
...
int
SendClientKeyExchange (WOLFSSL* ssl)
{
  int ret = 0;
#ifdef WOLFSSL_ASYNC_CRYPT
 ...
 */
int
DecodePrivateKey (WOLFSSL *ssl, word16* length)
{
  int ret;
  int keySz;
  word32 idx;
...
static void
FreeScvArgs (WOLFSSL* ssl, void* pArgs)
{
...
int
SendCertificateVerify (WOLFSSL* ssl)
{
  int ret = 0;
#ifdef WOLFSSL_ASYNC_CRYPT
...
int
SetTicket (WOLFSSL* ssl, const byte* ticket, word32 length)
{
...
static int
DoSessionTicket (WOLFSSL* ssl, const byte* input, word32* inOutIdx,
                 word32 size)
{
  word32 begin = *inOutIdx;
  word32 lifetime;
...
static void
FreeSskeArgs (WOLFSSL* ssl, void* pArgs)
{
...
int
SendServerKeyExchange (WOLFSSL* ssl)
{
  int ret;
#ifdef WOLFSSL_ASYNC_CRYPT
...
static int
FindSuite (Suites* suites, byte first, byte second)
{
  int i;
...
static int
VerifyServerSuite (WOLFSSL* ssl, word16 idx)
{
...
static int
CompareSuites (WOLFSSL* ssl, Suites* peerSuites, word16 i,
               word16 j)
{
...
int
MatchSuite (WOLFSSL* ssl, Suites* peerSuites)
{
  int ret;
  word16 i, j;
...
int
ProcessOldClientHello (WOLFSSL* ssl, const byte* input, word32* inOutIdx,
...
int
DoClientHello (WOLFSSL* ssl, const byte* input, word32* inOutIdx,
               word32 helloSz)
{
  byte b;
...
static void
FreeDcvArgs (WOLFSSL* ssl, void* pArgs)
{
...
static int
DoCertificateVerify (WOLFSSL* ssl, byte* input,
                     word32* inOutIdx, word32 size)
{
  int ret = 0;
#ifdef WOLFSSL_ASYNC_CRYPT
...
int
SendServerHelloDone (WOLFSSL* ssl)
{
  byte* output;
...
int
DoClientTicket (WOLFSSL* ssl, const byte* input, word32 len)
{
  ExternalTicket* et;
  InternalTicket* it;
  int ret;
...
static int
SendHelloVerifyRequest (WOLFSSL* ssl,
...
static void
FreeDckeArgs (WOLFSSL* ssl, void* pArgs)
{
...
static int
DoClientKeyExchange (WOLFSSL* ssl, byte* input, word32* inOutIdx,
                     word32 size)
{
  int ret;
#ifdef WOLFSSL_ASYNC_CRYPT
...
int
wolfSSL_AsyncPop (WOLFSSL* ssl, byte* state)
{
  int ret = 0;
...
int
wolfSSL_AsyncInit (WOLFSSL* ssl, WC_ASYNC_DEV* asyncDev, word32 flags)
{
  int ret;
  WOLF_EVENT* event;
...
int
wolfSSL_AsyncPush (WOLFSSL* ssl, WC_ASYNC_DEV* asyncDev)
{
  int ret;
  WOLF_EVENT* event;
