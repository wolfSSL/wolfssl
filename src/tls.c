...
static word32 TLSX_QSHPK_WriteR(QSHScheme* format, byte* output);
static word32 TLSX_QSHPK_WriteR(QSHScheme* format, byte* output)
{
    word32 offset = 0;
    word16 public_len = 0;
                                                                 ...
                                                                 byte isRequest)
{
    ...
    while ((offset_len < offset_pk) && numKeys) {
        ...
        PKLen = 0;
        PK = TLSX_QSHKeyFind_Pub(ssl->NtruKey, &PKLen, temp->name);
        r  = TLSX_UseQSHScheme(&ssl->extensions, temp->name, PK, PKLen);

        /* store peers key */
        ssl->peerNtruKeyPresent = 1;
        if (TLSX_AddQSHKey(&ssl->peerNtruKey, temp) != 0)
            return MEMORY_E;
...
int TLSX_CreateNtruKey(WOLFSSL* ssl, int type)
{
    ...
    TLSX_AddQSHKey(&ssl->NtruKey, temp);
    ...
    (void)isServer;
        ...
        if (!isServer) {
            ...
            if (!ssl->user_set_QSHSchemes) {
                ...
                public_key = TLSX_QSHKeyFind_Pub(ssl->NtruKey,
