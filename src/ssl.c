 ...
 * WOLFSSL objects, WOLF_SSL_SUCCESS on ok */
...
int wolfSSL_set_read_fd(WOLFSSL* ssl, int fd)
{
    ...
    WOLFSSL_LEAVE("SSL_set_read_fd", WOLF_SSL_SUCCESS);
    return WOLF_SSL_SUCCESS;
}
...
int wolfSSL_set_write_fd(WOLFSSL* ssl, int fd)
{
    ...
    WOLFSSL_LEAVE("SSL_set_write_fd", WOLF_SSL_SUCCESS);
    return WOLF_SSL_SUCCESS;
}
...
int wolfSSL_dtls_set_peer(WOLFSSL* ssl, void* peer, unsigned int peerSz)
{
    ...
    (void)peer;
    (void)peerSz;
    return WOLF_SSL_NOT_IMPLEMENTED;
#endif
}
...
/* return underlying connect or accept, WOLF_SSL_SUCCESS on ok */
int wolfSSL_negotiate(WOLFSSL* ssl)
{
    int err = WOLF_SSL_FATAL_ERROR;
...
#ifndef NO_DH
/* server Diffie-Hellman parameters, WOLF_SSL_SUCCESS on ok */
...
/* server ctx Diffie-Hellman parameters, WOLF_SSL_SUCCESS on ok */
...
/* let's use async hardware, WOLF_SSL_SUCCESS on ok */
...
/* set hint interval, WOLF_SSL_SUCCESS on ok */
...
/* set user context, WOLF_SSL_SUCCESS on ok */
...
int wolfSSL_shutdown(WOLFSSL* ssl)
{
    ...
    if (ssl->options.quietShutdown) {
        ...
        if (!ssl->options.isClosed && !ssl->options.connReset &&
            ...
            if (ssl->options.closeNotify)
                ...
                ret = WOLF_SSL_SHUTDOWN_NOT_DONE;
        ...
        if (ssl->options.sentNotify && !ssl->options.closeNotify) {
            ...
            if (ret < 0) {
                ...
                ssl->error = WOLF_SSL_ERROR_SYSCALL;   /* simulate OpenSSL behavior */
                ret = WOLF_SSL_SUCCESS;
            }
        }
    }
    ...
    if (ret == WOLF_SSL_SUCCESS) {
        if (wolfSSL_clear(ssl) != WOLF_SSL_SUCCESS) {
...
int wolfSSL_get_error(WOLFSSL* ssl, int ret)
{
    ...
    if (ssl->error == WANT_READ)
        return WOLF_SSL_ERROR_WANT_READ;         /* convert to OpenSSL type */
    else if (ssl->error == WANT_WRITE)
        return WOLF_SSL_ERROR_WANT_WRITE;        /* convert to OpenSSL type */
    else if (ssl->error == ZERO_RETURN)
        return WOLF_SSL_ERROR_ZERO_RETURN;       /* convert to OpenSSL type */
    return ssl->error;
}


/* retrive alert history, WOLF_SSL_SUCCESS on ok */
...
/* Set minimum downgrade version allowed, WOLF_SSL_SUCCESS on ok */
...
int AddCA(WOLFSSL_CERT_MANAGER* cm, DerBuffer** pDer, int type, int verify)
{
    ...
    return ret == 0 ? WOLF_SSL_SUCCESS : ret;
}
                                      ...
                                      int passwordSz, EncryptedInfo* info)
{
    ...
    if (ret > 0) {
        ret = WOLF_SSL_BAD_FILE; /* Reset error return */
#ifndef NO_DES3
        if (XSTRNCMP(info->name, EVP_DES_CBC, EVP_DES_SIZE) == 0)
              ...
              DerBuffer** pDer, void* heap, EncryptedInfo* info, int* eccKey)
{
...
#if defined(OPENSSL_EXTRA) || defined(HAVE_WEBSERVER)
    {
        ...
        if (line != NULL) {
            ...
            if (start == NULL) return WOLF_SSL_BAD_FILE;
            if (info == NULL)  return WOLF_SSL_BAD_FILE;
    ...
    if (encrypted_key || header == BEGIN_ENC_PRIV_KEY) {
        ...
        if (!info || !info->ctx || !info->ctx->passwd_cb)
            return WOLF_SSL_BAD_FILE;  /* no callback error */
                         ...
                         long* used, EncryptedInfo* info)
{
    ...
    if (type == CERT_TYPE && info->consumed < sz) {
        ...
        while (consumed < sz) {
            ...
            if (format == WOLF_SSL_FILETYPE_PEM) {
                ret = PemToDer(buff + consumed, remain, type, &part,
                               heap, info, &eccKey);
            }
            else {
                int length = remain;
                if (format == WOLF_SSL_FILETYPE_ASN1) {
                         ...
                         long* used, int userChain)
{
    ...
    /* check args */
    if (format != WOLF_SSL_FILETYPE_ASN1 && format != WOLF_SSL_FILETYPE_PEM
                                    && format != WOLF_SSL_FILETYPE_RAW)
        return WOLF_SSL_BAD_FILETYPE;
...
#if defined(OPENSSL_EXTRA) || defined(HAVE_WEBSERVER)
    /* for WOLF_SSL_FILETYPE_PEM, PemToDer manage the decryption if required */
    if (info->set && (format != WOLF_SSL_FILETYPE_PEM)) {
        /* decrypt */
        int   passwordSz;
#ifdef WOLFSSL_SMALL_STACK
        char* password = NULL;
    ...
    if (type == CA_TYPE) {
        ...
        FreeDer(&der);
        return WOLF_SSL_BAD_CERTTYPE;
    }

    if (type == PRIVATEKEY_TYPE && format != WOLF_SSL_FILETYPE_RAW) {
    #ifndef NO_RSA
        ...
        if (ret < 0) {
            ...
            if (PemToDer(buff + used, sz - used, CRL_TYPE, &der, NULL, &info,
                ...
                wolfSSL_CertManagerLoadCRLBuffer(ctx->cm, der->buffer,
                                                der->length, WOLF_SSL_FILETYPE_ASN1);
                FreeDer(&der);
                used += info.consumed;
                continue;
            }
                                   ...
                                   const unsigned char* buff, long sz, int type)
{
    ...
    if (cm->crl == NULL) {
        if (wolfSSL_CertManagerEnableCRL(cm, 0) != WOLF_SSL_SUCCESS) {
...
/* Verify the certificate, WOLF_SSL_SUCCESS for ok, < 0 for error */
...
/* check CRL if enabled, WOLF_SSL_SUCCESS  */
                ...
                WOLFSSL* ssl, int userChain, WOLFSSL_CRL* crl)
{
    ...
    if (fname == NULL) return WOLF_SSL_BAD_FILE;

    file = XFOPEN(fname, "rb");
    if (file == XBADFILE) return WOLF_SSL_BAD_FILE;
                                     ...
                                     const char* path)
{
    ...
    if (file)
        ret = ProcessFile(ctx, file, WOLF_SSL_FILETYPE_PEM, CA_TYPE, NULL, 0, NULL);

    if (ret == WOLF_SSL_SUCCESS && path) {
#ifndef NO_WOLFSSL_DIR
        char* name = NULL;
    #ifdef WOLFSSL_SMALL_STACK
        ...
        while (fileRet == 0 && name) {
            ret = ProcessFile(ctx, name, WOLF_SSL_FILETYPE_PEM, CA_TYPE,
                                                          NULL, 0, NULL);
            if (ret != WOLF_SSL_SUCCESS)
                break;
        ...
        if (ret == WOLF_SSL_SUCCESS && fileRet != -1) {
            ret = fileRet;
        }
                                          ...
                                          int format)
{
    ...
    if (ProcessFile(ctx, file, format, CA_TYPE, NULL, 0, NULL) == WOLF_SSL_SUCCESS)
        return WOLF_SSL_SUCCESS;
                                     ...
                                     int format)
{
    ...
    if (ProcessFile(ctx, file, format, CERT_TYPE, NULL, 0, NULL) == WOLF_SSL_SUCCESS)
        return WOLF_SSL_SUCCESS;
...
int wolfSSL_CTX_use_certificate_chain_file(WOLFSSL_CTX* ctx, const char* file)
{
   ...
   if (ProcessFile(ctx, file, WOLF_SSL_FILETYPE_PEM,CERT_TYPE,NULL,1, NULL)
                   == WOLF_SSL_SUCCESS)
       return WOLF_SSL_SUCCESS;
                                                     ...
                                                     int nid, int* c, int* idx)
{
    ...
    if (obj != NULL) {
        if (wolfSSL_sk_ASN1_OBJECT_push(sk, obj) != WOLF_SSL_SUCCESS) {
            WOLFSSL_MSG("Error pushing ASN1 object onto stack");
            wolfSSL_ASN1_OBJECT_free(obj);
            wolfSSL_sk_ASN1_OBJECT_free(sk);
            sk = NULL;
...
int wolfSSL_use_PrivateKey(WOLFSSL* ssl, WOLFSSL_EVP_PKEY* pkey)
{
    ...
    return wolfSSL_use_PrivateKey_buffer(ssl, (unsigned char*)pkey->pkey.ptr,
                                         pkey->pkey_sz, WOLF_SSL_FILETYPE_ASN1);
}
                                ...
                                long derSz)
{
    ...
    return wolfSSL_use_PrivateKey_buffer(ssl, der, derSz, WOLF_SSL_FILETYPE_ASN1);
}
...
int wolfSSL_use_certificate_ASN1(WOLFSSL* ssl, unsigned char* der, int derSz)
{
    ...
    if (der != NULL && ssl != NULL) {
        if (ProcessBuffer(NULL, der, derSz, WOLF_SSL_FILETYPE_ASN1, CERT_TYPE, ssl,
                                                        &idx, 0) == WOLF_SSL_SUCCESS)
            return WOLF_SSL_SUCCESS;
    }
...
int wolfSSL_use_certificate(WOLFSSL* ssl, WOLFSSL_X509* x509)
{
    ...
    if (x509 != NULL && ssl != NULL && x509->derCert != NULL) {
        if (ProcessBuffer(NULL, x509->derCert->buffer, x509->derCert->length,
                     WOLF_SSL_FILETYPE_ASN1, CERT_TYPE, ssl, &idx, 0) == WOLF_SSL_SUCCESS)
            return WOLF_SSL_SUCCESS;
    }
...
int wolfSSL_use_certificate_chain_file(WOLFSSL* ssl, const char* file)
{
   ...
   if (ProcessFile(ssl->ctx, file, WOLF_SSL_FILETYPE_PEM, CERT_TYPE,
                   ssl, 1, NULL) == WOLF_SSL_SUCCESS)
       return WOLF_SSL_SUCCESS;
...
int wolfSSL_CTX_use_NTRUPrivateKey_file(WOLFSSL_CTX* ctx, const char* file)
{
    ...
    if (ProcessFile(ctx, file, WOLF_SSL_FILETYPE_RAW, PRIVATEKEY_TYPE, NULL, 0, NULL)
                         == WOLF_SSL_SUCCESS) {
        ctx->haveNTRU = 1;
        return WOLF_SSL_SUCCESS;
    }
...
void wolfSSL_CTX_set_verify(WOLFSSL_CTX* ctx, int mode, VerifyCallback vc)
{
    ...
    if (mode & WOLF_SSL_VERIFY_FAIL_EXCEPT_PSK) {
...
int wolfSSL_SetServerID(WOLFSSL* ssl, const byte* id, int len, int newSession)
{
    ...
    if (newSession == 0) {
        ...
        if (session) {
            if (SetSession(ssl, session) != WOLF_SSL_SUCCESS) {
...
int wolfSSL_memsave_session_cache(void* mem, int sz)
{
    ...
    WOLFSSL_LEAVE("wolfSSL_memsave_session_cache", WOLF_SSL_SUCCESS);
...
int wolfSSL_memrestore_session_cache(const void* mem, int sz)
{
    ...
    WOLFSSL_LEAVE("wolfSSL_memrestore_session_cache", WOLF_SSL_SUCCESS);
...
long wolfSSL_CTX_set_session_cache_mode(WOLFSSL_CTX* ctx, long mode)
{
    ...
    if (mode == WOLF_SSL_SESS_CACHE_OFF)
        ctx->sessionCacheOff = 1;

    if ((mode & WOLF_SSL_SESS_CACHE_NO_AUTO_CLEAR) != 0)
        ctx->sessionCacheFlushOff = 1;

#ifdef HAVE_EXT_CACHE
    if ((mode & WOLF_SSL_SESS_CACHE_NO_INTERNAL_STORE) != 0)
...
int wolfSSL_CTX_set_cipher_list(WOLFSSL_CTX* ctx, const char* list)
{
    ...
    return (SetCipherList(ctx, ctx->suites, list)) ? WOLF_SSL_SUCCESS : WOLF_SSL_FAILURE;
}
...
int wolfSSL_set_cipher_list(WOLFSSL* ssl, const char* list)
{
    ...
    return (SetCipherList(ssl->ctx, ssl->suites, list)) ? WOLF_SSL_SUCCESS : WOLF_SSL_FAILURE;
}
...
/* user may need to alter init dtls recv timeout, WOLF_SSL_SUCCESS on ok */
...
/* user may need to alter max dtls recv timeout, WOLF_SSL_SUCCESS on ok */
...
{
    int result = WOLF_SSL_SUCCESS;

    if (!ssl->options.handShakeDone &&
        (DtlsMsgPoolTimeout(ssl) < 0 || DtlsMsgPoolSend(ssl, 0) < 0)) {

        result = WOLF_SSL_FATAL_ERROR;
    }
    return result;
}
...
/* client only parts */
    ...
    int wolfSSL_connect(WOLFSSL* ssl)
    {
        ...
        case SECOND_REPLY_DONE:
            ...
            WOLFSSL_LEAVE("SSL_connect()", WOLF_SSL_SUCCESS);
            return WOLF_SSL_SUCCESS;
...
int SetSession(WOLFSSL* ssl, WOLFSSL_SESSION* session)
{
    ...
    return WOLF_SSL_FAILURE;  /* session timed out */
}
...
{
    int row, col, result = WOLF_SSL_FAILURE;
...
/* requires session_mutex lock held, WOLF_SSL_SUCCESS on ok */
...
/* turn on wolfSSL zlib compression
   returns WOLF_SSL_SUCCESS for success, else error (not built in)
*/
...
/* old NO_FILESYSTEM end */
   ...
   /* returns previous set cache size which stays constant */
    ...
    /* store keys returns WOLF_SSL_SUCCESS or -1 on error */
    ...
    void wolfSSL_set_shutdown(WOLFSSL* ssl, int opt)
    {
        ...
        ssl->options.sentNotify =  (opt&WOLF_SSL_SENT_SHUTDOWN) > 0;
        ssl->options.closeNotify = (opt&WOLF_SSL_RECEIVED_SHUTDOWN) > 0;
    }
    ...
    int wolfSSL_BIO_read(WOLFSSL_BIO* bio, void* buf, int len)
    {
        ...
        if (ret == 0)
            ...
            if ( !(err == WOLF_SSL_ERROR_WANT_READ || err == WOLF_SSL_ERROR_WANT_WRITE) )
                front->eof = 1;
        }
        return ret;
    }
...
int wolfSSL_EVP_MD_type(const WOLFSSL_EVP_MD *md)
{
    ...
    /* return WOLF_SSL_SUCCESS on ok, 0 on failure to match API compatibility */
                          ...
                          word32 len)
    {
        ...
        return WOLF_SSL_SUCCESS;  /* success */
    }
...
#include "wolfcrypt/src/evp.c"
    ...
    /* store for external read of iv, WOLF_SSL_SUCCESS on success */
    ...
    /* set internal IV from external, WOLF_SSL_SUCCESS on success */
    ...
    {
        return WOLF_SSL_SUCCESS;  /* wolfCrypt provides enough seed internally */
    }
    ...
    {
        /* WOLF_SSL_MODE_ACCEPT_MOVING_WRITE_BUFFER is wolfSSL default mode */
...
static void ExternalFreeX509(WOLFSSL_X509* x509)
{
    ...
    /* write X509 serial number in unsigned binary to buffer
       ...
       return WOLF_SSL_SUCCESS on success */
...
WOLFSSL_X509* wolfSSL_X509_load_certificate_file(const char* fname, int format)
{
    ...
    /* Check the inputs */
    if ((fname == NULL) ||
        (format != WOLF_SSL_FILETYPE_ASN1 && format != WOLF_SSL_FILETYPE_PEM))
        return NULL;
...
int wolfSSL_get_shutdown(const WOLFSSL* ssl)
{
    ...
    /* in OpenSSL, WOLF_SSL_SENT_SHUTDOWN = 1, when closeNotifySent   *
     * WOLF_SSL_RECEIVED_SHUTDOWN = 2, from close notify or fatal err */
                                 ...
                                 const char* file, long type)
{
    ...
    do {
        if (XSTRNSTR((char*)curr, BEGIN_X509_CRL, (unsigned int)sz) != NULL) {
            ...
            ret = BufferLoadCRL(cm->crl, curr, sz, WOLF_SSL_FILETYPE_PEM, 1);
            if (ret != WOLF_SSL_SUCCESS)
                goto end;
#endif
...
{
    int result = WOLF_SSL_FATAL_ERROR;
...
unsigned long wolfSSL_set_options(WOLFSSL* ssl, unsigned long op)
{
    ...
    if ((ssl->options.mask & WOLF_SSL_OP_NO_SSLv2) == WOLF_SSL_OP_NO_SSLv2) {
        WOLFSSL_MSG("\tWOLF_SSL_OP_NO_SSLv2 : wolfSSL does not support SSLv2");
    }
...
long wolfSSL_set_tmp_dh(WOLFSSL *ssl, WOLFSSL_DH *dh)
{
    ...
    return pSz > 0 && gSz > 0 ? ret : WOLF_SSL_FATAL_ERROR;
}
#endif /* !NO_DH */
...
long wolfSSL_CTX_add_extra_chain_cert(WOLFSSL_CTX* ctx, WOLFSSL_X509* x509)
{
    ...
    if (ctx->certificate == NULL) {
        ...
        ret = ProcessBuffer(ctx, der, derSz, WOLF_SSL_FILETYPE_ASN1, CERT_TYPE,
                            NULL, NULL, 1);
        if (ret != WOLF_SSL_SUCCESS) {
                                     ...
                                     WOLFSSL_BN_ULONG w)
{
    ...
    if (bn == NULL || bn->internal == NULL) {
        WOLFSSL_MSG("bn NULL error");
        return (WOLFSSL_BN_ULONG)WOLF_SSL_FATAL_ERROR;
    }
...
int wolfSSL_DH_generate_key(WOLFSSL_DH* dh)
{
    ...
    if (dh == NULL || dh->p == NULL || dh->g == NULL)
        WOLFSSL_MSG("Bad function arguments");
    else if (dh->inSet == 0 && SetDhInternal(dh) != WOLF_SSL_SUCCESS)
...
WOLFSSL_DH *wolfSSL_DSA_dup_DH(const WOLFSSL_DSA *dsa)
{
    ...
    if (dsa->p != NULL &&
        SetIndividualInternal(((WOLFSSL_DSA*)dsa)->p, &key->p) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("rsa p key error");
        wolfSSL_DH_free(dh);
        return NULL;
    }
    if (dsa->g != NULL &&
        SetIndividualInternal(((WOLFSSL_DSA*)dsa)->g, &key->g) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("rsa g key error");
        wolfSSL_DH_free(dh);
        return NULL;
    }

    if (SetIndividualExternal(&dh->p, &key->p) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("dsa p key error");
        wolfSSL_DH_free(dh);
        return NULL;
    }
    if (SetIndividualExternal(&dh->g, &key->g) != WOLF_SSL_SUCCESS) {
...
static int SetDsaExternal(WOLFSSL_DSA* dsa)
{
    ...
    if (SetIndividualExternal(&dsa->p, &key->p) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("dsa p key error");
        return WOLF_SSL_FATAL_ERROR;
    }

    if (SetIndividualExternal(&dsa->q, &key->q) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("dsa q key error");
        return WOLF_SSL_FATAL_ERROR;
    }

    if (SetIndividualExternal(&dsa->g, &key->g) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("dsa g key error");
        return WOLF_SSL_FATAL_ERROR;
    }

    if (SetIndividualExternal(&dsa->pub_key, &key->y) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("dsa y key error");
        return WOLF_SSL_FATAL_ERROR;
    }

    if (SetIndividualExternal(&dsa->priv_key, &key->x) != WOLF_SSL_SUCCESS) {
...
static int SetDsaInternal(WOLFSSL_DSA* dsa)
{
    ...
    if (dsa->p != NULL &&
        SetIndividualInternal(dsa->p, &key->p) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("rsa p key error");
        return WOLF_SSL_FATAL_ERROR;
    }

    if (dsa->q != NULL &&
        SetIndividualInternal(dsa->q, &key->q) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("rsa q key error");
        return WOLF_SSL_FATAL_ERROR;
    }

    if (dsa->g != NULL &&
        SetIndividualInternal(dsa->g, &key->g) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("rsa g key error");
        return WOLF_SSL_FATAL_ERROR;
    }

    if (dsa->pub_key != NULL) {
        if (SetIndividualInternal(dsa->pub_key, &key->y) != WOLF_SSL_SUCCESS) {
    ...
    if (dsa->priv_key != NULL) {
        if (SetIndividualInternal(dsa->priv_key, &key->x) != WOLF_SSL_SUCCESS) {
...
static int SetRsaExternal(WOLFSSL_RSA* rsa)
{
    ...
    if (SetIndividualExternal(&rsa->n, &key->n) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("rsa n key error");
        return WOLF_SSL_FATAL_ERROR;
    }

    if (SetIndividualExternal(&rsa->e, &key->e) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("rsa e key error");
        return WOLF_SSL_FATAL_ERROR;
    }

    if (SetIndividualExternal(&rsa->d, &key->d) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("rsa d key error");
        return WOLF_SSL_FATAL_ERROR;
    }

    if (SetIndividualExternal(&rsa->p, &key->p) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("rsa p key error");
        return WOLF_SSL_FATAL_ERROR;
    }

    if (SetIndividualExternal(&rsa->q, &key->q) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("rsa q key error");
        return WOLF_SSL_FATAL_ERROR;
    }

    if (SetIndividualExternal(&rsa->dmp1, &key->dP) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("rsa dP key error");
        return WOLF_SSL_FATAL_ERROR;
    }

    if (SetIndividualExternal(&rsa->dmq1, &key->dQ) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("rsa dQ key error");
        return WOLF_SSL_FATAL_ERROR;
    }

    if (SetIndividualExternal(&rsa->iqmp, &key->u) != WOLF_SSL_SUCCESS) {
...
static int SetRsaInternal(WOLFSSL_RSA* rsa)
{
    ...
    if (SetIndividualInternal(rsa->n, &key->n) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("rsa n key error");
        return WOLF_SSL_FATAL_ERROR;
    }

    if (SetIndividualInternal(rsa->e, &key->e) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("rsa e key error");
        return WOLF_SSL_FATAL_ERROR;
    }
    ...
    if (rsa->d != NULL) {
        if (SetIndividualInternal(rsa->d, &key->d) != WOLF_SSL_SUCCESS) {
            WOLFSSL_MSG("rsa d key error");
            return WOLF_SSL_FATAL_ERROR;
        }
    ...
    if (rsa->p != NULL &&
        SetIndividualInternal(rsa->p, &key->p) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("rsa p key error");
        return WOLF_SSL_FATAL_ERROR;
    }

    if (rsa->q != NULL &&
        SetIndividualInternal(rsa->q, &key->q) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("rsa q key error");
        return WOLF_SSL_FATAL_ERROR;
    }

    if (rsa->dmp1 != NULL &&
        SetIndividualInternal(rsa->dmp1, &key->dP) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("rsa dP key error");
        return WOLF_SSL_FATAL_ERROR;
    }

    if (rsa->dmq1 != NULL &&
        SetIndividualInternal(rsa->dmq1, &key->dQ) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("rsa dQ key error");
        return WOLF_SSL_FATAL_ERROR;
    }

    if (rsa->iqmp != NULL &&
        SetIndividualInternal(rsa->iqmp, &key->u) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("rsa u key error");
        return WOLF_SSL_FATAL_ERROR;
    }
                                ...
                                void* cb)
{
...
#ifdef WOLFSSL_KEY_GEN
    {
        ...
        else if (SetRsaExternal(rsa) != WOLF_SSL_SUCCESS)
...
int wolfSSL_RSA_blinding_on(WOLFSSL_RSA* rsa, WOLFSSL_BN_CTX* bn)
{
    ...
    return WOLF_SSL_SUCCESS;  /* on by default */
}
...
int wolfSSL_DSA_generate_key(WOLFSSL_DSA* dsa)
{
    ...
    if (dsa->inSet == 0) {
        ...
        if (SetDsaInternal(dsa) != WOLF_SSL_SUCCESS) {
...
#ifdef WOLFSSL_KEY_GEN
    {
        ...
        if (rng) {
            ...
            else if (SetDsaExternal(dsa) != WOLF_SSL_SUCCESS)
...
/* return WOLF_SSL_SUCCESS on success, < 0 otherwise */
                           ...
                           unsigned int* sigLen, WOLFSSL_RSA* rsa)
{
    ...
    if (rsa->inSet == 0)
    {
        ...
        if (SetRsaInternal(rsa) != WOLF_SSL_SUCCESS) {
            WOLFSSL_MSG("SetRsaInternal failed");
            return 0;
        }
    }
...
/* generate p-1 and q-1, WOLF_SSL_SUCCESS on ok */
                         ...
                         unsigned char* passwd, int passwdSz, byte **cipherInfo)
{
    ...
    if (wolfSSL_RAND_bytes(info->iv, info->ivSz) != WOLF_SSL_SUCCESS) {
    ...
    if (wolfssl_encrypt_buffer_key(der, *derSz,
                                   passwd, passwdSz, info) != WOLF_SSL_SUCCESS) {
...
static int SetECPointInternal(WOLFSSL_EC_POINT *p)
{
    ...
    if (p->X != NULL && SetIndividualInternal(p->X, point->x) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("ecc point X error");
        return WOLF_SSL_FATAL_ERROR;
    }

    if (p->Y != NULL && SetIndividualInternal(p->Y, point->y) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("ecc point Y error");
        return WOLF_SSL_FATAL_ERROR;
    }

    if (p->Z != NULL && SetIndividualInternal(p->Z, point->z) != WOLF_SSL_SUCCESS) {
...
static int SetECPointExternal(WOLFSSL_EC_POINT *p)
{
    ...
    if (SetIndividualExternal(&p->X, point->x) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("ecc point X error");
        return WOLF_SSL_FATAL_ERROR;
    }

    if (SetIndividualExternal(&p->Y, point->y) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("ecc point Y error");
        return WOLF_SSL_FATAL_ERROR;
    }

    if (SetIndividualExternal(&p->Z, point->z) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("ecc point Z error");
        return WOLF_SSL_FATAL_ERROR;
    }
...
static int SetECKeyExternal(WOLFSSL_EC_KEY* eckey)
{
    ...
    if (eckey->pub_key->internal != NULL) {
        ...
        if (SetECPointExternal(eckey->pub_key) != WOLF_SSL_SUCCESS) {
    ...
    if (key->type == ECC_PRIVATEKEY) {
        if (SetIndividualExternal(&eckey->priv_key, &key->k) != WOLF_SSL_SUCCESS) {
...
static int SetECKeyInternal(WOLFSSL_EC_KEY* eckey)
{
    ...
    if (eckey->pub_key != NULL) {
        if (SetECPointInternal(eckey->pub_key) != WOLF_SSL_SUCCESS) {
    ...
    if (eckey->priv_key != NULL) {
        if (SetIndividualInternal(eckey->priv_key, &key->k) != WOLF_SSL_SUCCESS) {
                                   ...
                                   const WOLFSSL_BIGNUM *priv_key)
{
    ...
    if (SetECKeyInternal(key) != WOLF_SSL_SUCCESS) {
...
int wolfSSL_EC_KEY_generate_key(WOLFSSL_EC_KEY *key)
{
    ...
    if (SetECKeyExternal(key) != WOLF_SSL_SUCCESS) {
                                  ...
                                  const WOLFSSL_EC_POINT *pub)
{
    ...
    if (pub->inSet == 0) {
        if (SetECPointInternal((WOLFSSL_EC_POINT *)pub) != WOLF_SSL_SUCCESS) {
                        ...
                        unsigned char *out, unsigned int *len)
{
    ...
    if (p->inSet == 0) {
        ...
        if (SetECPointInternal((WOLFSSL_EC_POINT *)p) != WOLF_SSL_SUCCESS) {
                        ...
                        const WOLFSSL_EC_GROUP *group, WOLFSSL_EC_POINT *p)
{
    ...
    if (p->exSet == 0) {
        ...
        if (SetECPointExternal(p) != WOLF_SSL_SUCCESS) {
                                                ...
                                                WOLFSSL_BN_CTX *ctx)
{
    ...
    if (point->inSet == 0) {
        ...
        if (SetECPointInternal((WOLFSSL_EC_POINT *)point) != WOLF_SSL_SUCCESS) {
            WOLFSSL_MSG("SetECPointInternal failed");
            return WOLF_SSL_FAILURE;
        }
    }
                         ...
                         const WOLFSSL_BIGNUM *m, WOLFSSL_BN_CTX *ctx)
{
    ...
    if (q->inSet == 0) {
        ...
        if (SetECPointInternal((WOLFSSL_EC_POINT *)q) != WOLF_SSL_SUCCESS) {
    ...
    if (rng) {
        ...
        if (mp_init_multi(&sig_r, &sig_s, NULL, NULL, NULL, NULL) == MP_OKAY) {
            if (wc_ecc_sign_hash_ex(d, dlen, rng, (ecc_key*)key->internal,
                ...
                else if (SetIndividualExternal(&(sig->r), &sig_r)!=WOLF_SSL_SUCCESS){
                    WOLFSSL_MSG("ecdsa r key error");
                    wolfSSL_ECDSA_SIG_free(sig);
                    sig = NULL;
                }
                else if (SetIndividualExternal(&(sig->s), &sig_s)!=WOLF_SSL_SUCCESS){
                                           ...
                                           void *out, size_t *outlen))
{
    ...
    if (ecdh->inSet == 0)
    {
        ...
        if (SetECKeyInternal(ecdh) != WOLF_SSL_SUCCESS) {
            WOLFSSL_MSG("SetECKeyInternal failed");
            return WOLF_SSL_FATAL_ERROR;
        }
    }
                                       ...
                                       unsigned char **pem, int *plen)
{
    ...
    if (ecc->inSet == 0) {
        ...
        if (SetECKeyInternal(ecc) != WOLF_SSL_SUCCESS) {
            WOLFSSL_MSG("SetDsaInternal failed");
            return WOLF_SSL_FAILURE;
        }
    }
...
/* return WOLF_SSL_SUCCESS if success, WOLF_SSL_FATAL_ERROR if error */
int wolfSSL_RSA_LoadDer(WOLFSSL_RSA* rsa, const unsigned char* derBuf, int derSz)
{
    ...
    if (SetRsaExternal(rsa) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("SetRsaExternal failed");
        return WOLF_SSL_FATAL_ERROR;
    }
...
int wolfSSL_DSA_LoadDer(WOLFSSL_DSA* dsa, const unsigned char* derBuf, int derSz)
{
    ...
    if (SetDsaExternal(dsa) != WOLF_SSL_SUCCESS) {
        WOLFSSL_MSG("SetDsaExternal failed");
        return WOLF_SSL_FATAL_ERROR;
    }
...
/* Get peer's PEM certificate at index (idx), output to buffer if inLen big
   ...
   outLen. Output length is in *outLen WOLF_SSL_SUCCESS on ok */
...
void* wolfSSL_GetRsaDecCtx(WOLFSSL* ssl)
{
    ...
    int wolfSSL_BIO_read_filename(WOLFSSL_BIO *b, const char *name) {
        ...
        if ((wolfSSL_BIO_get_fp(b, &fp) == WOLF_SSL_SUCCESS) && (fp != NULL))
        {
            XFCLOSE(fp);
        }
        ...
        if (wolfSSL_BIO_set_fp(b, fp, BIO_CLOSE) != WOLF_SSL_SUCCESS) {
            XFCLOSE(fp);
            return WOLF_SSL_BAD_FILE;
        }
...
WOLFSSL_BIO *wolfSSL_BIO_new_file(const char *filename, const char *mode)
{
    ...
    if (wolfSSL_BIO_set_fp(bio, fp, BIO_CLOSE) != WOLF_SSL_SUCCESS) {
        XFCLOSE(fp);
        wolfSSL_BIO_free(bio);
        bio = NULL;
    }
...
/* Intialize ctx->dh with dh's params. Return WOLF_SSL_SUCCESS on ok */
...
{
    if (ssl && ctx && SetSSL_CTX(ssl, ctx, 0) == WOLF_SSL_SUCCESS)
        return ssl->ctx;
    return NULL;
}
...
int wolfSSL_CTX_get_verify_mode(WOLFSSL_CTX* ctx)
{
    ...
    if (ctx->verifyPeer)
        mode |= WOLF_SSL_VERIFY_PEER;
    else if (ctx->verifyNone)
        mode |= WOLF_SSL_VERIFY_NONE;

    if (ctx->failNoCert)
        mode |= WOLF_SSL_VERIFY_FAIL_IF_NO_PEER_CERT;

    if (ctx->failNoCertxPSK)
        mode |= WOLF_SSL_VERIFY_FAIL_EXCEPT_PSK;
...
#else /* WOLFSSL_KEY_GEN */
    ed25519_key key;
    int ret = WOLF_SSL_FAILURE, check = 0;
    ...
    defined(WOLFSSL_MYSQL_COMPATIBLE)
    {
        ...
        while (1) {
            ...
            if (ret != WANT_READ && ret != WANT_WRITE &&
                    ret != ZERO_RETURN && ret != WOLF_SSL_ERROR_ZERO_RETURN &&
 ...
 * returns WOLF_SSL_SUCCESS to indicate success.
 */
