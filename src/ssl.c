...
{
    WOLF_STACK_OF(WOLFSSL_CIPHER)* ret;
    ...
    /* A context is shared between connections, so the stack is built under
     * the same lock that guards the suites it is built from.  Two threads
     * arriving here together would otherwise each build one, and every stack
     * but the last would be leaked. */
    if (wolfSSL_RefWithMutexLock(&((WOLFSSL_CTX*)ctx)->ref) != 0) {
        WOLFSSL_MSG("Failed to lock CTX mutex for cipher stack");
        return NULL;
    }

    /* Populate the stack on first use, re-checked now the lock is held. */
    if (ctx->suitesStack == NULL) {
        ...
        /* Highest priority suite ends up on top of the stack.  A failed
         * allocation stops the loop before it starts rather than returning,
         * which would leave the lock held. */
        for (i = ctx->suites->suiteSz - 2;
                (ctx->suitesStack != NULL) && (i >= 0); i -= 2) {
            struct WOLFSSL_CIPHER cipher;
    ...
    if (wolfSSL_RefWithMutexUnlock(&((WOLFSSL_CTX*)ctx)->ref) != 0) {
        WOLFSSL_MSG("Failed to unlock CTX mutex after cipher stack build");
    }
