...
* **Behavioral change (certificate policies that cannot be decoded)**: a
  ...
  Previously such a certificate failed to parse with `ASN_PARSE_E`. The
  decoded policy list can therefore be a subset of what the certificate
  carries, so any application that makes decisions from
  `DecodedCert.extCertPolicies` or `WOLFSSL_X509` certificate policies must
  also check the truncated flag.
...
* **API (`wc_SetExtKeyUsageOID` and `wc_SetCustomExtension` return codes)**:
  ...
  Invalid strings, including an arc with a leading zero such as `"1.02.3"`,
