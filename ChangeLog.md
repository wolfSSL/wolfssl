...
* **Behavioral change (CryptoCell RSA honours the padding scheme)**: the
  CryptoCell arms in `RsaPublicEncryptEx()`/`RsaPrivateDecryptEx()` chose the
  hardware on the operation direction and block type alone, so an OAEP, PSS or
  unpadded request was handed to `CRYS_RSA_PKCS1v15_*` and served as PKCS#1
  v1.5 under a different name.  They now require `WC_RSA_PKCSV15_PAD` and
  anything else falls through to the wolfCrypt software path, so RSA-PSS and
  OAEP produce what the caller asked for.  Two visible differences: an
  application that both encrypted and decrypted through CryptoCell with a
  non-v1.5 `pad_type` was interoperating with nothing but itself and now
  changes wire format, and a private-key OAEP or PSS operation on a key pair
  generated by `cc310_RSA_GenerateKeyPair()` now fails, because that generator
  writes only the public half back into the `RsaKey` and the software path has
  no `d` to work with.  Compressed-point ECC import also builds again on the
  ports that exclude `mp_sqrtmod_prime()`, which `HAVE_COMP_KEY` called
  regardless.  Verified on CryptoCell, where it was an implicit-declaration
  compile error; the ATECC508A/608A gating is identical, and on the TA100 the
  helper was declared without a definition, so that build failed at link.
...
* Added `WOLFSSL_NO_RSA_SW` and `WOLFSSL_NO_ECC_SW` for hardware-only public-key
  builds on the NXP SE05x and Arm CryptoCell-310 ports. `WOLFSSL_NO_RSA_SW`
  leaves out the software modular exponentiation, the prime search behind
  `wc_MakeRsaKey()` and the software key consistency check (it forces
  `WOLFSSL_RSA_KEY_CHECK` off), and when RSA-PSS is not enabled it drops the
  PKCS#1 padding generation and the OAEP/PSS mask-generation helpers as well,
  roughly halving `rsa.o`. `WOLFSSL_NO_ECC_SW` leaves out the projective point
  arithmetic, the scalar multiplication, the software signer, the software ECDH
  and `wc_ecc_make_pub()`, taking a 3-curve `ecc.o` from about 30 KB to 17 KB;
  it extends to the SE05x port the exclusions `ecc.c` already applied to the
  ATECC508A/608A, TA100, CryptoCell, SiLabs and KCAPI ports. Operations with no
  hardware provider return `NOT_COMPILED_IN`: raw `wc_RsaFunction()`/
  `wc_RsaDirect()`, RSA-PSS, and ECDH with a private key that is not resident
  in the SE050 (PKCS#7 KARI included), and `wc_ecc_make_pub()`, which derives a
  public key from an imported private scalar. In an `OPENSSL_EXTRA`
  build the same applies to `EC_POINT_add()`, `EC_POINT_mul()` and
  `ECDH_compute_key()`, which are built on the same curve arithmetic.
  `EC_POINT_is_at_infinity()` and the Jacobian-to-affine conversion behind
  `EC_POINT_get_affine_coordinates_GFp()` and `EC_POINT_invert()` remain but
  always report failure. Both macros are documented in
  `wolfcrypt/src/port/nxp/README_SE050.md` and `IDE/CRYPTOCELL/README.md`. by @SparkiDev

## Fixes

* **Fix (`make check` did not link on CryptoCell builds)**: three ECC API test
  bodies in `tests/api/test_ecc.c` called `wc_ecc_mulmod()` and
  `wc_ecc_point_is_at_infinity()`, which `ecc.c` excludes for the CryptoCell
  port, so `unit.test` failed to link with undefined references. Their guards
  now list `WOLFSSL_CRYPTOCELL` alongside the other ports that replace the
  software curve arithmetic.
