...
/*!
    \ingroup AES
    \brief This function sets the key for AES encryption using the OpenSSL
    compatibility layer. It initializes the AES key structure with the
    provided key for encryption operations.

    \return 0 On successfully setting the encryption key.
    \return WOLFSSL_FATAL_ERROR If key or aes is NULL, or if setting the
    key fails.

    \param key pointer to the key buffer containing the encryption key
    \param bits number of bits in the key (128, 192, or 256)
    \param aes pointer to the WOLFSSL_AES_KEY structure to initialize

    _Example_
    \code
    WOLFSSL_AES_KEY aesKey;
    byte key[16] = { /* 128-bit key */ };
    int ret = wolfSSL_AES_set_encrypt_key(key, 128, &aesKey);
    if (ret != 0) {
        // failed to set encryption key
    }
    \endcode

    \sa wolfSSL_AES_set_decrypt_key
    \sa wolfSSL_AES_cbc_encrypt
*/
...
/*!
    \ingroup AES
    \brief This function sets the key for AES decryption using the OpenSSL
    compatibility layer. It initializes the AES key structure with the
    provided key for decryption operations.

    \return 0 On successfully setting the decryption key.
    \return WOLFSSL_FATAL_ERROR If key or aes is NULL, or if setting the
    key fails.

    \param key pointer to the key buffer containing the decryption key
    \param bits number of bits in the key (128, 192, or 256)
    \param aes pointer to the WOLFSSL_AES_KEY structure to initialize
    ...
    WOLFSSL_AES_KEY aesKey;
    byte key[16] = { /* 128-bit key */ };
    int ret = wolfSSL_AES_set_decrypt_key(key, 128, &aesKey);
    if (ret != 0) {
        // failed to set decryption key
    }
    \endcode

    \sa wolfSSL_AES_set_encrypt_key
    \sa wolfSSL_AES_cbc_encrypt
*/
...
/*!
    \ingroup AES
    \brief This function performs AES CBC encryption or decryption using
    the OpenSSL compatibility layer. It encrypts or decrypts the input
    data and updates the IV for chaining.

    \return none No return value.

    \param in pointer to the input buffer containing data to encrypt or
    decrypt
    \param out pointer to the output buffer to store the result
    \param len length of the input data in bytes (must be multiple of
    AES_BLOCK_SIZE)
    \param key pointer to the WOLFSSL_AES_KEY structure containing the key
    \param iv pointer to the initialization vector (updated after operation)
    \param enc encryption direction: AES_ENCRYPTION to encrypt,
    ...
    wolfSSL_AES_set_encrypt_key(key, 128, &aesKey);
    wolfSSL_AES_cbc_encrypt(plaintext, ciphertext, 32, &aesKey, iv,
                            AES_ENCRYPTION);
    \endcode
...
void wolfSSL_AES_cbc_encrypt(const unsigned char *in, unsigned char* out,
                              size_t len, WOLFSSL_AES_KEY *key,
                              unsigned char* iv, const int enc);

/*!
    \ingroup AES
    \brief This function performs AES ECB encryption or decryption of a
    single block using the OpenSSL compatibility layer. ECB mode encrypts
    each block independently without an IV. Warning: ECB mode is not
    recommended for most use cases as it does not provide semantic security.

    \return none No return value.

    \param in pointer to the input buffer containing one AES block to
    encrypt or decrypt
    \param out pointer to the output buffer to store the result
    \param key pointer to the WOLFSSL_AES_KEY structure containing the key
    \param enc encryption direction: AES_ENCRYPTION to encrypt,
    ...
    WOLFSSL_AES_KEY aesKey;
    byte key[16] = { /* 128-bit key */ };
    byte plaintext[16] = { /* one block of data */ };
    byte ciphertext[16];
    
    wolfSSL_AES_set_encrypt_key(key, 128, &aesKey);
    wolfSSL_AES_ecb_encrypt(plaintext, ciphertext, &aesKey, AES_ENCRYPTION);
    \endcode
...
/*!
    \ingroup AES
    \brief This function performs AES CFB-128 encryption or decryption
    using the OpenSSL compatibility layer. CFB-128 mode operates on
    128-bit blocks and updates the IV for stream cipher operation.
    ...
    \param key pointer to the WOLFSSL_AES_KEY structure containing the key
    \param iv pointer to the initialization vector (updated after operation)
    \param num pointer to the number of bytes processed in the current block
    \param enc encryption direction: AES_ENCRYPTION to encrypt,
    AES_DECRYPTION to decrypt
    ...
    wolfSSL_AES_set_encrypt_key(key, 128, &aesKey);
    wolfSSL_AES_cfb128_encrypt(plaintext, ciphertext, 100, &aesKey, iv,
                               &num, AES_ENCRYPTION);
    \endcode
...
/*!
    \ingroup AES
    \brief This function wraps (encrypts) a key using AES Key Wrap
    algorithm (RFC 3394) with the OpenSSL compatibility layer. This is
    commonly used to securely transport cryptographic keys.

    \return Length of wrapped key in bytes on success.
    \return 0 If key, iv, out, or in is NULL, or if key length is invalid.

    \param key pointer to the WOLFSSL_AES_KEY structure containing the
    key-encryption key
    \param iv pointer to the initialization vector (typically the default
    IV 0xA6A6A6A6A6A6A6A6, or NULL to use default)
    \param out pointer to the output buffer to store the wrapped key
    \param in pointer to the input buffer containing the key to wrap
    \param inlen length of the key to wrap in bytes

    _Example_
    \code
    WOLFSSL_AES_KEY kek;
    byte key[16] = { /* key-encryption key */ };
    byte keyToWrap[16] = { /* key to wrap */ };
    byte wrappedKey[24]; // wrapped key is 8 bytes longer
    
    wolfSSL_AES_set_encrypt_key(key, 128, &kek);
    int wrappedLen = wolfSSL_AES_wrap_key(&kek, NULL, wrappedKey,
                                          keyToWrap, 16);
    if (wrappedLen == 0) {
        // failed to wrap key
    }
...
/*!
    \ingroup AES
    \brief This function unwraps (decrypts) a key using AES Key Unwrap
    algorithm (RFC 3394) with the OpenSSL compatibility layer. This is
    used to securely receive cryptographic keys that were wrapped.

    \return Length of unwrapped key in bytes on success.
    \return 0 If key, iv, out, or in is NULL, or if unwrapping fails.

    \param key pointer to the WOLFSSL_AES_KEY structure containing the
    key-encryption key
    \param iv pointer to the initialization vector (typically the default
    IV 0xA6A6A6A6A6A6A6A6, or NULL to use default)
    \param out pointer to the output buffer to store the unwrapped key
    \param in pointer to the input buffer containing the wrapped key
    \param inlen length of the wrapped key in bytes
    ...
    wolfSSL_AES_set_decrypt_key(key, 128, &kek);
    int unwrappedLen = wolfSSL_AES_unwrap_key(&kek, NULL, unwrappedKey,
                                              wrappedKey, 24);
    if (unwrappedLen == 0) {
...
/*!
    \ingroup AES
    \brief This function encrypts a single AES block using the OpenSSL
    compatibility layer. It performs direct AES encryption on one block
    of data (16 bytes).

    \return none No return value.

    \param input pointer to the input buffer containing one AES block
    (16 bytes) to encrypt
    \param output pointer to the output buffer to store the encrypted block
    \param key pointer to the WOLFSSL_AES_KEY structure containing the key
    ...
    WOLFSSL_AES_KEY aesKey;
    byte key[16] = { /* 128-bit key */ };
    byte plaintext[16] = { /* one block */ };
    byte ciphertext[16];
    
    wolfSSL_AES_set_encrypt_key(key, 128, &aesKey);
    wolfSSL_AES_encrypt(plaintext, ciphertext, &aesKey);
    \endcode

    \sa wolfSSL_AES_decrypt
    \sa wolfSSL_AES_set_encrypt_key
*/
...
/*!
    \ingroup AES
    \brief This function decrypts a single AES block using the OpenSSL
    compatibility layer. It performs direct AES decryption on one block
    of data (16 bytes).
    ...
    \param input pointer to the input buffer containing one AES block
    (16 bytes) to decrypt
    \param output pointer to the output buffer to store the decrypted block
    \param key pointer to the WOLFSSL_AES_KEY structure containing the key
    ...
    WOLFSSL_AES_KEY aesKey;
    byte key[16] = { /* 128-bit key */ };
    byte ciphertext[16] = { /* one block */ };
    byte plaintext[16];
    
    wolfSSL_AES_set_decrypt_key(key, 128, &aesKey);
    wolfSSL_AES_decrypt(ciphertext, plaintext, &aesKey);
    \endcode

    \sa wolfSSL_AES_encrypt
    \sa wolfSSL_AES_set_decrypt_key
*/
...
/*!
    ...
    with wc_AesDelete when no longer needed.
