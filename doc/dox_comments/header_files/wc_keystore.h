...
/*!
    ...
    device would produce through keySz. keySz is cleared first in that form,
    so any value passed in is discarded rather than preserved.
...
/*!
    ...
    size the device would produce through blobSz. blobSz is cleared first in
    that form, so any value passed in is discarded rather than preserved.
...
/*!
    ...
    \return 0 on success
    \return CRYPTOCB_UNAVAILABLE if no registered device handles the operation
    \return BAD_FUNC_ARG if keyRef or srcKeyRef is NULL, if keyRefSz or
    srcKeyRefSz is zero, or if deriv is NULL with a non-zero derivSz. A zero
    derivSz is legal: it is the form used when the KDF takes no extra input
