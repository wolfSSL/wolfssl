/*!
    ...
    \brief この関数は、与えられた乱数生成器rngを使用して、与えられたサイズ（keysize）のCurve448鍵を生成し、与えられたcurve448_key構造体に格納します。wc_curve448_init()を通じて鍵構造体が初期化された後に呼び出す必要があります。

    \return 0 鍵の生成に成功し、与えられたcurve448_key構造体に格納された場合に返されます。
    \return ECC_BAD_ARG_E 入力keysizeがcurve448鍵のkeysizeに対応していない場合（56バイト）に返されます。
...
/*!
    ...
    \brief この関数は、公開鍵-秘密鍵ペアをcurve448_key構造体にインポートします。ビッグエンディアンのみ。
...
/*!
    ...
    \brief この関数は、公開鍵-秘密鍵ペアをcurve448_key構造体にインポートします。ビッグエンディアンとリトルエンディアンの両方をサポートします。
...
/*!
    ...
    \brief この関数は、与えられた鍵構造体から鍵ペアをエクスポートし、結果をoutバッファに格納します。ビッグエンディアンのみ。
