...
/* That compile failure only covers the definitions written in this file.  The
 * platforms below either supply current_time() from another translation unit
 * or define the name away before the prototype is even seen, so nothing
 * diagnoses the mismatch: the caller reads an integer return register the
 * callee never wrote, and every timing is garbage.  Refuse the combination
 * instead.  Converting these clocks is the future work noted above. */
#if defined(WC_BENCH_INT_ONLY) && !defined(WOLFSSL_LINUXKM) &&                 \
    (defined(WOLFSSL_CURRTIME_REMAP) || defined(WOLFSSL_USER_CURRTIME) ||      \
     defined(WOLFSSL_IAR_ARM_TIME) || defined(WOLFSSL_MDK_ARM) ||              \
     defined(WOLFSSL_TIRTOS))
    #error "benchmark integer time path needs a current_time returning \
bench_real_t in WC_BENCH_FP_SCALE units; this platform supplies double seconds \
from another file"
#endif
