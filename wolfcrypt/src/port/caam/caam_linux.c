...
/* mmap() takes an off_t, and on a 32-bit build that is 32 bits wide unless
 * large file support is asked for - too narrow for the 36-bit physical
 * addresses these parts use, and the truncation would quietly map the wrong
 * range. Ask for a 64-bit off_t before any system header is pulled in.
 * Nothing crosses this translation unit's boundary as an off_t, so the wider
 * type stays local to the mmap() call. */
...
/* JRCFGR_JR and JRCFGR_IMSK come from caam_driver.h; bit 0 masks the ring
 * interrupt. caamGetJobStatus() is careful to preserve it. */
...
static void* caamMapPhys(unsigned long long phys, unsigned int len)
{
    ...
    /* belt and braces for a platform where the request above did not take */
    if ((unsigned long long)off != phys) {
        WOLFSSL_MSG("caam: physical address does not fit an mmap offset");
        return NULL;
    }

    if (caamMemFd < 0) {
        caamMemFd = open(CAAM_LINUX_MEM_DEV, O_RDWR | O_SYNC | O_CLOEXEC);
        if (caamMemFd < 0) {
    ...
    map = mmap(NULL, len, PROT_READ | PROT_WRITE, MAP_SHARED, caamMemFd, off);
    if (map == MAP_FAILED) {
...
/* The job ring memory, taken from the pool on the first init and then kept.
 * The pool has no free, so allocating a fresh ring on every init would let
 * repeated init/teardown cycles walk through the reserved range. */
#define CAAM_LINUX_RING_SZ 1024
static unsigned char* caamRingVirt = NULL;
static CAAM_ADDRESS   caamRingPhys = 0;
    ...
    void** ringInVir)
{
    ...
    *base = (CAAM_ADDRESS)caamSecMap + CAAM_LINUX_JR_OFFSET;
    ...
    CAAM_WRITE(*base + JRCFGR_JR,
        CAAM_READ(*base + JRCFGR_JR) | JRCFGR_IMSK);

    if (caamRingVirt == NULL) {
        caamRingVirt = caamPoolAlloc(CAAM_LINUX_RING_SZ, &caamRingPhys);
        if (caamRingVirt == NULL) {
            return -1;
        }
    }
    else {
        /* stale entries from the previous owner are not ours to complete */
        memset(caamRingVirt, 0, CAAM_LINUX_RING_SZ);
    }

    *ringInPhy = caamRingPhys;
    *ringInVir = caamRingVirt;
 ...
 * that rings the doorbell. --enable-caam=linux is a configure choice rather
 * than an architecture one, so use the compiler's full barrier, which is
 * "sync" on the PowerPC parts this was developed against. */
int CAAM_ADR_SYNC(void* vaddr, int sz)
{
    ...
    __sync_synchronize();
#else
    #error "CAAM Linux port needs a full memory barrier for this compiler"
#endif
