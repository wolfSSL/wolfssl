...
runs in software, and is not advertised to the crypto callback layer at all,
so an unsupported operation is a performance question, not a correctness one.
A request longer than the staging buffer is split into successive jobs rather
than handed back; the engine stores the updated IV after each one, so CBC and
CTR chain across the split. Hashing has no
...
The in-tree `caam` driver claims all four job rings at boot and configures the
block globally through `MCFGR`, which this driver also needs: it writes 32-bit
pointer words, so `MCFGR[PS]` must be clear. Sharing is not an option, so take
the whole engine, rings and parent:
...
On a T1040 `MCFGR[PS]` reads back clear after this, which is what the port
needs; a part where the kernel has set it for 64-bit descriptor pointers would
have to have it cleared before the engine is usable from here.
...
claims the ring and `caamGetJobStatus()` preserves the mask; the driver polls
anyway.
...
| `CAAM_LINUX_AES_MAX` | 16 KB | AES staging buffer; longer requests are split into this many bytes per job |

### Exercising the job split

`CAAM_LINUX_AES_MAX` is the staging buffer size, and a request longer than it
is split into successive jobs that chain the IV. No wolfCrypt test vector is
anywhere near 16 KB, so at the default the split never runs; only the
benchmark crosses it, and the benchmark does not check its output. Build the
on target test with a small staging buffer instead and every existing AES
vector becomes a multi job chaining test:

    ./configure --host=powerpc-linux-gnu CC=powerpc-linux-gnu-gcc \
                --enable-caam=linux CFLAGS="-DCAAM_LINUX_AES_MAX=64"
    make
    ./wolfcrypt/test/testwolfcrypt

A run at 64 bytes and a run at the default should both pass. The first covers
the chaining and the per job error paths, the second covers the sizes real
traffic uses.
