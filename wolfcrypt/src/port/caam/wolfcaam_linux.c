...
#ifndef WOLFSSL_MUTEX_INITIALIZER
    /* No static initializer on this build, so the mutex has to be created at
     * run time. wolfCrypt_Init() is the documented single threaded entry
     * point that reaches here first, so the flag needs no lock of its own. */
    if (!caamLockInit) {
