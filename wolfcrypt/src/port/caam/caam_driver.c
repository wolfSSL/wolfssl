...
static int caamTRNG(unsigned char *out, int outSz)
{
    ...
    if (reg > caam.baseAddr + CAAM_RTENT_MAX && sz > 0) {
        return -1;
    }
    ...
    if (reg != caam.baseAddr + CAAM_RTENT_MAX) {
...
static int SetupJobRing(struct JobRing* r)
{
    ...
    /* get environment specific addresses to use for job rings. A failure
     * here leaves the ring pointers untouched, so the arithmetic and the
     * memset() below would run against whatever they held before. Report it
     * instead and let InitCAAM() unwind. */
    if (CAAM_SET_JOBRING_ADDR(&r->BaseAddr, &r->JobIn, &r->VirtualIn) != 0) {
        return -1;
    }
...
int InitCAAM(void)
{
    ...
    if (ret != 0) {
        ...
        /* SetupJobRing() only fails before it claims the ring or creates the
         * ring mutex, so the base address mapping is all there is to undo. */
    ...
    if (caam.vrs < 9) {
        ...
        if (caamInitRng(&caam) != 0) {
            ...
            /* Leave nothing behind. CleanupCAAM() is not reached on this
             * path because the caller never records the driver as
             * initialized, and a later retry must start from a clean
             * state rather than remap the window and re-create a live
             * mutex. */
