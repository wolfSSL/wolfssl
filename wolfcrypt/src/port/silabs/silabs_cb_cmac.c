...
/* One-shot: key, message and output in a single call.
 *
 * wc_AesCmacGenerate_ex() dispatches here BEFORE wc_InitCmac_ex() runs, so
 * cmac->devCtx and cmac->aes.heap are still uninitialised caller storage. Keep
 * all state on the stack and never touch the Cmac. Declining part-way is safe:
 * software redoes the operation from the key and message. */
...
int wc_SilabsCmac(wc_CryptoInfo* info)
{
    ...
    /* Dispatch happens before wc_AesCmacGenerate_ex() validates its
     * pointer/length pairs, so check them now - a NULL buffer with a positive
     * length would otherwise pass as an empty message. Checking before any SE
     * state exists also avoids leaving a device context behind. */
    ...
    /* From here the SE holds the key and message so far, while the software
     * AES/CMAC state was never set up, so a later failure must be terminal
     * rather than a decline the core would try to resume. */
    ...
    if (info->cmac.out != NULL) {
        /* The SE writes a full block, but wolfCrypt allows a truncated tag,
         * so finalize into a local block and copy out what was asked for. */
        byte tag[WC_AES_BLOCK_SIZE];
