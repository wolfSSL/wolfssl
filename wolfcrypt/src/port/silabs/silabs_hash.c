...
/* The _status variants hand back the raw SE Manager status so an unimplemented
 * command (which the callback port turns into a software fallback) can be told
 * from a hardware failure. The plain wrappers still return 0 or WC_HW_E. */
