...
/* Raw-status ECB: negative for an argument error, otherwise the SE status
 * unchanged so an unsupported command can be told from a hardware failure.
 * The callback port needs that to fall back; the wrapper below does not. */
