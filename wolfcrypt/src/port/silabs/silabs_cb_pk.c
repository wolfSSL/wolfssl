...
/* Raw r||s pair at the curve size, and the verify scratch that adds the two
 * decode halves alongside it. Both exceed the stack budget for a small target,
 * so WOLFSSL_SMALL_STACK moves them to the heap. */
#define SILABS_ECC_RAW_SZ             (ECC_MAX_CRYPTO_HW_SIZE * 2)
#define SILABS_ECC_VERIFY_SCRATCH_SZ  (ECC_MAX_CRYPTO_HW_SIZE * 4)
...
static int silabs_pk_curve_ok(const ecc_key* key)
{
    ...
    /* ECC_MAX_CRYPTO_HW_SIZE sizes key_raw and the r||s scratch below, but an
     * #elif chain can pick a smaller value for another backend built alongside
     * this one (ATECC, TA100). A wider curve would overrun that buffer, so
     * decline and let software, which sizes from the curve, handle it. */
...
/* The SE takes the digest as-is, assuming it is exactly the curve size, and
 * does not apply the ECDSA truncation rules. Hand those to software. */
...
/* ecc.c rejects an all-zero digest unless WC_ALLOW_ECC_ZERO_HASH is set, but
 * the callback runs first, so apply the same policy here. */
...
static int silabs_pk_load(ecc_key* key, int pub, int priv)
{
    ...
    /* silabs_ecc_import() rewrites key->type from the pub/priv flags. Here it
     * only refreshes the SE buffer, so preserve the type: downgrading a key
     * that holds a private scalar would break a later private-key export. */
    savedType = key->type;
...
static int silabs_pk_dispatch(wc_CryptoInfo* info)
{
    ...
    case WC_PK_TYPE_EC_KEYGEN:
        ...
        /* Use the curve's field size, not the requested one:
         * wc_ecc_make_key_ex() rounds up to the next curve (40 selects P-384),
         * so eckg.size would read the SE's key back at the wrong stride. */
    ...
    case WC_PK_TYPE_ECDH:
        ...
        /* wc_ecc_shared_secret() dispatches here before its own checks, so
         * repeat them to return the documented ECC_BAD_ARG_E. */
    ...
    case WC_PK_TYPE_ECDSA_SIGN:
    ...
    #endif
        /* A key bound by wc_SilabsSe_EccUse*Key() holds no software scalar by
         * design - the SE signs with the bound descriptor - so it is a valid
         * source. Any other key must carry a real scalar, or the direct port's
         * silabs_ecc_sign_hash() would silently sign with the SE attestation
         * key instead of the one the caller supplied. */
        ...
        if (ret == 0) {
            ...
            byte*  raw = NULL;
#else
            byte   raw[SILABS_ECC_RAW_SZ];
#endif
            word32 rawLen = SILABS_ECC_RAW_SZ;
            word32 keySz  = (word32)info->pk.eccsign.key->dp->size;

#ifdef WOLFSSL_SMALL_STACK
            raw = (byte*)XMALLOC(SILABS_ECC_RAW_SZ,
                info->pk.eccsign.key->heap, DYNAMIC_TYPE_TMP_BUFFER);
            if (raw == NULL) {
                ret = MEMORY_E;
            }
            if (ret == 0)
#endif
            {
                ...
                ForceZero(raw, SILABS_ECC_RAW_SZ);
            }
#ifdef WOLFSSL_SMALL_STACK
            XFREE(raw, info->pk.eccsign.key->heap, DYNAMIC_TYPE_TMP_BUFFER);
#endif
        }
        break;
#endif /* HAVE_ECC_SIGN */
    ...
    case WC_PK_TYPE_ECDSA_VERIFY:
        ...
        if (ret == 0) {
            ...
            byte*  raw = NULL;
#else
            byte   raw[SILABS_ECC_VERIFY_SCRATCH_SZ];
#endif
            byte*  rBuf;
            byte*  sBuf;
            word32 keySz = (word32)info->pk.eccverify.key->dp->size;
            word32 rLen  = ECC_MAX_CRYPTO_HW_SIZE;
            word32 sLen  = ECC_MAX_CRYPTO_HW_SIZE;

#ifdef WOLFSSL_SMALL_STACK
            raw = (byte*)XMALLOC(SILABS_ECC_VERIFY_SCRATCH_SZ,
                info->pk.eccverify.key->heap, DYNAMIC_TYPE_TMP_BUFFER);
            if (raw == NULL) {
                ret = MEMORY_E;
            }
            if (ret == 0)
#endif
            {
                /* One allocation, carved into r||s and the two decode
                 * halves, so there is a single free on the way out. */
                rBuf = raw + SILABS_ECC_RAW_SZ;
                sBuf = rBuf + ECC_MAX_CRYPTO_HW_SIZE;

                /* The decoder strips DER's leading zeros, but the SE wants
                 * each value at exactly the curve size, so left pad both. */
                ...
                if (ret == 0) {
                    XMEMSET(raw, 0, SILABS_ECC_RAW_SZ);
                ...
                ForceZero(raw, SILABS_ECC_VERIFY_SCRATCH_SZ);
            }
#ifdef WOLFSSL_SMALL_STACK
            XFREE(raw, info->pk.eccverify.key->heap, DYNAMIC_TYPE_TMP_BUFFER);
#endif
        }
        break;
...
/* WC_ALGO_TYPE_PK. ECC only - Series 2 has no RSA hardware, so RSA always
 * declines to software. With a wrapped or built-in key the ecc_key holds no
 * private scalar, so declining would run software ECC with no key behind it.
