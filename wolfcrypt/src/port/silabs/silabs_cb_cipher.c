...
/* Point the SE key descriptor at aes->devKey, the raw key wc_AesSetKey keeps
 * for offload alongside the expanded schedule. It lives as long as the Aes. */
...
/* AES-CTR. The SE tracks the keystream with an mbedTLS-style nc_off, wolfCrypt
 * with aes->left over aes->tmp. Rather than translate mid-keystream, offload
 * only whole blocks on a block boundary; the rest falls back to software, which
 * stays consistent because the SE updates aes->reg the same way. */
static int silabs_cipher_ctr(wc_CryptoInfo* info)
{
#ifdef WOLFSSL_ARMASM
    /* The Thumb2 assembly keeps the keystream remainder on a different
     * contract than the C code, so a stream split between it and the SE does
     * not line up (confirmed on an EFR32FG25). Leave CTR to software. */
    (void)info;
    return WC_NO_ERR_TRACE(CRYPTOCB_UNAVAILABLE);
#else
...
static int silabs_cipher_chachapoly(wc_CryptoInfo* info)
{
    ...
    if (info->cipher.enc) {
        ...
        /* On a real failure clear the output, as wc_ChaCha20Poly1305_Decrypt
         * does - its own ForceZero never runs for this path. A decline is
         * excluded: the SE did not touch the buffer and AEAD may be in place,
         * so clearing would destroy the ciphertext software still needs. */
...
/* Map a raw SE status for an AEAD. An authentication failure keeps its own
 * error; an unsupported command still declines so software can run. */
...
/* Decrypt counterpart: on a real failure clear the plaintext, as the software
 * AEADs do. A decline is excluded - the SE did not touch the buffer and AEAD
 * may be in place, so clearing would destroy the ciphertext. */
...
/* WC_ALGO_TYPE_CIPHER. With a wrapped or built-in key the Aes holds no key
 * material, so declining would run software AES on an unset schedule and
 * silently produce wrong ciphertext. Fail outright instead. */
