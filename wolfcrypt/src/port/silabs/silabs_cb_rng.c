 ...
 * every status to WC_HW_E, this maps through silabs_cb_status so an
 * unsupported command declines to software while a real TRNG failure still
 * returns WC_HW_E instead of silently downgrading the entropy source. */
