...
int wc_NuvotonCb_Free(wc_CryptoInfo* info)
{
    ...
    /* A Key Store handle in devCtx belongs to whoever wrote it, so there is
     * nothing here to release. Decline: wc_AesFree() then clears devCtx and
     * wipes the rest of the object, which is what we want. */
