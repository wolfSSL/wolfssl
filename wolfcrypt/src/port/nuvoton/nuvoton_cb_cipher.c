...
/* A decline sends the operation to software. A Key Store key has no software
 * counterpart, so report a hardware error rather than a decline that surfaces
 * later as an unrelated MISSING_KEY. */
static int wc_NuvotonAesDecline(const Aes* aes)
{
#ifdef WOLFSSL_NUVOTON_KS
    ...
    const byte* in, word32 sz, int mode, int enc)
{
    ...
    if (mode != WC_NUVOTON_AES_GCM) {
        if (sz == 0 || (sz % WC_AES_BLOCK_SIZE) != 0) {
            return wc_NuvotonAesDecline(aes);
        }
    }
...
#endif
    {
        /* wc_AesSetKey() fills devKey only for an Aes bound to a device id.
         * Under WOLF_CRYPTO_CB_FIND a software-keyed context reaches this
         * callback too, and its devKey is all zeros. */
    ...
    word32 tagSz, byte* out, word32 sz, int authErr)
{
    if (ret == 0) {
        if (ConstantCompare(tag, expected, (int)tagSz) != 0) {
            ...
            /* Caller supplies the matching auth-failure code. */
            ret = authErr;
        }
    }
    ...
    switch (info->cipher.type) {
        ...
        case WC_CIPHER_AES_CTR:
            ...
            if (info->cipher.aesctr.aes != NULL &&
                ...
                return wc_NuvotonAesDecline(info->cipher.aesctr.aes);
            }
...
#endif
#if defined(WOLFSSL_NUVOTON_KS) && defined(WOLF_CRYPTO_CB_AES_SETKEY)
        case WC_CIPHER_AES:
            /* A raw key is being installed on this Aes. Anything the Key
             * Store bound to it earlier has to go first: the dispatch above
             * prefers devCtx over devKey, so leaving the handle in place
             * would have the engine keep using the stored key and quietly
             * ignore the key just supplied. Declining afterwards lets the
             * normal software setup take the raw key. */
            if (info->cipher.aessetkey.aes != NULL) {
                info->cipher.aessetkey.aes->devCtx = NULL;
            }
            break;
#endif
        default:
