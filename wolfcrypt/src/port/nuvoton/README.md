...
With the default set on, an engine can still be dropped for code size with its
opt-out: `WOLFSSL_NUVOTON_NO_AESGCM` leaves AES-GCM in software, and
`WOLFSSL_NUVOTON_NO_KS` drops the Key Store.
...
| Deterministic (RFC 6979) or caller-set ECDSA k | The engine draws its own random k; software honors `wc_ecc_set_deterministic`/`wc_ecc_sign_set_k` |
...
AES-GCM against software across payload and tag lengths, since the suite's GCM
vectors use a single payload length and so miss the boundary between the
one-shot and cascade forms.
...
A run on the non-secure path that stops printing after the RSA test is a
`WOLFSSL_KEY_GEN` effect, not a hang or a TrustZone problem: the next thing the
suite does is a software 2048-bit DH parameter search, which takes minutes
because key generation is not offloaded. Each of those tests passes when called
directly across the boundary (`rsa 0`, `dh 0`, `pwdbased 0`, `ecc 0`).
