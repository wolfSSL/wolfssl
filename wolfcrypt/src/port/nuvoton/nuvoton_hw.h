 ...
 * image next to the veneers in embedded/nuvoton_m2354/secure/ in
 * wolfssl-examples, which export these
...
typedef struct wc_NuvotonAesReq {
    ...
    const byte* aad;      /* GCM only */
    byte*       tag;      /* GCM only, out on encrypt, in on decrypt */
    word32      sz;
...
/* Bring the CRPT block up, and the Key Store and TRNG when those engines are
 * built: ungate the clocks, release from reset and open the drivers.
 * Idempotent, reference counted, and safe to call from more than one thread.
 * Returns 0 or a wolfCrypt error. */
 ...
 * blocks; GCM goes as one packed packet of IV, AAD and payload, and a GCM
 * payload too large for the staging buffers runs through the DMA cascade. GCM
 * is built when WOLFSSL_NUVOTON_AESGCM is set. */
