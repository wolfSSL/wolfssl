 ...
 * WOLFSSL_BASE16 turned on in every build. */
    ...
    (defined(WOLFSSL_NUVOTON_RSA) && !defined(NO_RSA))
static int nuvoton_hex_to_bin(const char* hex, byte* out, word32 outSz)
{
    mp_int a[1];
    int    ret;

    if (hex == NULL || out == NULL || outSz == 0 || hex[0] == '\0') {
        /* mp_read_radix() accepts an empty string as the value 0, which would
         * turn an untouched driver buffer into an all-zero result. */
        return BAD_FUNC_ARG;
    }
...
/* nuvoton_settings.h turns WOLFSSL_NUVOTON_ECC on before HAVE_ECC is known -
 * its comment says to check here - so this is where the two are reconciled.
 * Without it an RSA-only build compiles ecc_key and info->pk.eccsign, neither
 * of which exists. The dispatcher below already pairs the two. */
#if defined(WOLFSSL_NUVOTON_ECC) && defined(HAVE_ECC)
...
static int nuvoton_curve_id(const ecc_key* key)
{
    /* Never trust dp->id alone: a custom curve can name a standard id while
     * carrying different parameters, and the engine would compute on its own
     * built-in parameters instead. wolfCrypt excludes ECC_CUSTOM_IDX at every
     * hardware dispatch for the same reason. */
    if (key->idx == ECC_CUSTOM_IDX) {
        return WC_NUVOTON_CURVE_NONE;
    }
    /* No per-curve #ifdefs: the ecc_curve_ids values are unconditional, and a
     * curve absent from ecc_sets[] can never reach here through key->dp->id.
     * Guarding on HAVE_ECCxxx would wrongly collapse a default build (where
     * those are undefined) to P-256 only. */
    switch (key->dp->id) {
        case ECC_SECP192R1:
    ...
    const ecc_set_type* dp, char* out, word32 outSz)
{
    ...
    if (ret == MP_OKAY) {
        if (useSz * WOLFSSL_BIT_SIZE > (word32)orderBits) {
            /* mp_rshb() returns void under USE_FAST_MATH and the heap math;
             * ecc.c calls it as a statement for the same reason. */
            mp_rshb(e, (int)(WOLFSSL_BIT_SIZE -
...
static int nuvoton_mp_to_hex(mp_int* a, char* out, word32 outSz)
{
    ...
    need = (word32)mp_unsigned_bin_size(a) * 2 + 1;
    if (need < 3) {
        need = 3;   /* mp_tohex writes "00\0" for a zero value */
    }
    if (need > outSz) {
        return BUFFER_E;
    }
...
static int nuvoton_digest_ok(const byte* digest, word32 digestSz)
{
    ...
    /* wc_ecc_sign_hash/wc_ecc_verify_hash already enforce the per-operation
     * minimum before the callback runs, and WC_MIN_DIGEST_SIZE_FOR_VERIFY may
     * be shorter than WC_MIN_DIGEST_SIZE for legacy verify, so only the upper
     * bound is this helper's to check. */
...
static int nuvoton_ecc_sign(wc_CryptoInfo* info)
{
    ...
    /* The engine draws its own random k, so it cannot honor a caller-set k
     * (wc_ecc_sign_set_k) or RFC 6979 deterministic signing. Decline both so
     * software, which does honor them, handles the request. */
    if (
    ...
    if (ret == 0) {
        ret = mp_init_multi(r, s, order, NULL, NULL, NULL);
        if (ret == MP_OKAY) {
            ...
            if (ret == MP_OKAY) {
                /* The BSP's ECC_GenerateSignature() returns success even when
                 * the engine timed out, leaving r = s = the base point. Reject
                 * a zero, out-of-range or r == s pair as a hardware error
                 * rather than emitting an invalid signature as success. */
                if (mp_iszero(r) || mp_iszero(s) ||
                        mp_cmp(r, order) != MP_LT ||
                        mp_cmp(s, order) != MP_LT ||
                        mp_cmp(r, s) == MP_EQ) {
                    ret = WC_HW_E;
                }
            }
...
static int nuvoton_ecc_verify(wc_CryptoInfo* info)
{
    ...
    /* Default the result so a caller that reads res without first checking the
     * return value never sees an indeterminate value. */
    *info->pk.eccverify.res = 0;
    ...
    ret = nuvoton_digest_ok(info->pk.eccverify.hash,
    ...
    if (ret == MP_OKAY) {
        ...
        if (ret == 0) {
            /* r and s must be in [1, n-1]. The engine cannot tell an
             * out-of-range value from a genuinely bad signature, so decline
             * those to software, which returns the canonical error. */
            if (mp_iszero(r) || mp_iszero(s) ||
...
static int nuvoton_ecc_keygen(wc_CryptoInfo* info)
{
    ...
    if (ret == MP_OKAY) {
        ret = mp_read_radix(order, key->dp->order, MP_RADIX_HEX);
        if (ret == MP_OKAY) {
            /* ecc_get_k_raw(), not wc_ecc_key_get_priv(): with
             * WOLFSSL_ECC_BLIND_K the getter hands back a value regenerated
             * into scratch, so a scalar written through it is discarded and
             * the next call reads back k ^ kb over the top of it. */
            ret = wc_ecc_gen_k(info->pk.eckg.rng, key->dp->size,
                ecc_get_k_raw(key), order);
        }
        mp_free(order);
    }
    if (ret == 0) {
        /* Install a blind over the scalar just written, so the key leaves
         * here in the state the rest of wolfCrypt expects. */
        ret = ecc_blind_k_rng(key, info->pk.eckg.rng);
    }
    if (ret == 0) {
        ret = nuvoton_mp_to_hex(wc_ecc_key_get_priv(key), hex->d,
            (word32)sizeof(hex->d));
    ...
    if (ret == 0) {
        ...
        /* Zeroize the scalar on a late failure. ecc_blind_k_rng() ran above,
         * so use ecc_forcezero_k(), which clears k, the blind kb and the
         * unblinded ku; a plain mp_forcezero(key->k) would leave kb live and
         * wc_ecc_key_get_priv() would still return a bogus non-zero scalar.
         * Also clear the partially written public point. */
        ecc_forcezero_k(key);
        mp_forcezero(key->pubkey.x);
        mp_forcezero(key->pubkey.y);
        mp_forcezero(key->pubkey.z);
    }
...
#endif /* WOLFSSL_NUVOTON_ECC && HAVE_ECC */
...
static int nuvoton_rsa(wc_CryptoInfo* info)
{
    ...
    /* The accelerator is specified for a base in [2, n-1]; software rejects
     * anything else with RSA_OUT_OF_RANGE_E, which the callback layer skips.
     * Decline an out-of-range value so software returns the canonical error
     * rather than the engine producing an undefined result. */
    if (mp_init(c) != MP_OKAY) {
        return WC_NO_ERR_TRACE(CRYPTOCB_UNAVAILABLE);
    }
    if (mp_read_unsigned_bin(c, info->pk.rsa.in, info->pk.rsa.inLen) != MP_OKAY
            || mp_cmp_d(c, 1) != MP_GT || mp_cmp(c, &key->n) != MP_LT) {
        mp_free(c);
        return WC_NO_ERR_TRACE(CRYPTOCB_UNAVAILABLE);
    }
    mp_free(c);
    ...
    if (isPrivate) {
        ...
        /* No private key material in a public-only build. */
        return WC_NO_ERR_TRACE(CRYPTOCB_UNAVAILABLE);
#else
        exp = &key->d;
        if (mp_iszero(exp)) {
