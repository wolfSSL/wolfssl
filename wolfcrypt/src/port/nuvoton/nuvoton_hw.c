...
/* The DMA reachability check and address helpers are used only by the AES and
 * SHA paths; an ECC/RSA/TRNG-only build has no DMA caller. */
#if defined(WOLFSSL_NUVOTON_CIPHER) || defined(WOLFSSL_NUVOTON_HASH)
...
/* What goes into a DMA address register. The engine takes 32 bits, which is
 * the whole address space on this part. */
#define NUVOTON_DMA_ADDR(p)  ((uint32_t)(uintptr_t)(p))
...
static int nuvoton_dma_ok(const void* p, word32 sz)
{
    ...
    /* The engine reaches SRAM and nothing else. */
    region = (uint32_t)addr & NUVOTON_DMA_REGION_MASK;
    if (region != NUVOTON_DMA_REGION_S &&
            region != NUVOTON_DMA_REGION_NS) {
        return 0;
    }

    return 1;
}
#endif /* WOLFSSL_NUVOTON_CIPHER || WOLFSSL_NUVOTON_HASH */
...
int wc_nuvoton_hw_init(void)
{
    ...
    if (nuvotonRefCount == 0) {
        ...
        /* The CRPT clock is always needed. The Key Store shares the AHB
         * domain but has its own gate, and the TRNG has its own on APB1 that
         * RNG_Open() ungates. */
...
/* Digest length the engine produces for each mode. SHA_Read() writes only
 * this many bytes, so it is also the only safe bound on the copy out - the
 * size of the staging array is not, and under WOLFSSL_NUVOTON_NSC the caller
 * supplying digestSz is the non-secure world. */
static word32 nuvoton_sha_digest_len(int shaMode)
{
    switch (shaMode) {
...
int wc_nuvoton_hw_sha(wc_NuvotonShaReq* req)
{
    ...
    if (req->last && (req->digest == NULL || req->digestSz == 0 ||
                      req->digestSz > nuvoton_sha_digest_len(req->shaMode))) {
        return BAD_FUNC_ARG;
    }
    ...
    CRPT->HMAC_CTL     = ctl;
    CRPT->HMAC_FBADDR  = NUVOTON_DMA_ADDR(req->fdbck);
    CRPT->HMAC_SADDR   = NUVOTON_DMA_ADDR(req->in);
    ...
    while (((CRPT->INTSTS & CRPT_INTSTS_HMACIF_Msk) == 0) ||
        ...
        if (timeout-- == 0) {
            ...
            /* Stop the engine first: the caller frees req->in and req->fdbck
             * on error, and a still-live DMA would write into freed memory. */
    ...
    switch (mode) {
        ...
        case WC_NUVOTON_AES_CTR:
            *opMode = AES_MODE_CTR;
            break;
        default:
            /* GCM does not come through here: it needs the GCM packet
    ...
    word32 sz, const uint32_t iv[4])
{
    ...
    if (!nuvoton_dma_ok(in, sz)) {
        if (!nuvoton_dma_ok(nuvotonDmaIn, sizeof(nuvotonDmaIn))) {
            WOLFSSL_MSG("Nuvoton: DMA staging buffer not engine addressable");
            return WC_HW_E;
        }
    ...
    AES_SetDMATransfer(CRPT, 0, NUVOTON_DMA_ADDR(src),
        NUVOTON_DMA_ADDR(dst), sz);
...
/* Packed AEAD packet staging for GCM under the hardware mutex. Sets where GCM
 * switches from one round to the DMA cascade. */
 ...
 * that encrypts it. Only the GCM tag repair uses them. */
...
/* One AES round for GCM. Caller holds the mutex and has set the key
 * and the counts. */
...
/* One piece of the GHASH input. A NULL pointer is a run of padding zeros. */
typedef struct {
    const byte* p;
    word32      sz;
} wc_NuvotonGhashSeg;

/* Copy want bytes of the logical segment stream, starting at from, into buf.
 * Returns how many were available. */
static word32 nuvoton_ghash_fill(const wc_NuvotonGhashSeg* segs, int segCount,
    word32 from, word32 want, byte* buf)
{
    word32 pos = 0;
    word32 got = 0;
    int    i;

    for (i = 0; i < segCount && got < want; i++) {
        word32 segEnd = pos + segs[i].sz;

        if (from + got < segEnd) {
            word32 off  = (from + got) - pos;
            word32 take = segs[i].sz - off;

            if (take > want - got) {
                take = want - got;
            }
            if (segs[i].p != NULL) {
                XMEMCPY(buf + got, segs[i].p + off, take);
            }
            else {
                XMEMSET(buf + got, 0, take);
            }
            got += take;
        }
        pos = segEnd;
...
/* GHASH the segment stream. Every segment is a whole number of blocks once
 * its padding is counted, so a staging buffer boundary never falls inside a
 * block and the input can be handed over in pieces: one round while it fits,
 * otherwise a DMA cascade through AES_FBADDR, the same mechanism
 * nuvoton_gcm_cascade() uses. That is what keeps the tag repair below free of
 * a payload length limit. Caller holds the mutex and has set the key. */
static int nuvoton_ghash_segs(uint32_t ctl, const wc_NuvotonGhashSeg* segs,
    int segCount, word32 total, byte* out)
{
    word32 chunkMax;
    word32 done;
    int    ret;

    chunkMax = (word32)sizeof(nuvotonGcmIn) & ~(word32)(WC_AES_BLOCK_SIZE - 1);

    if (total <= chunkMax) {
        (void)nuvoton_ghash_fill(segs, segCount, 0, total, nuvotonGcmIn);
        return nuvoton_gcm_run(ctl, nuvotonGcmIn, out, total);
    }

    XMEMSET(nuvotonGcmFb, 0, sizeof(nuvotonGcmFb));
    CRPT->AES_FBADDR = NUVOTON_DMA_ADDR(nuvotonGcmFb);

    ret = 0;
    for (done = 0; ret == 0 && done < total; ) {
        word32   chunk = total - done;
        uint32_t mode;
        uint32_t flags;
        ...
        (void)nuvoton_ghash_fill(segs, segCount, done, chunk, nuvotonGcmIn);

        if (done == 0) {
            mode  = CRYPTO_DMA_FIRST;
            flags = CRPT_AES_CTL_FBOUT_Msk;
        }
        else if (done + chunk >= total) {
            mode  = CRYPTO_DMA_LAST;
            flags = CRPT_AES_CTL_FBIN_Msk;
        }
        else {
            mode  = CRYPTO_DMA_CONTINUE;
            flags = CRPT_AES_CTL_FBIN_Msk | CRPT_AES_CTL_FBOUT_Msk;
        }

        ret = nuvoton_gcm_run_dma(ctl | flags, nuvotonGcmIn, out, chunk, mode);
        done += chunk;
    }
    ...
    uint32_t keySize)
{
     ...
     * one block holding both bit counts. Described rather than assembled, so
     * a payload larger than the staging buffer streams through instead of
     * being rejected - the caller cannot fall back to software once the
     * engine has written its output, and in and out are allowed to alias. */
    nuvoton_gcm_bitlen(lenBlock, req->aadSz);
    nuvoton_gcm_bitlen(lenBlock + 8, req->sz);

    segs[0].p = req->aad;
    segs[0].sz = req->aadSz;
    segs[1].p = NULL;
    segs[1].sz = aadSection - req->aadSz;
    segs[2].p = cipher;
    segs[2].sz = req->sz;
    segs[3].p = NULL;
    segs[3].sz = cSection - req->sz;
    segs[4].p = lenBlock;
    segs[4].sz = WC_AES_BLOCK_SIZE;
    ...
    ret = nuvoton_ghash_segs(
        ctl | (AES_MODE_GHASH << CRPT_AES_CTL_OPMODE_Pos),
        segs, 5, len, nuvotonGcmGhash);
    if (ret != 0) {
        return ret;
    }
...
/* The two payload lengths whose tag the engine gets wrong, and which the
 * repair below has to cover. */
/* The engine returns the wrong GCM tag when the payload length is 1 or 15
 * modulo the AES block size, so the port recomputes it for those. The standard
 * test vectors never reach those lengths; caught on hardware. */
#define WC_NUVOTON_GCM_BAD_TAG_RESIDUE_LO 1
#define WC_NUVOTON_GCM_BAD_TAG_RESIDUE_HI 15
static int nuvoton_gcm_tag_needed(const wc_NuvotonAesReq* req)
{
    word32 residue = req->sz % WC_AES_BLOCK_SIZE;

    return (residue == WC_NUVOTON_GCM_BAD_TAG_RESIDUE_LO ||
            residue == WC_NUVOTON_GCM_BAD_TAG_RESIDUE_HI);
}


/* Whether that repair can run at all. The GHASH over the payload streams, so
 * only a non 96 bit IV, which is hashed in one round, still has to fit. Asked
 * before the engine writes anything: discovering it afterwards would leave
 * the caller retrying in software over a buffer this already transformed. */
static int nuvoton_gcm_tag_fits(const wc_NuvotonAesReq* req)
{
    if (req->ivSz != GCM_NONCE_MID_SZ) {
        if (nuvoton_gcm_align(req->ivSz) + WC_AES_BLOCK_SIZE >
                (word32)sizeof(nuvotonGcmIn)) {
            return 0;
        }
    }
...
/* Reset the engine and load the key. Runs once per engine pass, and a request
 * that repairs its tag on decrypt makes two. Caller holds the mutex. */
static int nuvoton_gcm_load_key(const wc_NuvotonAesReq* req,
    const uint32_t* keyWords, uint32_t keySize)
{
...
#endif
#ifdef WOLFSSL_NUVOTON_TRNG
    /* The reset also wipes the CRPT PRNG seed that RNG_Open() loaded from the
     * TRNG at init. RNG_Random() never sets SEEDRLD, so without re-seeding
     * here every later TRNG read would run the PRNG from its reset state and
     * hand back predictable bytes with no error. */
    if (RNG_Open() != 0) {
        SYS_LockReg();
        WOLFSSL_MSG("Nuvoton: RNG_Open failed after CRPT reset");
        return WC_HW_E;
    }
#endif
    SYS_LockReg();
    ...
    if (req->keySlot == WC_NUVOTON_NO_SLOT) {
        AES_SetKey(CRPT, 0, (uint32_t*)keyWords, keySize);
        return 0;
    }
    ...
    {
        KS_MEM_Type mem;
        int ret = nuvoton_ks_mem(req->keyMem, &mem);
...
/* Byte counts, not block counts, and the unpadded ones. Set for every pass so
 * the repair sees the same register state whichever side of the GCM run it
 * happens on. */
static void nuvoton_gcm_counts(const wc_NuvotonAesReq* req)
{
...
static int nuvoton_gcm_cascade(wc_NuvotonAesReq* req, uint32_t ctl)
{
    ...
    if (req->sz == 0) {
        /* GMAC always fits the one-round path; the cascade assumes a payload
         * and would otherwise copy a tag the engine never produced. */
        return BAD_LENGTH_E;
    }
...
static int nuvoton_aes_gcm(wc_NuvotonAesReq* req)
{
    ...
    int      repair;
    int      ret;
    byte     repairTag[WC_AES_BLOCK_SIZE];
    ...
    /* Cap each length so neither nuvoton_gcm_align() nor the packed-size sum
     * of the three block-aligned sections can wrap word32 (0x3FFFFFFF each
     * keeps the sum below 0xC0000010). A >4GB buffer cannot exist on this
     * part, but under the NSC veneers these lengths are attacker supplied. */
    if (req->sz    > 0x3FFFFFFFu ||
        req->aadSz > 0x3FFFFFFFu ||
        req->ivSz  > 0x3FFFFFFFu) {
        return BAD_LENGTH_E;
    }

    /* The GCM path programs these static buffers straight into the DMA address
     * registers; a linker script that places .bss where the engine cannot
     * reach would otherwise fail silently, as the AES round path notes. */
    if (!nuvoton_dma_ok(nuvotonGcmIn, sizeof(nuvotonGcmIn)) ||
        !nuvoton_dma_ok(nuvotonGcmOut, sizeof(nuvotonGcmOut)) ||
        !nuvoton_dma_ok(nuvotonGcmFb, sizeof(nuvotonGcmFb)) ||
        !nuvoton_dma_ok(nuvotonGcmGhash, sizeof(nuvotonGcmGhash)) ||
        !nuvoton_dma_ok(nuvotonGcmTag, sizeof(nuvotonGcmTag))) {
        return WC_HW_E;
    }
    ...
    /* Decline before anything is staged or written. Returning a length error
     * once the engine has produced output would have the core retry software
     * GCM over a buffer already transformed in place, which for CTR based GCM
     * undoes the encryption and puts plaintext on the wire under a tag that
     * verifies. */
    repair = nuvoton_gcm_tag_needed(req);
    if (repair && !nuvoton_gcm_tag_fits(req)) {
        return BAD_LENGTH_E;
    }
    ...
    XMEMSET(repairTag, 0, sizeof(repairTag));

    /* On decrypt the ciphertext the tag covers is req->in, and in and out are
     * allowed to alias, so a repair has to run before the engine writes the
     * plaintext back over it. That is true of the one round path and more so
     * of the cascade, which fills req->out chunk by chunk as it goes. */
    if (repair && !req->encrypt) {
        ret = nuvoton_gcm_load_key(req, keyWords, keySize);
        if (ret == 0) {
            nuvoton_gcm_counts(req);
            ret = nuvoton_gcm_tag(req, req->in, keySize);
        }
        if (ret == 0) {
            XMEMCPY(repairTag, req->tag, req->tagSz);
        }
    }
    ...
    if (ret == 0) {
        ...
        if (!repair) {
            /* The tag follows the padded payload; the cascade has already
             * taken its own. */
            if (oneShot) {
            ...
            /* The ciphertext the tag covers is what was just produced. */
            ret = nuvoton_gcm_tag(req, req->out, keySize);
        }
        else {
            /* Put back the tag computed before the plaintext landed, over
             * whatever the engine left in its place. */
            XMEMCPY(req->tag, repairTag, req->tagSz);
        }
    }
    ...
    ForceZero(keyWords, sizeof(keyWords));
    ForceZero(repairTag, sizeof(repairTag));
...
int wc_nuvoton_hw_aes(wc_NuvotonAesReq* req)
{
    ...
    /* Reject a request with neither a key nor a Key Store slot before the
     * GCM dispatch, so the check covers every AES mode. */
    ...
    if (req->keySlot == WC_NUVOTON_NO_SLOT) {
#ifdef WOLFSSL_NUVOTON_KS
        /* Clear any Key Store source a previous request left in AES_KSCTL:
         * AES_SetKey() writes only AES_KEY[], so a stale RSRC would make the
         * engine fetch from the store and encrypt under the wrong key. */
        CRPT->AES_KSCTL = 0;
#endif
    ...
    for (done = 0; ret == 0 && done < req->sz; done += chunk) {
         ...
         * The ciphertext and the counter are unambiguous and already in
         * hand. */
        switch (req->mode) {
            case WC_NUVOTON_AES_CBC:
                if (req->encrypt) {
 ...
 * the wolfssl-examples project installs the handler; the port README says
 * so too. */
...
#else
        /* Every other Key Store call site guards this the same way; without
         * it an ECC-only build has no nuvoton_ks_mem() to call. */
        wolfSSL_CryptHwMutexUnLock();
        return BAD_FUNC_ARG;
#endif
    }
...
int wc_nuvoton_hw_rsa(wc_NuvotonRsaReq* req)
{
    ...
    if (rc == 0) {
        ...
        if (rc != 0) {
            /* A timed-out or errored op may still be DMA-active into buf,
             * which is freed below; stop the engine before releasing it. */
            CRPT->RSA_CTL = CRPT_RSA_CTL_STOP_Msk;
        }
    }
    if (rc == 0) {
        /* RSA_Read() writes keyBits/4 hex characters plus the NUL with no bound
         * of its own, so refuse a buffer that cannot hold them. */
        if (req->outSz < (word32)(req->keyBits / 4) + 1) {
            rc = -1;
        }
        else {
