...
static  const byte bench_iv_buf[] =
{
...
/* additional authentication data for AES-GCM*/
static  const byte add_auth_data[] = 
{
                ...
                const char*     encLabel,   const char* decLabel)
{
    ...
    /* forward declaration (following functions are located in benchmark.c)*/
    void bench_stats_start(int* count, double* start);
    int  bench_stats_sym_check(double start);
    ...
    do {
        for (times = 0; times < numBlocks; times++ ) {
            ...
            if( ret != 0 ){
                WOLFSSL_MSG("wc_tsip_AesCbcEncrypt failed" );
                goto exit;
            }
        }
        count += times;
    ...
    do {
        for (times = 0; times < numBlocks ; times++) {
            ...
            ret = wc_tsip_AesCbcDecrypt(
                    ...
                    bench_size);
            if( ret != 0){
                WOLFSSL_MSG("wc_tsip_AesCbcDecrypt failed");
                goto exit;
            }
        }
        count += times;
...
*   returns  0  if this function processed
*            1  this function did nothing due to unacceptable key size 
-------------------------------------------------------------------*/
                ...
                const char*     encLabel,   const char* decLabel)
{
    ...
    XMEMCPY(bench_addtn,    add_auth_data,AES_AUTH_ADD_SZ );
    XMEMSET(bench_plain,    0x30, bench_buf_size +16);
    XMEMSET(bench_cipher,   0,    bench_buf_size +16);  
    ...
    do {
        for (times = 0; times < numBlocks; times++ ) {
            ...
            ret = wc_tsip_AesGcmEncrypt(
                    ...
                    bench_size,
                    (byte*)iv,      ivSz, 
                    bench_tag,      AES_AUTH_TAG_SZ,
                    bench_addtn,    AES_AUTH_ADD_SZ);
    
            if( ret != 0 ){
                WOLFSSL_MSG("wc_tsip_AesGcmEncrypt failed" );
                goto exit;
            }
        }
        count += times;
    ...
    do {
        for (times = 0; times < numBlocks ; times++) {
            ...
            ret = wc_tsip_AesGcmDecrypt(
                    ...
                    bench_addtn,    AES_AUTH_ADD_SZ);
            if( ret != 0){
                WOLFSSL_MSG("wc_tsip_AesGcmDecrypt failed");
                goto exit;
            }
        }
        count += times;
