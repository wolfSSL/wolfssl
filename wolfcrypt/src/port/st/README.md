...
| `WOLFSSL_STM32G0`     | G0xx (no HW crypto; SW only)                                        |
...
| `WOLFSSL_STM32C0`     | C0xx (no HW crypto; SW only)                                        |
...
| `WOLFSSL_STM32MP25`   | MP25 (HASH / RNG / SAES / PKA; Cortex-A35 + M33)                    |
...
If your chip simply does not have an IP block (e.g. H7Ax has no CRYP/HASH; F207 has no CRYP/HASH; C0 has no HW crypto at all) the family arm sets the appropriate `NO_STM32_*` defines for you.
...
Newer STM32 silicon (U3/U5/WBA/H5/C5; the `WC_STM32_HAS_DHUK` family gate) carries a chip-unique 256-bit key (DHUK) burned into the SAES key derivation path. wolfSSL exposes this via three crypto-callback devIds:

| devId macro                       | value | Purpose                                          |
|-----------------------------------|-------|--------------------------------------------------|
| `WOLFSSL_SAES_DEVID`              | 807   | Drive a software key through the SAES IP block   |
| `WOLFSSL_DHUK_DEVID`              | 808   | Use DHUK to wrap a plaintext key                 |
| `WOLFSSL_DHUK_WRAPPED_DEVID`      | 809   | Use a previously-DHUK-wrapped key for AES        |

(Legacy aliases `WOLFSSL_STM32U5_DHUK_DEVID` / `WOLFSSL_STM32U5_SAES_DEVID` / `WOLFSSL_STM32U5_DHUK_WRAPPED_DEVID` continue to work.)

### Enabling

```
#define WOLFSSL_DHUK             /* legacy: WOLFSSL_STM32U5_DHUK */
#define WOLFSSL_STM32_USE_SAES   /* required -- DHUK is SAES-only */
```

`WC_STM32_HAS_DHUK` is auto-defined when the active family is one of U3/U5/H5/WBA/C5 and `WOLFSSL_DHUK` is set. Other families silently bypass DHUK code.

### Public API

```
int  wc_Stm32_Aes_Wrap(Aes* aes, byte* out, const byte* in, word32 sz);
int  wc_Stm32_Aes_UnWrap(Aes* aes, byte* out, const byte* in, word32 sz);
void wc_Stm32_Aes_SetDHUK_IV(Aes* aes, const byte* iv, word32 ivLen);
int  wc_Stm32_Aes_DhukOp(Aes* aes, byte* out, const byte* in, word32 sz,
                         int isEncrypt);
```

`wc_AesEncrypt` / `wc_AesDecrypt` automatically route to `wc_Stm32_Aes_DhukOp` when `aes->devId == WOLFSSL_DHUK_WRAPPED_DEVID`. ECB is fully wired today; CBC integration (multi-block IV chaining) is in progress.

### Current state

- **Wrap (KEYSEL=HW, MODE=ENCRYPT)** -- silicon-validated on STM32U385 and STM32WBA52. Output is deterministic and chip-bound (same wrapped bytes across resets), confirming DHUK is the actual encryption key.
- **UnWrap-decrypt from non-secure state** -- hangs on both STM32U3 and STM32WBA52 with identical signature: `SAES_SR.KEYVALID=1`, `CR` retains configured bits, `CCF` never asserts, `EN` auto-clears with no error flags. Two chips, two families, both TZEN=0. Strongly suggests the wrapped-key unwrap path requires TrustZone secure-state context. Validation via an S-state harness is open work.
