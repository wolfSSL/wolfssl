 ...
 *
 * Defined here, ahead of every PKA user, because the DHUK crypto-callback
 * ECDSA sign (Stm32Dhuk_Sign) drives SAES immediately before the PKA and so
 * needs the same re-arm. */
#if defined(WOLFSSL_STM32_CUBEMX) && defined(HAVE_ECC)
static void Stm32Pka_EnsureInit(void)
{
    ...
    needInit = ((hpka.Instance->CR & PKA_CR_EN) == 0U);
#ifdef PKA_SR_INITOK
    /* INITOK is a V2-PKA status flag; V1 PKA (WB/WL/G4/L5) does not have it
     * and the bit is undefined there (same guard as the bare HAL_PKA_Init
     * shim in this file). */
    if ((hpka.Instance->SR & PKA_SR_INITOK) == 0U) {
        needInit = 1;
    }
#endif
    if (needInit) {
...
/* The ECC scalar-mul shims serve wc_ecc_mulmod_ex2 only, which is compiled
 * out under WC_STM32_PKA_VERIFY_ONLY (STM32H563's "light" PKA has no
 * generic scalar-mul mode -- only the integrated ECDSA verify). Gate them
 * the same way so they are not unused static functions on that part. */
...
/* The next two helpers are DHUK-only. WOLFSSL_STM32_USE_SAES on its own just
 * routes the TinyAES driver at the SAES instance and never reaches them --
 * it needs only the BUSY/init/RNG helpers below -- so gate them, or they are
 * unused static functions on a USE_SAES-without-DHUK build (N6, H7S). */
#ifdef WOLFSSL_DHUK
 ...
 * passes DO raise CCF, provided SR.BUSY was waited out after KEYSEL latched
 * (see Stm32SaesDeriveKeyFromSeed). The __DMB()
...
/* Spin until SR.BUSY clears. Used to wait out the SAES self-init and the DHUK
 * load that latching KEYSEL=HW starts -- CR writes made while BUSY is high are
 * dropped. Compute passes signal completion via CCF, not this. */
...
#ifdef WOLFSSL_DHUK   /* DHUK-only; see the note above */
...
/* WC_STM32_HAS_DHUK as well as WOLFSSL_DHUK: every caller of
 * Stm32SaesDeriveKeyFromSeed lives in the DHUK crypto-callback backend, which
 * needs both, so gating on WOLFSSL_DHUK alone leaves this an unused static on a
 * SAES family without silicon DHUK. */
 ...
 * CCF, with SR.KEYVALID confirming the loaded key. CR writes made while
 * SR.BUSY is high are dropped, so BUSY is waited out before CR.EN is set. */
static int Stm32SaesDeriveKeyFromSeed(const byte* seed, word32 seedSz)
{
    ...
    /* SAES consumes the blob as 32-bit words in the order ST's
     * HAL_CRYPEx_UnwrapKey pushes them -- the byte-reverse of how it sits in a
    ...
    /* Program the whole key path in one write, MODE included. Latching
     * KEYSEL = HW starts the DHUK load and raises SR.BUSY, and CR writes made
     * while BUSY are dropped -- a MODE written separately after this point
     * reads back as 0 and the derivation pass then never runs. */
    cr = AES_CR_KEYSIZE | AES_CR_KMOD_0 | AES_CR_KEYSEL_0 | AES_CR_MODE_0;
    SAES->CR = cr;
    __DSB();
    /* Wait out that DHUK load before enabling, or the derivation pass runs
     * against an incomplete key and quietly yields the wrong result. */
    ...
    /* Decryption pass -- blob in via DINR, key lands in KEYR. */
    SAES->CR = (SAES->CR & ~(word32)AES_CR_MODE) | AES_CR_MODE_1;
    SAES->CR |= AES_CR_EN;
    for (i = 0; i < 8u; i += 4u) {
...
#endif /* WOLFSSL_DHUK && WC_STM32_HAS_DHUK */
 ...
 *                         (loaded into KEYR).
 *
 * rawOrder selects the blob word order:
 *   1 -- raw: words go to DINR exactly as they sit in memory. Matches
 *        wc_Stm32_Aes_DhukOp_ex and the DHUK crypto-callback derive path, so
 *        blobs are interchangeable across those APIs and across build paths.
 *        This is what the bare-metal build has always produced.
 *   0 -- legacy: byte-reverse the input before wrapping and the output after,
 *        reproducing what the CubeMX/HAL implementation (built on
    ...
    byte* out, word32* outSz, const byte* iv, int ivSz, int rawOrder)
{
    ...
    /* Stage input in the caller's chosen word order (see rawOrder above). */
    XMEMCPY(buf, in, inSz);
    if (!rawOrder) {
...
{
    return wc_Stm32_Aes_Wrap_ex(aes, in, inSz, out, outSz, iv, ivSz,
        WC_STM32_WRAP_DEFAULT_RAW_ORDER);
}
 ...
 * completion via CCF plus SR.KEYVALID, and
 ...
 * It IS the inverse of wc_Stm32_Aes_Wrap: unwrapping a blob that
 * wc_Stm32_Aes_Wrap produced from K in WC_STM32_WRAP_ORDER_RAW puts K back
 * in KEYR (verified on U385 and U545 against ST's HAL_CRYPEx_WrapKey).
 *
 ...
 * to opt in. */
/* The real implementation shares Stm32SaesDeriveKeyFromSeed(), which is gated
 * on WC_STM32_HAS_DHUK, so require that here too rather than break the build
 * on a WOLFSSL_DHUK config for a family without a DHUK. */
#if !defined(WOLFSSL_STM32_DHUK_UNWRAP) || !defined(WC_STM32_HAS_DHUK)
    ...
    word32 sz, int isEnc, int isCbc)
{
    ...
    /* Step 1: unwrap the 256-bit blob under the silicon DHUK, straight into
     * KEYR. Shared with the crypto-callback device so the two APIs cannot
     * drift apart again. */
    ret = Stm32SaesDeriveKeyFromSeed((const byte*)aes->key, 32);
    if (ret != 0) {
        goto exit;
    }
                     ...
                     const byte* aad, word32 aadSz, int isEnc)
{
    ...
    if (isEnc) {
        ...
        /* Constant-time tag verify; scrub the recovered plaintext on mismatch. */
        if (ConstantCompare((const byte*)hwTag, tag,
                            (int)(tagSz < 16 ? tagSz : 16)) == 0) {
            ret = 0;
        }
        else {
            if (sz > 0 && out != NULL) {
...
/* (The TinyAES arm has no Stm32AesPrepareKey helper: the decrypt
 * key-derivation pass is run inline by Stm32AesSetupCR below, which already
 * holds the assembled CR value. An unused duplicate of that helper lived here
 * and was removed -- it had no callers on any build.) */
...
#if defined(AES_CR_GCMPH_Msk)
/* ----- TinyAES HW GCM (AES peripheral GCM mode, CHMOD=GCM) -----
 * Runs the four GCM phases -- init, header (AAD), payload, final (tag) --
 * selected by AES_CR.GCMPH, mirroring ST's HAL_CRYP GCM sequence
 * (stm32XXxx_hal_cryp.c): EN stays set across phase transitions, only GCMPH
 * changes; wait CCF after each block. A partial trailing payload block is
 * handled in HW via NPBLB (number of padding bytes) so the tag is correct.
 *
 * Standard 12-byte IV only (J0 = IV || 0x00000001); other IV lengths need a
 * software GHASH of the IV, so they return CRYPTOCB_UNAVAILABLE and fall back to
 * SW. Both encrypt and decrypt run on the HW GCM engine; decrypt verifies the
 * received tag in constant time and scrubs the output on mismatch
 * (AES_GCM_AUTH_E). */

#define STM32_AES_GCMPH_INIT     0u
#define STM32_AES_GCMPH_HEADER   AES_CR_GCMPH_0
#define STM32_AES_GCMPH_PAYLOAD  AES_CR_GCMPH_1
#define STM32_AES_GCMPH_FINAL    (AES_CR_GCMPH_0 | AES_CR_GCMPH_1)

/* Select a GCM phase (preserving MODE / CHMOD / KEYSIZE / DATATYPE) and enable
 * the peripheral. EN is cleared by the HW when a phase's computation completes,
 * so each new phase must re-assert it (matches ST's HAL, which re-enables per
 * phase). */
static void Stm32AesGcmPhase(word32 phase)
{
    WC_STM32_AES_INST->CR =
        (WC_STM32_AES_INST->CR & ~(word32)AES_CR_GCMPH_Msk) | phase;
    WC_STM32_AES_INST->CR |= AES_CR_EN;
}

/* Feed one 16-byte AAD/header block (no output read); wait + clear CCF. */
static int Stm32AesGcmAadBlock(const byte* block)
{
    int ret;
    word32 i;
    word32 buf[WC_AES_BLOCK_SIZE/sizeof(word32)];

    XMEMCPY(buf, block, WC_AES_BLOCK_SIZE);
    for (i = 0; i < 4; i++) {
        WC_STM32_AES_INST->DINR = buf[i];
    }
    ret = Stm32AesWaitCCF();
                     ...
                     const byte* aad, word32 aadSz, int isEnc)
{
    ...
    word32 hwTag[WC_AES_BLOCK_SIZE/sizeof(word32)];

    if (aes == NULL || iv == NULL || tag == NULL) return BAD_FUNC_ARG;
    if (sz > 0 && (in == NULL || out == NULL)) return BAD_FUNC_ARG;
    if (aadSz > 0 && aad == NULL) return BAD_FUNC_ARG;
    if (tagSz < 4u || tagSz > WC_AES_BLOCK_SIZE) return BAD_FUNC_ARG;

    /* HW path is the standard 12-byte IV. Return CRYPTOCB_UNAVAILABLE (NOT
     * NOT_COMPILED_IN) so aes.c falls back to SW GCM for a non-12-byte IV. */
    if (ivSz != GCM_NONCE_MID_SZ) {
        return CRYPTOCB_UNAVAILABLE;
    }
    ...
    ret = wc_AesGetKeySize(aes, &keyLen);
    if (ret != 0) return ret;
    if (keyLen != 16 && keyLen != 32) return CRYPTOCB_UNAVAILABLE; /* 192 unsup */
    ...
    printf("[STM32 BARE TinyAES GCM] -> HW %s (sz=%u aadSz=%u)\n",
           isEnc ? "enc" : "dec", (unsigned)sz, (unsigned)aadSz);
#endif
    ...
    WC_STM32_AES_CLK_ENABLE_INST();
#ifdef WOLFSSL_STM32_USE_SAES
    ret = Stm32SaesEnsureRng(); if (ret != 0) goto exit;
    ret = Stm32SaesWaitInit();  if (ret != 0) goto exit;
#endif

    /* ---- Init phase (GCMPH=00): load key + ICB, compute H and E(J0) ---- */
    cr = STM32_AES_DATATYPE_BYTE | Stm32AesKeySizeBits(keyLen) |
         STM32_AES_CHMOD_GCM | STM32_AES_GCMPH_INIT |
         (isEnc ? STM32_AES_MODE_ENC : STM32_AES_MODE_DEC);
    WC_STM32_AES_INST->CR = 0;
#ifdef WOLFSSL_STM32_USE_SAES
    ret = Stm32SaesWaitInit(); if (ret != 0) goto exit;
    ...
    XMEMSET(icb, 0, sizeof(icb));
    XMEMCPY(icb, iv, GCM_NONCE_MID_SZ);
    icb[WC_AES_BLOCK_SIZE - 1] = 0x02;   /* payload counter starts at J0+1 = 2 */
    Stm32AesLoadIV(icb, WC_AES_BLOCK_SIZE);
    ...
    /* ---- Header phase (GCMPH=01): AAD, partial padded with zeros ---- */
    if (aadSz > 0) {
        Stm32AesGcmPhase(STM32_AES_GCMPH_HEADER);
        for (b = 0; b < aadBlocks; b++) {
            ret = Stm32AesGcmAadBlock(aad + b * WC_AES_BLOCK_SIZE);
            if (ret != 0) goto exit;
        }
        if (aadPartial > 0) {
            XMEMSET(pad, 0, sizeof(pad));
            XMEMCPY(pad, aad + aadBlocks * WC_AES_BLOCK_SIZE, aadPartial);
            ret = Stm32AesGcmAadBlock(pad);
            if (ret != 0) goto exit;
        }
    }
    ...
    if (sz > 0) {
        ...
        if (partial > 0) {
            ...
            STM32_AES_CLEAR_CCF();
            for (i = 0; i < 4u; i++) {
                last[i] = WC_STM32_AES_INST->DOUTR;
            }
            XMEMCPY(out + blocks * WC_AES_BLOCK_SIZE, last, partial);
        }
    }

    /* ---- Final phase (GCMPH=11): [len(AAD) || len(PT)] in bits -> tag ----
     *
     * The two AES-IP generations disagree on whether CR.DATATYPE's byte swap
     * applies to this length block, and ST's own HAL keys the difference on
     * the NPBLB feature: parts without NPBLB pre-swap each word with __REV,
     * NPBLB parts write plain uint32s. Match that split.
     *
     * The NPBLB arm is hardware-validated (NUCLEO-U385RG-Q: whole-block GCM
     * KAT plus a 30-case AAD sweep, tags matching the SP 800-38D vectors);
     * the non-NPBLB arm follows ST's reference driver. */
...
#else
    WC_STM32_AES_INST->DINR = __REV((word32)(aadBits >> 32));
    WC_STM32_AES_INST->DINR = __REV((word32)aadBits);
    WC_STM32_AES_INST->DINR = __REV((word32)(ptBits >> 32));
    WC_STM32_AES_INST->DINR = __REV((word32)ptBits);
#endif
    ret = Stm32AesWaitCCF();
    if (ret != 0) goto exit;
    for (b = 0; b < 4u; b++) {
        hwTag[b] = WC_STM32_AES_INST->DOUTR;
    }
    STM32_AES_CLEAR_CCF();
    if (isEnc) {
        ...
        /* Constant-time tag verify; scrub the recovered plaintext on mismatch. */
        if (ConstantCompare((const byte*)hwTag, tag, (int)tagSz) == 0) {
            ret = 0;
        }
        else {
            if (sz > 0 && out != NULL) {
...
#else /* AES peripheral without GCM mode (older TinyAES: WB/WL/G0/...) */
/* No HW GCM on this AES IP variant -- fall back to software GCM (its AES blocks
 * still run on HW via wc_AesEncrypt -> wc_Stm32_Aes_Ecb). */
int wc_Stm32_Aes_Gcm(struct Aes* aes, byte* out, const byte* in, word32 sz,
                     const byte* iv, word32 ivSz,
                     byte* tag, word32 tagSz,
                     const byte* aad, word32 aadSz, int isEnc)
...
/* Make sure the HW key source (aes->key) actually holds the plaintext key
 * before a plaintext-key device operation.
 *
 * Everything below -- wc_Stm32_Aes_Init's Init.pKey, Stm32AesLoadKey, and the
 * native wc_Stm32_Aes_Gcm -- reads aes->key, which wc_AesSetKey normally fills
 * with the byte-reversed (big-endian word) user key. Under
 * WOLF_CRYPTO_CB_ONLY_AES there is no software key schedule: AesSetKeyLocal()
 * records keylen/rounds/IV and returns, leaving aes->key zeroed, and the
 * plaintext key exists only in aes->devKey. Without this, the one build the
 * device exists for would encrypt everything under an all-zero key. Staging is
 * idempotent, so it is safe to redo on every request. */
static int Stm32Aes_StageDevKey(struct Aes* aes)
{
    ...
    XMEMCPY(aes->key, aes->devKey, keyLen);
    /* Must match the reversal wc_AesSetKey's STM32 arm applies (aes.c), since
     * everything downstream reads aes->key in that form. A CubeMX build
     * without STM32_HAL_V2 consumes aes->key un-reversed. */
#if !defined(WOLFSSL_STM32_CUBEMX) || defined(STM32_HAL_V2)
    ByteReverseWords(aes->key, aes->key, keyLen);
#endif
    return 0;
#else
    /* wc_AesSetKey already populated aes->key. */
    return (aes == NULL) ? BAD_FUNC_ARG : 0;
#endif
}
#endif /* WOLF_CRYPTO_CB && (CUBEMX || BARE) */
 ...
 * fall-through. Uses the plaintext key on the Aes and the plain CRYP
...
#if defined(HAVE_AESGCM) && defined(STM32_CRYPTO_AES_GCM)
/* AES-GCM in-callback on the HAL GCM engine (hardware), keyed from the plaintext
 * aes->key. Reuses the native HAL GCM implementation (wc_AesGcmEncrypt_STM32 /
 * wc_AesGcmDecrypt_STM32 in aes.c) so all AAD/partial-block/tag handling is the
 * tested one -- the bulk cipher and GHASH run on the AES peripheral. */
static int Stm32Cube_AesGcm(struct wc_CryptoInfo* info)
{
    if (info->cipher.enc) {
        return wc_AesGcmEncrypt_STM32(info->cipher.aesgcm_enc.aes,
            info->cipher.aesgcm_enc.out, info->cipher.aesgcm_enc.in,
            info->cipher.aesgcm_enc.sz, info->cipher.aesgcm_enc.iv,
            info->cipher.aesgcm_enc.ivSz, info->cipher.aesgcm_enc.authTag,
            info->cipher.aesgcm_enc.authTagSz, info->cipher.aesgcm_enc.authIn,
            info->cipher.aesgcm_enc.authInSz);
    }
    return wc_AesGcmDecrypt_STM32(info->cipher.aesgcm_dec.aes,
        info->cipher.aesgcm_dec.out, info->cipher.aesgcm_dec.in,
        info->cipher.aesgcm_dec.sz, info->cipher.aesgcm_dec.iv,
        info->cipher.aesgcm_dec.ivSz, info->cipher.aesgcm_dec.authTag,
        info->cipher.aesgcm_dec.authTagSz, info->cipher.aesgcm_dec.authIn,
        info->cipher.aesgcm_dec.authInSz);
}
#endif /* HAVE_AESGCM && STM32_CRYPTO_AES_GCM */

/* Cipher dispatch (shared with the CCB device below). Services AES-ECB and
 * AES-GCM on the HAL engine with the plaintext key; other modes return
 * CRYPTOCB_UNAVAILABLE so the caller falls through to the native HAL path or
 * another provider. */
static int Stm32Cube_Cipher(struct wc_CryptoInfo* info)
{
    int ret = 0;   /* both cases below can be compiled out */
    ...
    case WC_CIPHER_AES_ECB:
        ret = Stm32Aes_StageDevKey(info->cipher.aesecb.aes);
        if (ret != 0) {
            return ret;
        }
    ...
    case WC_CIPHER_AES_GCM:
        ret = Stm32Aes_StageDevKey(info->cipher.enc ?
            info->cipher.aesgcm_enc.aes : info->cipher.aesgcm_dec.aes);
        if (ret != 0) {
            return ret;
        }
        return Stm32Cube_AesGcm(info);
#endif
    default:
        (void)ret;
        return CRYPTOCB_UNAVAILABLE;
...
/* Bare-metal plaintext-key AES crypto-callback device -- the counterpart to the
 * bare DHUK device (Stm32_CryptoDevCb at WC_DHUK_DEVID), but keyed from the
 * Aes's own key on the plain CRYP engine, not SAES/DHUK. Register at
 * WOLFSSL_STM32_AES_DEVID; it can be registered alongside the DHUK device so an
 * application selects plaintext vs DHUK-seed per Aes by devId. Reuses the bare
 * HW AES primitives (wc_Stm32_Aes_Ecb / wc_Stm32_Aes_Gcm defined above).
 * Services AES-ECB in the callback -- needed so wc_AesGcmSetKey can derive the
 * GHASH subkey H through the ECB callback under WOLF_CRYPTO_CB_ONLY_AES -- and
 * AES-GCM, which must be serviced here rather than left to aes.c's direct
 * dispatch: under WOLF_CRYPTO_CB_ONLY_AES the key only reaches the hardware
 * once Stm32Aes_StageDevKey has run. wc_Stm32_Aes_Gcm still returns
 * CRYPTOCB_UNAVAILABLE for shapes the IP cannot do, so the software GCM
 * fall-through (same plaintext key) is preserved. */
#if defined(WOLFSSL_STM32_BARE) && defined(STM32_CRYPTO) && \
    defined(WOLF_CRYPTO_CB) && !defined(NO_AES)
static int Stm32Aes_Cipher(struct wc_CryptoInfo* info)
{
    int ret = 0;   /* both cases below can be compiled out */
    ...
    case WC_CIPHER_AES_ECB:
        ...
        return wc_Stm32_Aes_Ecb(info->cipher.aesecb.aes,
            info->cipher.aesecb.out, info->cipher.aesecb.in,
            info->cipher.aesecb.sz, info->cipher.enc);
#endif
#ifdef HAVE_AESGCM
    case WC_CIPHER_AES_GCM:
        if (info->cipher.enc) {
            ret = Stm32Aes_StageDevKey(info->cipher.aesgcm_enc.aes);
            if (ret != 0) {
                return ret;
            }
            return wc_Stm32_Aes_Gcm(info->cipher.aesgcm_enc.aes,
                info->cipher.aesgcm_enc.out, info->cipher.aesgcm_enc.in,
        ...
        ret = Stm32Aes_StageDevKey(info->cipher.aesgcm_dec.aes);
        if (ret != 0) {
            return ret;
        }
        /* const cast: the decrypt path compares the tag, never writes it. */
        return wc_Stm32_Aes_Gcm(info->cipher.aesgcm_dec.aes,
            info->cipher.aesgcm_dec.out, info->cipher.aesgcm_dec.in,
...
static int Stm32Aes_CryptoDevCb(int devId, struct wc_CryptoInfo* info,
    void* ctx)
{
    (void)devId;
    (void)ctx;
 ...
 * verifies. The key-derivation/decrypt passes signal completion via CCF, with
 * SR.KEYVALID confirming the loaded key; SR.BUSY must be waited out after the
 * KEYSEL=HW latch first, since CR writes made while BUSY are dropped. */

/* AES modes for Stm32Dhuk_Aes (was in the removed dhuk.h). */
#define WC_DHUK_MODE_ECB 0
#define WC_DHUK_MODE_CBC 1

/* The 256-bit derivation seed is never held in module state: every entry point
 * takes it by parameter from the caller's own object (Aes.devKey for the
 * cipher paths, ecc_key.dhuk_seed for the sign path) and it is consumed inside
 * a single crypto-HW-mutex hold. A file-static "staged seed" would race
 * between threads -- two Aes objects with different DHUK seeds could stage in
 * one order and derive in the other -- and would leave a 32-byte secret in
 * .bss between calls. */

/* GMAC tag using a key derived from the caller's seed via the silicon DHUK. */
static int Stm32Dhuk_Gmac(const byte* seed, word32 seedSz,
    const byte* iv, word32 ivSz,
    const byte* aad, word32 aadSz, byte* tag, word32 tagSz)
{
    ...
    /* Derive the DHUK-bound working key into SAES KEYR from the caller's
     * seed. */
    ret = Stm32SaesDeriveKeyFromSeed(seed, seedSz);
    if (ret != 0) {
        goto exit;
    }
...
/* Full-payload AES-GCM using a key derived from the caller's seed via the
...
static int Stm32Dhuk_Gcm(const byte* seed, word32 seedSz,
    int enc, const byte* in, word32 sz, byte* out,
...
/* AES ECB/CBC using a key derived from the caller's seed via the silicon DHUK.
...
static int Stm32Dhuk_Aes(const byte* seed, word32 seedSz,
    int mode, int enc, const byte* in,
    word32 sz, byte* out, const byte* iv, word32 ivSz)
{
     ...
     * schedule); that prep is a compute pass and signals via CCF. */
    cr = AES_CR_DATATYPE_1 | AES_CR_KEYSIZE | chmod;
    if (!enc) {
        /* Normal-mode (KMOD=NORMAL) decrypt key-schedule prep: a data/compute
         * pass, so it raises CCF. Waiting on BUSY here clears too early and
         * yields an incomplete inverse schedule. */
        SAES->CR = cr | AES_CR_MODE_0;     /* MODE = KEYDERIVATION */
        SAES->CR |= AES_CR_EN;
        ret = Stm32SaesWaitCCF();
        if (ret != 0) {
...
/* ECDSA sign with a DHUK-protected private key. The caller's 256-bit seed
 * derives an intermediate AES key inside SAES (key never in SW); that key
 * AES-ECB-decrypts the wrapped private scalar (key->dhuk_wrapped_priv) into a
 * short-lived stack buffer; HAL_PKA_ECDSASign runs; the scalar is
 * ForceZero-scrubbed. Output is a DER-encoded signature (cryptocb EccSign
 * contract). The crypto HW mutex is held across both the SAES unwrap and the
 * PKA sign -- they are one operation, and the hold keeps another thread from
 * taking the PKA through a reset in between (see Stm32Pka_EnsureInit). */
static int Stm32Dhuk_Sign(const byte* seed, word32 seedSz,
    const struct ecc_key* keyIn,
    ...
    struct WC_RNG* rng)
{
    ...
    status = Stm32SaesDeriveKeyFromSeed(seed, seedSz);
    if (status != 0) {
        goto saes_exit;
    }
...
saes_exit:
    ...
    /* ---- PKA ECDSA sign with the recovered scalar, still under the crypto HW
     * mutex taken above. ---- */
    ...
    /* The SAES work above (and HAL_CCB_Init/DeInit on a CCB build) can leave
     * the PKA disabled; re-arm it or the HAL_MAX_DELAY wait never returns. */
    Stm32Pka_EnsureInit();
...
/* Hand back the 256-bit seed an Aes carries in devKey (set via the normal key
 * API). devKey, not key: under WOLF_CRYPTO_CB_ONLY_AES aes->key is never
 * populated. Returns CRYPTOCB_UNAVAILABLE if not a 256-bit seed key. */
static int Stm32Dhuk_AesSeed(Aes* aes, const byte** seed)
{
...
/* The Aes a cipher request carries, for the modes whose crypto-callback
 * payload exposes one. Returns NULL when this cipher type has no Aes we can
 * reach, so callers can fail closed rather than guess at the key. */
static Aes* Stm32Dhuk_CipherAes(struct wc_CryptoInfo* info)
{
    switch (info->cipher.type) {
#if defined(HAVE_AES_ECB) || defined(WOLFSSL_AES_DIRECT) || \
    defined(WOLF_CRYPTO_CB_ONLY_AES)
...
static int Stm32Dhuk_Cipher(struct wc_CryptoInfo* info)
{
    ...
    defined(WOLF_CRYPTO_CB_ONLY_AES)
    case WC_CIPHER_AES_ECB:
        ret = Stm32Dhuk_AesSeed(info->cipher.aesecb.aes, &seed);
        if (ret != 0) {
            return ret;
        }
        return Stm32Dhuk_Aes(seed, 32u, WC_DHUK_MODE_ECB, info->cipher.enc,
                             info->cipher.aesecb.in, info->cipher.aesecb.sz,
                             info->cipher.aesecb.out, NULL, 0);
#endif
#if defined(HAVE_AES_CBC)
    case WC_CIPHER_AES_CBC:
        ret = Stm32Dhuk_AesSeed(info->cipher.aescbc.aes, &seed);
        if (ret != 0) {
            return ret;
        }
        ...
        ret = Stm32Dhuk_Aes(seed, 32u, WC_DHUK_MODE_CBC, info->cipher.enc,
                            info->cipher.aescbc.in, info->cipher.aescbc.sz,
    ...
    case WC_CIPHER_AES_GCM:
         ...
         * the derivation seed itself (not the SAES-derived device key),
         * producing a non-device-bound result -- so the request is serviced
         * here rather than returning CRYPTOCB_UNAVAILABLE. */
        if (info->cipher.enc) {
            ret = Stm32Dhuk_AesSeed(info->cipher.aesgcm_enc.aes, &seed);
            if (ret != 0) {
                return ret;
            }
            if (info->cipher.aesgcm_enc.sz == 0) {
                return Stm32Dhuk_Gmac(seed, 32u,
            ...
            return Stm32Dhuk_Gcm(seed, 32u, 1,
            ...
            ret = Stm32Dhuk_AesSeed(info->cipher.aesgcm_dec.aes, &seed);
            if (ret != 0) {
                return ret;
            }
            ...
            return Stm32Dhuk_Gcm(seed, 32u, 0,
...
#endif
    default:
        /* A cipher mode the SAES backend cannot service (AES-CTR, AES-CCM,
         * AES-XTS, ...). What to return depends on whether the request
         * actually carries a DHUK seed:
         *
         *   256-bit key (a seed) -- fail loudly with ALGO_ID_E. Declining with
         *     CRYPTOCB_UNAVAILABLE would let wolfCrypt fall back to a
         *     software/HW path keyed on the raw seed instead of the
         *     HW-derived device key. NOT_COMPILED_IN is unsuitable too --
         *     wc_CryptoCb_TranslateErrorCode() remaps it back to
         *     CRYPTOCB_UNAVAILABLE. ALGO_ID_E propagates unchanged.
         *
         *   anything else -- not a seed, so there is no seed to leak. Decline
         *     so the request falls through to the plaintext-key device (or
         *     software) with the caller's literal key, which is what a
         *     plaintext AES-CTR request on this devId expects.
         *
         * Fail closed when the Aes cannot be identified for this cipher type:
         * an unrecognised request is treated as if it carried a seed. */
        aes = Stm32Dhuk_CipherAes(info);
        if (aes == NULL || aes->keylen == 32) {
            return ALGO_ID_E;
        }
        return CRYPTOCB_UNAVAILABLE;
...
static int Stm32Dhuk_PkSign(struct wc_CryptoInfo* info)
{
    ...
    return Stm32Dhuk_Sign(key->dhuk_seed, key->dhuk_seed_sz, key,
                          info->pk.eccsign.in, info->pk.eccsign.inlen,
...
{
    /* No device state to initialise: each request carries its own seed. */
...
/* Only wc_ecc_mulmod_ex2 below uses this, and that is compiled out under
 * WC_STM32_PKA_VERIFY_ONLY (the STM32H563 "light" PKA has no generic
 * scalar-mul mode), so gate it the same way or it is an unused static. */
#if !defined(WC_STM32_PKA_VERIFY_ONLY)
...
#ifdef WOLFSSL_STM32_PKA_V2
#if !defined(WC_STM32_PKA_VERIFY_ONLY)   /* see the note above */
