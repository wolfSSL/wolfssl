 ...
 * job ring state machine, so descriptor assembly, submission, completion,
 * timeout, reset escalation and status decoding run under make check on any
 * machine, with fault injection for paths real hardware cannot be asked to
 * take on demand.
 *
 * The model is CPU-native end to end (it implements wc_SecQoriqRead/Write
 * itself and interprets the descriptor words the driver wrote), so it runs
 * the same on little and big endian hosts. Data buffers keep the port's big
 * endian fixed-width conventions, which mp_read/to_unsigned_bin share.
 *
 * Descriptors execute against wolfCrypt's own software crypto: AES CBC/CTR/
 ...
 * and the RSA/modexp protocols. ECDSA/ECDH jobs report a DECO error unless
 * a status is injected; modelling the PKHA's curve arithmetic is out of
 * scope.
 */
...
void wc_SecQoriqSimHighPhysAlloc(int on)
{
    sim.highPhysAlloc = on;
}

void wc_SecQoriqSimLongPtr(int on)
{
    sim.longPtr = on;
}
    ...
    switch (aai) {
        ...
        case 0x10: /* CBC */
            if (job->haveCtx && job->ctxSz >= WC_AES_BLOCK_SIZE) {
                /* The updated context is the last ciphertext block, which on
                 * decrypt is the input. An in-place request maps in and
                 * outMsg to the same buffer, so take the copy before the
                 * operation overwrites it with plaintext. */
                XMEMSET(lastIn, 0, sizeof(lastIn));
                if (inSz >= WC_AES_BLOCK_SIZE) {
                    XMEMCPY(lastIn, in + inSz - WC_AES_BLOCK_SIZE,
                        WC_AES_BLOCK_SIZE);
                }
                ...
                if (ret == 0 && job->outCtx != NULL &&
                    ...
                    XMEMCPY(job->outCtx,
                        enc ? job->outMsg + inSz - WC_AES_BLOCK_SIZE : lastIn,
                        WC_AES_BLOCK_SIZE);
                }
            }
            break;
    ...
    switch (eff) {
        case SEC_QORIQ_MCFGR:
            return (sim.dmaResetStuck ? SEC_QORIQ_MCFGR_DMA_RESET : 0) |
                   (sim.longPtr ? SEC_QORIQ_MCFGR_LONG_PTR : 0);
...
void* wc_SecQoriqDmaAlloc(word32 sz, word64* physOut)
{
    ...
    if (sim.highPhysAlloc) {
        /* model a 36-bit system whose pages sit above 4 GB */
        simAllocs[i].phys += 0x100000000ULL;
    }
    simAllocs[i].sz   = sz;
