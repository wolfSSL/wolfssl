 ...
 * (PDB) of parameters and pointers, and the HEADER's start index points at
 * the OPERATION word after it. Every operation has the same shape, a size
 * word then buffers then an OPERATION, which secPkhaRun() implements once
 * for all of them. Domain parameters are always explicit; this silicon
 * refuses the built-in curve shortcut (see SEC_QORIQ_PKHA_PDB_L_SHIFT in
 * sec_qoriq.h).
 *
    ...
    int count, word32* statusOut)
{
    ...
    for (i = 0; (ret == 0) && (i < count); i++) {
        ...
        /* No length accompanies a PDB pointer, so the recorded size only
         * confirms the platform can make the whole buffer DMA safe; the
         * direction has to be stated because the bare word carries none. */
        ret = wc_SecQoriqDescAddPdbPtr(&desc, items[i].buf, items[i].sz,
            items[i].out);
    }
...
int wc_SecQoriqEccSupported(int curveId, word32 keySz)
{
    ...
    /* The simulated backend does not model PKHA curve arithmetic, so it
     * declares no ECC capability and curve work falls back to software. */
    ...
    word32 hashSz, byte* r, byte* s, word32 keySz)
{
     ...
     * returns a CCB "RNG not instantiated" error. Instantiation happened
     * once inside wc_SecQoriqInit(); here only re-read RDSTA (side-effect
     * free, and rngReady only ever goes 0 -> 1) in case another agent
     * instantiated the handle since. NOT_COMPILED_IN so the callback
     * router sends the signature to software. */
    if (dev->rngReady == 0) {
