...
static int secRunLocked(SecQoriqDev* dev, SecQoriqDesc* desc)
{
    ...
    while (secRead(dev->jr, SEC_QORIQ_ORSF) == 0) {
        if (++waited > SEC_QORIQ_POLL_MAX) {
             ...
             * descriptor stage, the rings, and on a flat-mapped backend the
             * caller's buffers), and the index mirrors no longer track it.
             * Escalate until DMA provably cannot reach reusable memory:
...
static int secRingsAlloc(SecQoriqDev* dev)
{
    ...
    /* Valid memory the engine cannot address in 32-bit pointer mode is a
     * platform limitation, not an allocation failure: NOT_COMPILED_IN so
     * wolfCrypt_Init() treats the whole backend as unavailable and stays on
     * software, exactly like a non-E part. A 36-bit Linux system whose
     * kernel hands out high pages lands here. */
    if ((dev->inRingPhys >> 32) != 0 || (dev->outRingPhys >> 32) != 0) {
        WOLFSSL_MSG("sec_qoriq: rings above 4GB, engine cannot reach them");
...
/* Unified failure path for wc_SecQoriqInit() once the registers are mapped:
 * release whatever was built and clear the whole device so nothing dangling
 * (dev->regs especially) is left for wc_SecQoriqFree() to trip over. */
static int secInitFail(SecQoriqDev* dev, int ret)
{
    secRingsFree(dev);
...
int wc_SecQoriqInit(void)
{
    ...
    /* Every failure from here on funnels through secInitFail() so no
     * partially built device survives: wc_SecQoriqFree() keys on dev->regs,
     * and a nonfatal decline (NOT_COMPILED_IN, which wolfCrypt_Init()
     * tolerates) must not leave it pointing at an unmapped window for a
     * later wolfCrypt_Cleanup() to dereference. */
    ...
    if (((chanumMs >> SEC_QORIQ_CHANUM_MS_JRNUM_SHIFT) & 0xF) <=
        ...
        return secInitFail(dev, BAD_FUNC_ARG);
    }
    ...
    if (mcfgr & SEC_QORIQ_MCFGR_LONG_PTR) {
        ...
        return secInitFail(dev, NOT_COMPILED_IN);
    }
    dev->longPtr = 0;

    ret = secRingsAlloc(dev);
    if (ret != 0) {
        return secInitFail(dev, ret);
    }
