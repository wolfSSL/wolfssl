...
/* The fallback promise covers run time declines too, not just what
 * secCbCanDo() sees in advance - but only declines proven to happen before
 * a descriptor was handed to the engine: argument and shape validation, an
 * unmappable buffer, a full descriptor, a failed staging allocation, a
 * missing capability. Those are exactly BAD_FUNC_ARG, BUFFER_E, MEMORY_E
 * and NOT_COMPILED_IN under the wc_SecQoriqRun() error contract, and
 * software can safely redo the work over the same buffers.
 *
 * WC_HW_E is deliberately NOT translated. It can be raised after
 * submission (a timeout, a job the engine ran and faulted), when the
 * engine may already have modified the caller's output, an in-place RSA
 * block or a CBC/CTR chaining IV; a software retry would then compute over
 * corrupted state. A hard error for the one call in flight is the safe
 * answer, and later calls still fall back cleanly because the router's
 * device checks decline before dispatch once the device is dead.
 *
