...
/* The fallback promise covers run time declines too, but only those proven
 * to happen before a descriptor reached the engine: argument and shape
 * validation, an unmappable buffer, a full descriptor, a failed staging
 * allocation, a missing capability. Under the wc_SecQoriqRun() error
 * contract those are exactly BAD_FUNC_ARG, BUFFER_E, MEMORY_E and
 * NOT_COMPILED_IN, and software can safely redo the work over the same
 * buffers.
 *
 * WC_HW_E is deliberately not translated: it can be raised after submission
 * (a timeout, or a job the engine ran and faulted), by which point the
 * engine may already have modified the caller's output, an in-place RSA
 * block or a CBC/CTR chaining IV, and a software retry would compute over
 * corrupted state. Failing the one call in flight is the safe answer; later
 * calls still fall back cleanly because the router's device checks decline
 * before dispatch once the device is dead.
 *
 * AES_GCM_AUTH_E is not translated either: that is the engine's answer
 * about the data, not about itself, and the caller must see it. */
...
#if defined(HAVE_AES_CBC) || defined(WOLFSSL_AES_COUNTER) || defined(HAVE_AES_ECB)
...
#ifdef HAVE_AESGCM
/* The one GCM shape the engine takes, shared by the encrypt and decrypt
 * arms so their preconditions cannot drift apart: SP 800-38D 12 byte IV,
 * full 16 byte tag, and message/AAD each within a single command. */
static int secCbGcmCanDo(word32 ivSz, word32 tagSz, word32 sz, word32 aadSz)
{
    if (wc_SecQoriqGetDev() == NULL) {
        return 0;
    }
    if (ivSz != SEC_QORIQ_GCM_IV_SZ || tagSz != SEC_QORIQ_GCM_TAG_SZ) {
        return 0;
    }
    if (sz > SEC_QORIQ_MAX_XFER_SZ || aadSz > SEC_QORIQ_MAX_XFER_SZ) {
        return 0;
    }
    return 1;
}
    ...
    switch (info->cipher.type) {
        ...
        case WC_CIPHER_AES_GCM:
            if (info->cipher.enc) {
                ...
                if (aes == NULL ||
                        !secCbGcmCanDo(info->cipher.aesgcm_enc.ivSz,
                ...
                if (aes == NULL ||
                        !secCbGcmCanDo(info->cipher.aesgcm_dec.ivSz,
...
#if defined(HAVE_ECC_SIGN) || defined(HAVE_ECC_DHE)
/* wolfCrypt's own private key check runs only after the crypto callback
 * has been dispatched, in both wc_ecc_sign_hash() and
 * wc_ecc_shared_secret(), so a key carrying no scalar would reach the
 * engine as an all zero d. Declining lets software run and answer with the
 * ECC_BAD_ARG_E the caller is owed. */
static int secCbHasPriv(ecc_key* key)
{
...
#endif /* HAVE_ECC_SIGN || HAVE_ECC_DHE */
...
static int secCbEccSign(wc_CryptoInfo* info)
{
    ...
    defined(WOLFSSL_ECDSA_DETERMINISTIC_K_VARIANT)
    /* Same reason for a nonce installed with wc_ecc_sign_set_k(): the
     * engine would sign with its own RNG4 nonce, silently ignoring the one
     * the caller configured and leaving it armed for a later signature. */
    #ifndef WOLFSSL_NO_MALLOC
    if (key != NULL && key->sign_k != NULL) {
        return CRYPTOCB_UNAVAILABLE;
    }
    #else
    if (key != NULL && key->sign_k_set) {
        return CRYPTOCB_UNAVAILABLE;
    }
    #endif
#endif
 ...
 * mod p, so it rides on wc_SecQoriqModExp() and is only compiled when the
 * RSA half of the port is (WOLFSSL_SEC_QORIQ_RSA). A build that disables
 * RSA offload with WOLFSSL_SEC_QORIQ_NO_RSA keeps DH in software too; see
 * the README's "RSA and Diffie-Hellman" section. */
static int secCbDh(wc_CryptoInfo* info)
{
     ...
     * usually but not always the full width: a value whose top byte is zero
     * arrives one byte short, about one time in 256. The engine wants a whole
     * modulus-width block, so left pad rather than decline. */
    if (info->pk.dh.pubSz == 0 || info->pk.dh.pubSz > pSz ||
            info->pk.dh.privSz == 0 || info->pk.dh.privSz > pSz) {
        return CRYPTOCB_UNAVAILABLE;
    }

    /* wc_SecQoriqModExp() writes a whole modulus width, where the software
     * path writes only the significant bytes, so an output buffer shorter
     * than the modulus has to go to software. secCbRsa() and secCbEcdh()
     * guard their output capacity the same way. */
    if (*info->pk.dh.agreeSz < pSz) {
        return CRYPTOCB_UNAVAILABLE;
    }

    /* Two blocks: the group prime, then the padded peer public value. */
    SEC_CB_ALLOC(buf, pSz * 2);
    pbuf = buf;
    ubuf = buf + pSz;

    XMEMCPY(ubuf + (pSz - info->pk.dh.pubSz), info->pk.dh.otherPub,
        info->pk.dh.pubSz);
    ...
    if (mp_to_unsigned_bin_len(&key->p, pbuf, (int)pSz) != MP_OKAY) {
        ...
        ret = wc_SecQoriqModExp(ubuf, info->pk.dh.priv,
    ...
    if (ret == 0) {
        ...
        if (lead > 0) {
            XMEMMOVE(info->pk.dh.agree, info->pk.dh.agree + lead,
                pSz - lead);
            /* The move leaves the last 'lead' bytes of the secret sitting
             * beyond the length reported below, where a caller that clears
             * only agreeSz bytes would miss them. */
            ForceZero(info->pk.dh.agree + (pSz - lead), lead);
        }
        *info->pk.dh.agreeSz = pSz - lead;
    }

    /* The prime and the peer public value are both public, but free the
     * whole block uniformly. */
    SEC_CB_FREE(buf, pSz * 2);
