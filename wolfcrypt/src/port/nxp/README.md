...
- `linux` maps the SEC through `/dev/mem`. The engine never DMAs through ordinary process pages (they can be migrated or copied-on-write behind any `/proc/self/pagemap` snapshot): every job's descriptor and data buffers are staged through one pinned, pre-faulted, contiguity-verified bounce arena (`SEC_QORIQ_ARENA_SZ`, default 128 KB), and pagemap is consulted only for that driver-owned memory, re-verified before each job. It initialises on a stock kernel but **does not offload anything on a typical 36-bit part**; see "The Linux backend on a 36-bit part" below before using it.
- `sim` is a host-runnable model of the job ring used by the in-tree tests: it runs on any architecture, executes descriptors with wolfCrypt's own software crypto, and adds fault injection for the timeout, reset and status-decode paths. It exists for `make check` and development, never for production.

The same port builds through CMake with `-DWOLFSSL_SEC_QORIQ=baremetal|linux|sim` (or `yes`, which selects `baremetal`).
...
- Errors that can only occur before a descriptor is handed to the engine (argument and shape validation, an unmappable buffer, a missing capability) fall back to software through the crypto callback. `WC_HW_E` is a hard error by design: it can be raised after submission, when the engine may already have modified an in-place buffer or a chaining IV, and a silent software retry would compute over corrupted state. A dead engine still falls back cleanly for subsequent operations because the router's device checks decline before dispatch.
- Single-shot hashing is bounded by the 64-word descriptor at 30 FIFO loads, about 1.9 MB of message; larger inputs are refused with `BAD_FUNC_ARG` (`wc_SecQoriqHash` is only reachable through the direct driver API, the callback router never routes hashing).
- RNG4 instantiation runs exactly once, inside `wc_SecQoriqInit()` before the crypto callback is registered, because the RDSTA check and TRNG programming are not atomic. If every entropy delay up to the maximum is rejected, hardware seeding is recorded unavailable and seeding stays with the platform source; later RNG users only re-read RDSTA (side-effect free) in case another agent instantiated the handle.
- In-tree tests run against the simulated backend: `./configure --enable-sec-qoriq=sim && make check` (any host) exercises the job ring state machine with fault injection - submission, completion, timeout, the ring-reset / DMA-reset / poisoned-device escalation, status decoding, `jobCount` accounting - plus known answers for AES CBC (out-of-place and in-place), GCM including corrupted-tag rejection and plaintext scrubbing, multi-chunk hashing, RNG4 instantiation retry and exhaustion, and FFDHE-2048 agreement through the modexp protocol descriptor, each cross-checked against the software implementation. The `sec-qoriq.yml` workflow runs this plus PowerPC cross-compiles of both real backends on every PR.
