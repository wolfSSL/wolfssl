...
Finite field Diffie-Hellman is that same modular exponentiation with the group
prime as the modulus, so DH offload comes and goes with the RSA half of the
port: a build without RSA (`NO_RSA`, or `WOLFSSL_SEC_QORIQ_NO_RSA`) keeps FFDHE
in software even though the PKHA itself could compute it. Reaching it needed a
new callback hook in wolfCrypt: `WC_PK_TYPE_DH` was in the enum but had no
entry in `wc_CryptoInfo`, no `wc_CryptoCb_Dh()` and no call site in `dh.c`, so
no port could offload DH at all. That hook is part of this work.
...
performs the instantiation, retrying with a wider entropy sample (up to and
including the documented maximum delay) if the statistical checks reject the
attempt. It runs exactly once, inside `wc_SecQoriqInit()` before the crypto
callback is registered, because the status check and TRNG programming are not
atomic and must never race a second initialiser; if it fails, hardware
seeding is recorded unavailable and seeding stays with the platform source.
...
- `linux` maps the SEC through `/dev/mem`. The engine never DMAs through
  ordinary process pages (they can be migrated or copied-on-write behind any
  `/proc/self/pagemap` snapshot): every job's descriptor and data buffers
  are staged through one pinned, pre-faulted, contiguity-verified bounce
  arena (`SEC_QORIQ_ARENA_SZ`, default 128 KB), and pagemap is consulted
  only for that driver-owned memory, re-verified before each job. It
  initialises on a stock kernel but **does not offload anything on a typical
  36-bit part**; see "The Linux backend on a 36-bit part" below before using
  it.
- `sim` is a host-runnable model of the job ring used by the in-tree tests:
  it runs on any architecture, executes descriptors with wolfCrypt's own
  software crypto, and adds fault injection for the timeout, reset and
  status-decode paths. It exists for `make check` and development, never for
  production.

The same port builds through CMake with
`-DWOLFSSL_SEC_QORIQ=baremetal|linux|sim` (or `yes`, which selects
`baremetal`).
...
3. **The 4 GB pointer limit stops the offload.** The driver runs the engine in
   ...
   788.9 ops/s). Making this useful needs either 64-bit descriptor pointer
   mode (`MCFGR[PS]`), or DMA buffers below 4 GB. The second is reachable
   without a kernel driver: boot with `mem=` so Linux stops managing the top
   of DDR, then carve the arena out of that reserved range through
   `/dev/mem`. It is contiguous, it is below the limit, and being outside
   System RAM is also what lets `CONFIG_STRICT_DEVMEM` map it. That costs a
   boot argument rather than a code change, and is how the separate
   `--enable-caam=linux` backend reaches the same engine.
...
##### Bare metal chain-loaded from U-Boot

The same silent fallback bites on bare metal when the application is loaded by
something that has already brought the CAAM up. wolfBoot never touches the
engine, so an application it starts finds `MCFGR[PS]` at its reset value and
offloads normally. U-Boot does initialise the CAAM, and leaves it in 64-bit
pointer mode: on a T1040D4RDB, `MCFGR` reads `0x000122F1` at the U-Boot prompt.
`wc_SecQoriqInit()` refuses that rather than hand the engine descriptor
pointers it will read as 64-bit, and returns `NOT_COMPILED_IN` (-174), so
everything runs in software.

The distinction matters because the symptom is one line of output:
`QorIQ SEC: init failed (-174), software only` instead of
`QorIQ SEC: enabled`. The test suite passes either way. When measuring or
regression-testing, read the offload counters, and either start the
application from wolfBoot or clear `MCFGR[PS]` before entry.
...
| `SEC_QORIQ_ARENA_SZ` | 131072 | Linux pinned bounce arena. Bounds the largest job, since every descriptor and buffer is staged through it. |
...
| `WOLFSSL_SEC_QORIQ_NO_RSA` | undefined | Leave RSA in software, keeping wolfCrypt's base blinding. Also keeps FFDHE in software, which rides the same protocol. |
...
#### wolfCrypt benchmark on the T1040D4RDB

`wolfcrypt/benchmark`, 1 KB blocks, on the NXP T1040D4RDB (T1040E, e5500, 1400
MHz), built with the port's default configuration: SP math with PowerPC
assembly enabled. Both rows come from the same run. Offload was confirmed from
the driver counters (2,896,257 jobs submitted, cipher 100% offloaded), not
inferred from the timings.

| Algorithm | Software | SEC | Speedup |
|---|---:|---:|---:|
| AES-128-CBC-enc | 55.22 | **99.11** MiB/s | 1.8x |
| AES-256-CBC-enc | 40.06 | **97.88** MiB/s | 2.4x |
| AES-128-GCM-enc | 35.83 | **96.80** MiB/s | 2.7x |
| AES-256-GCM-enc | 29.19 | **93.07** MiB/s | **3.2x** |

Public key, same run, operations per second:

| Operation | Software | SEC | Speedup |
|---|---:|---:|---:|
| ECDHE P-256 agree | 479.24 | 499.33 | 1.0x |
| ECDSA P-256 verify | 422.21 | 355.79 | **0.8x** |
| ECDSA P-256 sign | 769.24 | 490.34 | **0.6x** |

Note the ECC rows: against SP math on a 1.4 GHz e5500, **the SEC does not win at
P-256**. It ties on ECDH and loses on sign, because software does the operation
in about 1.3 ms while a PKHA job costs what it costs regardless of operand
size. This differs from the T2080 table above, which was taken with `NO_ASM=1`
against a much slower software baseline. Offload is a per-operation decision,
not a per-chip one: on this part the win is bulk symmetric crypto.

RSA and DH are not built into this configuration, so they are not measured
here.

Reproducing this does not need TRACE32 or a NOR flash. Build the wolfBoot
bare-metal test app with `SEC_QORIQ=1 SEC_QORIQ_CCSRBAR=0xFE000000UL
WOLFCRYPT_BENCHMARK=1 CFLAGS_EXTRA=-DAPP_SKIP_WOLFBOOT_STATE`, then from the
board's U-Boot prompt `tftp 0x70000000 <image.bin>` and `go 0x70000000`.
**Clear the long-pointer bit first**: stock U-Boot leaves `MCFGR[PS]` set
(`md 0xFE300004` reads `0x000122f1`) and the driver then declines with
`NOT_COMPILED_IN`, printing "init failed (-174)". `mw 0xFE300004 0x000022f1`
clears it. Miss that step and the benchmark's HW rows silently equal its SW
rows.
...
- Public key covers ECDSA, ECDH, RSA and finite-field DH agreement. Key
  generation stays in software for all of them, so that the caller's `WC_RNG`
  remains the source of the private key. RSA private-key operations use the
  plain private exponent `d`; the CRT form (`p`, `q`, `dP`, `dQ`, `qInv`) is
  not implemented, so a CRT key still exponentiates against the full modulus
  rather than taking the faster path.
...
- ECDSA sign uses the engine's internal per-signature nonce rather than the
  ...
  hardware. Deterministic ECDSA (RFC 6979) and a nonce installed with
  `wc_ecc_sign_set_k()` therefore cannot be offloaded and fall back to
  software, so the configured nonce is honoured rather than silently
  replaced.
...
- The port is stack hungry on the public key paths, roughly 900 bytes on top
  ...
  which moves both to the heap. Every driver call also carries a
  `SecQoriqDesc` on its stack, about 800 bytes now that it records the
  buffer table resolved at submission.
...
- Errors that can only occur before a descriptor is handed to the engine
  (argument and shape validation, an unmappable buffer, a missing
  capability) fall back to software through the crypto callback. `WC_HW_E`
  is a hard error by design: it can be raised after submission, when the
  engine may already have modified an in-place buffer or a chaining IV, and
  a silent software retry would compute over corrupted state. A dead engine
  still falls back cleanly for subsequent operations because the router's
  device checks decline before dispatch.
- Single-shot hashing is bounded by the 64-word descriptor at 30 FIFO loads,
  about 1.9 MB of message; larger inputs are refused with `BAD_FUNC_ARG`
  (`wc_SecQoriqHash` is only reachable through the direct driver API, the
  callback router never routes hashing).
- RNG4 instantiation runs exactly once, inside `wc_SecQoriqInit()` before
  the crypto callback is registered, because the RDSTA check and TRNG
  programming are not atomic. If every entropy delay up to the maximum is
  rejected, hardware seeding is recorded unavailable and seeding stays with
  the platform source; later RNG users only re-read RDSTA (side-effect free)
  in case another agent instantiated the handle.
- In-tree tests run against the simulated backend: `./configure
  --enable-sec-qoriq=sim && make check` (any host) exercises the job ring
  state machine with fault injection - submission, completion, timeout, the
  ring-reset / DMA-reset / poisoned-device escalation, status decoding,
  `jobCount` accounting - plus known answers for AES CBC (out-of-place and
  in-place), GCM including corrupted-tag rejection and plaintext scrubbing,
  multi-chunk hashing, RNG4 instantiation retry and exhaustion, and
  FFDHE-2048 agreement through the modexp protocol descriptor, each
  cross-checked against the software implementation. The `sec-qoriq.yml`
  workflow runs this plus PowerPC cross-compiles of both real backends on
  every PR.
- Real hardware remains the final gate: the port was validated with an
