    ...
    byte* out)
{
    ...
    /* The 64-word descriptor bounds how much message the chunk loop below
     * can describe: header and OPERATION take 2 words, the digest store 2,
     * leaving 30 two-word FIFO loads. Check up front so an oversized
     * message is refused explicitly (the documented limit, about 1.9 MB)
     * instead of surfacing as a mid-assembly descriptor overflow. */
    chunkMax = SEC_QORIQ_FIFO_MAX - (SEC_QORIQ_FIFO_MAX % blockSz);
    maxIn = ((((SEC_QORIQ_DESC_MAX_WORDS - 4) / 2) - 1) * chunkMax) +
        SEC_QORIQ_FIFO_MAX;
    if (inSz > maxIn) {
        WOLFSSL_MSG("sec_qoriq: message too large for a single-shot hash");
        return BAD_FUNC_ARG;
    }
