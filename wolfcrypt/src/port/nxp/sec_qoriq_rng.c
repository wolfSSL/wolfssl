 ...
 *
 * Instantiation goes through the job ring, not direct DECO0 access: Linux
 * needs DECO0 because it must instantiate before handing out rings, whereas
 * we already own one.
 */
...
/* Runs exactly once, from wc_SecQoriqInit() before the crypto callback is
 * registered: the RDSTA check, the TRNG programming and the instantiation
 * job below are not atomic, and two initialisers interleaving their
 * RTSDCTL writes would leave the TRNG programmed by one and instantiated by
 * the other. Nothing else may call this once the device is visible. */
int wc_SecQoriqRngInit(void)
{
     ...
     * at this sample length, so widen it and retry. Inclusive bound: the
     * documented maximum delay is itself a valid, and last, attempt. */
...
int wc_SecQoriqRandomBlock(byte* out, word32 sz)
{
    ...
    if (dev->rngReady == 0) {
        /* Never (re)program the TRNG here: that ran once, single threaded,
         * in wc_SecQoriqInit(). Re-reading RDSTA is side-effect free and
         * picks up a handle another agent instantiated since; rngReady only
         * ever goes 0 -> 1, so the check is race free. */
        if (wc_SecQoriqRead(dev->regs, SEC_QORIQ_RDSTA) &
                SEC_QORIQ_RDSTA_IF0) {
            dev->rngReady = 1;
        }
