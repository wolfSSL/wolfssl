 ...
 *
 *   - the OPERATION comes before the IV, and the IV arrives as a FIFO LOAD
 *     of type IV rather than a context load,
 *   - the output FIFO STORE is emitted before the input FIFO LOAD,
 *   - every FIFO LOAD carries FC1 (flush class 1) except the last, which
 *     carries LC1. Which load is last depends on the direction and on
 *     whether there is AAD, so the flag is patched in afterwards,
 *   - on decrypt the expected tag is loaded as type ICV and the engine
 *     compares it itself, reporting a mismatch as a job error.
 *
