    ...
    const byte* in, word32 inSz, byte* out, byte* tag, word32 tagSz)
{
    ...
    if (tagSz == 0 || tagSz > SEC_QORIQ_AES_BLOCK) {
        return BAD_FUNC_ARG;
    }
    ...
    if (ret != 0) {
         ...
         *
         * Only then, though. Any other failure means the engine could not do
         * the job, and the router turns those into CRYPTOCB_UNAVAILABLE so
         * software redoes the work over this same buffer, which for in-place
         * decryption is also the ciphertext. */
