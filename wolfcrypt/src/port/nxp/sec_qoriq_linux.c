 ...
 *
 * Maps the SEC through /dev/mem. The kernel's caam driver must not hold the
 * job ring this driver claims; blacklist it or unbind that ring first.
 *
 * The engine never DMAs through ordinary process pages: they can migrate,
 * collapse or copy-on-write after any pagemap lookup, and an untouched
 * buffer may still be the shared zero page. Every job's descriptor and data
 * are staged instead through one pinned (MAP_LOCKED), pre-faulted,
 * physically contiguous arena; pagemap is consulted only for that
 * driver-owned memory, and the arena's translation is re-verified before
 * every job rather than trusted.
 *
...
/* Bounce arena state; the machinery lives further down with JobBegin/
 * MapBuf/JobEnd, but wc_SecQoriqMapRegs() retires a lost arena and
 * wc_SecQoriqUnmapRegs() releases a live one. */
...
int wc_SecQoriqMapRegs(byte** regsOut)
{
    ...
    /* A fresh mapping is a fresh session. An arena abandoned to a still-live
     * engine on a prior quiesce failure was leaked deliberately; retire the
     * stale pointer and clear the latch so the next job allocates a new one. */
    if (secArenaLost) {
        secArena = NULL;
        secArenaPhys = 0;
        secArenaLost = 0;
...
{
    /* MEMORY_E, not WC_HW_E: nothing was submitted, so the router falls back
     * to software rather than hard-failing the caller. A fresh
     * wc_SecQoriqMapRegs() clears the latch. */
    if (secArenaLost) {
        return MEMORY_E;
    }
