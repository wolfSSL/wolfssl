 ...
 * little-endian 32-bit word arrays. Used by the ECDSA paths and result statics. */
...
#if !defined(NO_HMAC) && !defined(NO_SHA256)
/* HUK-bound HMAC-SHA256 over a secure-key slot. The wolfCrypt HMAC
 * crypto-callback is incremental (wc_HmacUpdate delivers the message in chunks,
 * wc_HmacFinal produces the digest), but the HAL secure-key HMAC engine state
 * (sk_init -> update -> sk_final) cannot span those separate calls under one
 * mutex hold without risking interleave with other Hmac objects. So the message
 * is accumulated into a heap buffer hung off hmac->devCtx and the whole MAC runs
 * one-shot at final under a single mutex hold. This suits HUK MAC / KDF use
 * (short messages); it buffers the full message rather than streaming it. */
...
/* Append a message chunk to the per-Hmac accumulation buffer (grown geometric).
 * On allocation failure the partial buffer is released later by the HmacFree
 * cleanup callback (devCtx stays set). */
static int Rtl8735bHuk_HmacAccumulate(Hmac* hmac, const byte* in, word32 inSz)
{
    ...
    ctx = (Rtl8735bHmacCtx*)hmac->devCtx;
    if (ctx == NULL) {
        ...
        ctx = (Rtl8735bHmacCtx*)XMALLOC(sizeof(Rtl8735bHmacCtx), hmac->heap,
                                         DYNAMIC_TYPE_HMAC);
        if (ctx == NULL) {
            return MEMORY_E;
        }
    ...
    if (need > ctx->cap) {
        ...
        nb = (byte*)XREALLOC(ctx->buf, newCap, hmac->heap,
                             DYNAMIC_TYPE_HMAC);
        if (nb == NULL) {
            return MEMORY_E;
        }
...
{
    Rtl8735bHmacCtx* ctx = (Rtl8735bHmacCtx*)hmac->devCtx;
    if (ctx != NULL) {
        if (ctx->buf != NULL) {
            ForceZero(ctx->buf, ctx->cap);   /* cleartext message material */
            XFREE(ctx->buf, hmac->heap, DYNAMIC_TYPE_HMAC);
        }
        XFREE(ctx, hmac->heap, DYNAMIC_TYPE_HMAC);
        hmac->devCtx = NULL;
    }
}
...
/* The HW ECDSA completion statics (huk_ecdsaDone/Err/R/S/veriResult) are defined
...
/* Unwrap the HUK-wrapped private scalar into scalar[] (scalarSz bytes). Derives
 * the HUK slot key from the seed and ECB-sk decrypts the blob in place. */
static int Rtl8735bHuk_UnwrapScalar(const wc_Rtl8735b_EccKey* hk,
    byte* scalar, word32 scalarSz)
{
    int   ret;
#ifdef WOLFSSL_SMALL_STACK
    /* DMA'd by the HAL -- must be 32-byte aligned, so use the aligned bounce. */
    byte* wrapped;
    byte* wrappedAlloc = NULL;
#else
    XALIGNED(32) byte wrapped[WC_RTL8735B_MAX_WRAPPED];
#endif

#ifdef WOLFSSL_SMALL_STACK
    ret = Rtl8735b_BounceOut(WC_RTL8735B_MAX_WRAPPED, &wrapped, &wrappedAlloc);
    if (ret != 0) {
        return ret;
    }
#endif
    XMEMCPY(wrapped, hk->wrapped, hk->wrappedLen);
    ret = wolfSSL_CryptHwMutexLock();
    if (ret != 0) {
        goto cleanup;
    }
    ret = Rtl8735bHuk_DeriveSlotKey(hk->seed);
    if (ret == 0 && hal_crypto_aes_ecb_sk_init(
            (byte)WC_RTL8735B_DERIVED_WB_IDX, WC_RTL8735B_KEYLEN) != 0) {
        ret = WC_HW_E;
    }
    if (ret == 0 && hal_crypto_aes_ecb_decrypt(
            wrapped, hk->wrappedLen, NULL, 0, wrapped) != 0) {
        ret = WC_HW_E;
    }
    wolfSSL_CryptHwMutexUnLock();
    if (ret == 0) {
        XMEMCPY(scalar, wrapped, scalarSz);
    }
cleanup:
#ifdef WOLFSSL_SMALL_STACK
    Rtl8735b_BounceFree(wrappedAlloc, wrapped, WC_RTL8735B_MAX_WRAPPED);
#else
...
/* Route an ECDSA sign request to the HUK backend. The caller attaches a
 * wc_Rtl8735b_EccKey via key->devCtx (see rtl8735b.h). Modes:
 *   1. OTP-resident HW sign (useHwEngine, otpPrkSel != 0): scalar stays in OTP.
 *   2. Wrapped-scalar HW sign (useHwEngine, otpPrkSel == 0): ECB-unwrap, HW sign.
 *   3. Wrapped-scalar software sign (default): unwrap, then sign in software.
 * A plain ecc_key with no devCtx is a general HW offload (signs with its own
 * scalar). The wrapped blob is device-bound (unwraps only on its origin HUK). */
static int Rtl8735bHuk_PkSign(struct wc_CryptoInfo* info)
{
    ...
    /* Modes 2/3 need the seed + wrapped scalar. */
    ...
    if (hk->wrappedLen == 0 ||
            (hk->wrappedLen % WC_AES_BLOCK_SIZE) != 0 ||  /* ECB needs whole blocks */
            hk->wrappedLen > WC_RTL8735B_MAX_WRAPPED ||
            hk->plainLen == 0 ||
            hk->plainLen > hk->wrappedLen ||              /* scalar fits in the blob */
            hk->plainLen > (word32)MAX_ECC_BYTES) {
        ret = BAD_FUNC_ARG;
        goto cleanup;
    }
    curveId  = (key->dp != NULL) ? key->dp->id : ECC_SECP256R1;
     ...
     * make the core retry a software sign on the keyless HUK device key). */
    if (hk->useHwEngine && key->dp != NULL && key->dp->id == ECC_SECP256R1) {
...
int wc_Rtl8735b_HukSelfTest(void)
{
    ...
    hctx = (Rtl8735bHmacCtx*)hmac.devCtx;
