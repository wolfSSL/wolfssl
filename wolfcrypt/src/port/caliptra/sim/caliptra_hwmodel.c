                          ...
                          void       *resp,     word32 resp_len)
{
    ...
    if (status == MBOX_STATUS_CMD_FAILURE) {
        ...
        fprintf(stderr,
                ...
                (unsigned)cmd_id);
        /* Per the caliptra_mailbox_exec contract documented in
         * caliptra_port.h: when the failing command is CM_ECDSA_VERIFY,
         * the transport MUST return SIG_VERIFY_E so the port can map a
         * cryptographically invalid signature to the wolfSSL CryptoCb
         * convention of (ret=0, *res=0).  Any other negative code surfaces
         * as a system error rather than a clean verify-failed result.
         * All other commands keep the generic -EIO transport error. */
        if (cmd_id == CM_ECDSA_VERIFY)
            return SIG_VERIFY_E;
        return -EIO;
    }
