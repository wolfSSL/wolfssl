...
/* A stock wolfHAL board.h already names its devices BOARD_*_DEV. Accept those
...
/* Offload gates. A mode is dispatched to hardware only when wolfCrypt enables it
 * and board.h names a device for it. Anything else stays out of the dispatch,
 * returns CRYPTOCB_UNAVAILABLE, and falls back to software, so a board offloads
 * the subset its hardware covers without constraining the wolfSSL build. */
...
/* No fallback for the RNG: CUSTOM_RAND_GENERATE_BLOCK replaces the entropy
 * source outright rather than sitting behind a dispatch that can decline, so
 * there is nothing to fall back to. */
#if defined(WOLFSSL_WOLFHAL_RNG) && !defined(WC_WOLFHAL_RNG_DEV)
    #error "board.h must define WC_WOLFHAL_RNG_DEV (or BOARD_RNG_DEV)"
#endif
...
int wc_wolfHAL_GenerateBlock(byte* output, word32 sz)
{
    whal_Error err;
