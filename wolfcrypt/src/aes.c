...
/* Core RFC 3394 wrapping loop: plaintext blocks already at out+8, initial A in
 * aiv. Writes C[0]=A and wrapped R[i] in place. Validates structural inputs;
 * the caller still owns output-buffer sizing (outSz not visible here). */
        ...
        word32 outSz, const byte* iv)
{
    ...
    /* n must be at least two 64-bit blocks, on a 64-bit boundary; the (n + 1)-
     * block output must fit in outSz.  inSz is capped at INT_MAX - 8 so the
     * returned length (inSz + 8) stays a representable non-negative int. */
...
/* Core RFC 3394 unwrapping loop: decrypts (n+1) blocks in `in`, writes n
 * recovered blocks to out and recovered A to aOut. No integrity check; caller
 * verifies A. */
        ...
        word32 outSz, const byte* iv)
{
    ...
    /* ciphertext is (n + 1) >= 3 blocks on a 64-bit boundary; the n recovered
     * blocks must fit in outSz.  inSz is capped at INT_MAX so the returned
     * length (inSz - 8) stays a representable non-negative int. */
    if (aes == NULL || in == NULL || out == NULL ||
        inSz < 3 * KEYWRAP_BLOCK_SIZE || (inSz % KEYWRAP_BLOCK_SIZE) != 0 ||
        ...
        word32 outSz, const byte* iv)
{
    ...
    /* inSz is capped at INT_MAX - (2*8 - 1) so that rounding up to whole 64-bit
     * blocks and adding the AIV block cannot overflow (which would let padSz + 8
     * wrap and silently bypass the output-size check) and so the returned length
     * (padSz + 8) stays a representable non-negative int; RFC 5649 caps key data
     * at 2^32 octets, so this never rejects a legitimately wrappable key.  A
     * too-small output returns BAD_FUNC_ARG to match the non-padded paths. */
        ...
        word32 outSz, const byte* iv)
{
    ...
    byte zeros[KEYWRAP_BLOCK_SIZE];

    /* ciphertext is (n + 1) >= 2 blocks on a 64-bit boundary; the full
     * (inSz - 8) padded plaintext is written into out in place.  inSz is capped
     * at INT_MAX so the returned length (the recovered MLI, <= inSz) stays a
     * representable non-negative int.  A too-small output returns BAD_FUNC_ARG
     * to match the non-padded RFC 3394 paths. */
    if (aes == NULL || in == NULL || out == NULL ||
        inSz < 2 * KEYWRAP_BLOCK_SIZE || (inSz % KEYWRAP_BLOCK_SIZE) != 0 ||
        inSz > 0x7FFFFFFFU || outSz < inSz - KEYWRAP_BLOCK_SIZE)
        return BAD_FUNC_ARG;
    ...
    if (ConstantCompare(a, expConst, (int)sizeof(kwpAivConst)) != 0) {
        return BAD_KEYWRAP_IV_E;
    }
    ...
    if (mli <= KEYWRAP_BLOCK_SIZE * (n - 1) || mli > KEYWRAP_BLOCK_SIZE * n) {
        return BAD_KEYWRAP_IV_E;
    }

    /* check 3: the trailing b padding octets are zero */
    b = (KEYWRAP_BLOCK_SIZE * n) - mli;
    XMEMSET(zeros, 0, sizeof(zeros));
    if (b > 0 && ConstantCompare(out + mli, zeros, (int)b) != 0) {
        return BAD_KEYWRAP_IV_E;
    }
