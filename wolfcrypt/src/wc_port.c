...
{
    /* Volatile load forces an indirect call, thwarting LTO/DSE. Keeping
     * const in the cast avoids -Wcast-qual; top-level qualifiers on the
     * cast's own (rvalue) result are omitted to avoid MSVC C4197. */
    void (*fn)(const void*) =
        *(void (* const volatile *)(const void*))&bd_sink_ptr;
