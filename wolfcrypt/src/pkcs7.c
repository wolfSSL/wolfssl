...
#include <wolfssl/wolfcrypt/hash.h>
    ...
    /* FIPS v1 / CAVP builds omit these from rsa.h; provide fallbacks */
    #ifndef RSA_PSS_SALT_LEN_DEFAULT
        #define RSA_PSS_SALT_LEN_DEFAULT (-1)
    #endif
    /* MGF1 constants when not from rsa.h (e.g. FIPS v1 reduced header) */
    #ifndef WC_MGF1NONE
                                  ...
                                  byte* hash, word32 hashSz)
{
    ...
    if (hashSz != (word32)hashDigSz)
        return ASN_PARSE_E; /* digest size mismatch with PSS hash params */
     ...
     * saltLen + 2*hLen bytes), which can exceed MAX_PKCS7_DIGEST_SZ.
     * Use MAX_ENCRYPTED_KEY_SZ (512) to cover RSA keys up to 4096-bit. */
    digest = (byte*)XMALLOC(MAX_ENCRYPTED_KEY_SZ, pkcs7->heap,
                            DYNAMIC_TYPE_TMP_BUFFER);
    if (digest == NULL)
        return MEMORY_E;
    key = (RsaKey*)XMALLOC(sizeof(RsaKey), pkcs7->heap, DYNAMIC_TYPE_TMP_BUFFER);
    ...
    for (i = 0; i < MAX_PKCS7_CERTS; i++) {
        ...
        scratch = 0; /* decode from start of this cert's key each time */
        ret = wc_InitRsaKey_ex(key, pkcs7->heap, pkcs7->devId);
        if (ret != 0)
            break;
        ...
        {
            word32 pkSz = dCert->pubKeySize;

            /* RSAPSS certs may report oversized SPKI blobs */
        ...
        {
            int saltLen = (pkcs7->pssSaltLen >= 0) ? pkcs7->pssSaltLen
