...
#ifdef WOLF_CRYPTO_CB_ONLY_MLDSA
    if (ret == 0) {
        /* Not NO_VALID_DEVID: no device can service this one. Expanding a
         * seed is a local computation and the callback protocol has no seed
         * to hand over, so registering a device would not help. The seed
         * expansion is part of the software core this build removes. */
        ret = NOT_COMPILED_IN;
    }
#else
    if (ret == 0) {
...
#else
            /* The seed form needs the software seed expansion, which this
             * build does not have. */
            ret = NOT_COMPILED_IN;
#endif
        }
