                                     ...
                                     int* res, ed448_key* key, wc_Shake *sha)
{
    ...
    *res = 0;
    ...
    /* FIPS 186-5 sec 7.7 step 1: decoding shall fail for y >= p. The point
     * decoder reduces mod p instead, so check the range here. */
    {
        int j;
        int rangeRet = PUBLIC_KEY_E;
        /* The final byte carries only the x sign bit; bits 0-6 shall be zero
         * (RFC 8032 sec 5.2.2). */
        if ((key->p[ED448_PUB_KEY_SIZE - 1] & 0x7f) != 0) {
            return BAD_FUNC_ARG;
        }
        /* Check top part before 0xFE. y ends at byte ED448_PUB_KEY_SIZE-2;
         * starting at the sign byte would accept every y >= p. */
...
int wc_ed448_check_key(ed448_key* key)
{
    ...
    /* No private key, check Y is valid. */
        ...
        if ((key->p[ED448_PUB_KEY_SIZE - 1] & 0x7f) != 0) {
            ...
            /* Kept inside the else: at the outer level this would also clear
             * the non-canonical-pad rejection above. */
