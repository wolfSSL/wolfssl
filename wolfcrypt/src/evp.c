...
/* Guard matches the condition on the iv field of WOLFSSL_EVP_CIPHER_CTX. */
    ...
    {
        switch (ctx->cipherType) {
    /* Every mode below keeps its working IV in the same place, but each is
     * optional: with none of them built there is no case label left and the
     * return would be unreachable. */
    #if !defined(NO_AES) && (defined(HAVE_AES_CBC) || \
        defined(WOLFSSL_AES_COUNTER) || defined(WOLFSSL_AES_OFB) || \
        defined(WOLFSSL_AES_CFB))
        #ifdef HAVE_AES_CBC
            case WC_AES_128_CBC_TYPE:
            case WC_AES_192_CBC_TYPE:
