...
{
#if !defined(USE_ARM_CHACHA_SPEEDUP) && !defined(WOLFSSL_WIDE_BYTE)
    ...
    /* inIv holds one octet per cell, so XMEMCPY into a word32[] would overflow;
     * load the three little-endian nonce/counter words octet-wise instead. */
