...
static int wc_grb_reseed_local(int want_cpu)
{
    byte          seed[WC_DRBG_SEED_SZ];
    unsigned long flags;
    /* cpu is assigned only inside the ret == 0 branch below, and done is set
     * only inside that same branch, so the use under if (done) is always after
     * the assignment.  gcc cannot see that correlation on 32-bit and fails
     * -Werror=maybe-uninitialized there (i386 kernel build, x86_64 does not
     * warn), so give it an explicit initial value. */
    int           cpu = -1, ret, done = 0;
