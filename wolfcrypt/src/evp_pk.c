    ...
    unsigned char* pub, size_t* len)
{
    ...
    switch (pkey->type) {
    ...
    #ifdef HAVE_ED25519_MAKE_KEY
            /* A key carrying only its private half has no public bytes to
             * export, so derive them - into the key's own buffer, and only
             * when it does not already hold them.  wc_ed25519_make_public()
             * writes to whatever buffer it is handed but sets pubKeySet on the
             * key, so deriving into scratch would leave the key claiming a
             * public key it does not hold and every later export or signature
             * would use zeros.  It wants exactly the public key size, not the
             * capacity of the buffer. */
            if (pkey->ed25519->privKeySet && (!pkey->ed25519->pubKeySet)) {
                if (wc_ed25519_make_public(pkey->ed25519, pkey->ed25519->p,
                        ED25519_PUB_KEY_SIZE) != 0) {
                    WOLFSSL_MSG("wc_ed25519_make_public failed");
                    break;
                }
            }
    #endif /* HAVE_ED25519_MAKE_KEY: a verify only build cannot derive one,
            * and the export below reports it missing. */
            bufLen = (word32)sizeof(buf);
            if (wc_ed25519_export_public(pkey->ed25519, buf, &bufLen) != 0) {
        ...
        case WC_EVP_PKEY_ED448:
            ...
            /* See the Ed25519 case: derive into the key's own buffer so the
             * key is never left flagged as holding a public key it does not.
             * wc_ed448_make_public() has no build option of its own. */
            if (pkey->ed448->privKeySet && (!pkey->ed448->pubKeySet)) {
                if (wc_ed448_make_public(pkey->ed448, pkey->ed448->p,
