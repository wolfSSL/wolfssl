                            ...
                            void * heap, int devId)
{
    ...
    if (zSz == 0 || fixedInfoSz == 0 || outputSz == 0) {
        return BAD_FUNC_ARG;
    }

    if (salt == NULL || z == NULL || fixedInfo == NULL || output == NULL) {
        return BAD_FUNC_ARG;
    }
    ...
    /* step 1: cmac extract */
    ret = wc_InitCmac_ex(cmac, salt, salt_len, WC_CMAC_AES, NULL,
                         heap, devId);

    if (ret == 0) {
        ret = wc_CmacUpdate(cmac, z, zSz);
    }

    if (ret == 0) {
        word32 tag_len = WC_AES_BLOCK_SIZE;
        ret = wc_CmacFinal(cmac, Key_kdk, &tag_len);
                        ...
                        void * heap, int devId)
{
    ...
    if (KoutSz == 0 || KoutSz < WC_AES_BLOCK_SIZE || fixedInfoSz == 0) {
        return BAD_FUNC_ARG;
    }
    ...
    while (len_rem >= WC_AES_BLOCK_SIZE) {
        ...
        ret = wc_CmacFinal(cmac, &Kout[KoutSz - len_rem], &tag_len);
        if (ret) { break; }
    ...
    /* all remaining length should be consumed */
    if (ret == 0 && len_rem != 0) {
        WOLFSSL_MSG_EX("wc_KDA_KDF_PRF_cmac: got len_rem %d, expected 0\n",
                       len_rem);
        ret = -1;
    }
