                               ...
                               byte* out, word32* outlen)
{
...
#else
    {
        ...
        /* Unconditional, so it overrides the WOLFSSL_SE050 keyIdSet case that
         * clears checkInf: SE050 ECDH by key id is not usable in approved mode. */
        (void)checkInf;
        if (err == MP_OKAY) {
...
#if FIPS_VERSION3_GE(7,0,0)
/* SP 800-90A Rev 1 sec 11.4.2 and SP 800-90B sec 4.3: a DRBG or entropy source
 * failure shall report its own error indicator, not a caller's generic one. */
static int ecc_err_is_rng_indicator(int err)
{
    return (err == WC_NO_ERR_TRACE(RNG_FAILURE_E)) ||
           (err == WC_NO_ERR_TRACE(DRBG_KAT_FIPS_E)) ||
           (err == WC_NO_ERR_TRACE(DRBG_CONT_FIPS_E)) ||
           (err == WC_NO_ERR_TRACE(DRBG_SHA512_KAT_FIPS_E)) ||
           (err == WC_NO_ERR_TRACE(ENTROPY_RT_E)) ||
           (err == WC_NO_ERR_TRACE(ENTROPY_APT_E));
}
#endif
...
static int _ecc_pairwise_consistency_test(ecc_key* key, WC_RNG* rng)
{
    ...
    if (!err && (flags & WC_ECC_FLAG_DEC_SIGN)) {
        ...
        /* Only a completed sign/verify that disagreed is a PCT failure.  An
         * operational error keeps its own code as far as the remap below. */
        if ((err == 0) && (res == 0))
            err = ECC_PCT_E;
...
#if FIPS_VERSION3_GE(7,0,0)
    /* Let a DRBG/entropy indicator through so the FSM sees the real cause. */
    if ((err != 0) && (! ecc_err_is_rng_indicator(err)))
        err = ECC_PCT_E;
#else
    if (err != 0)
        err = ECC_PCT_E;
