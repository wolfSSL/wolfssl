                           ...
                           const byte* pub, word32 pubLen)
{
    ...
    /* The CCB hardware sign path (wc_Stm32_Ccb_EccSign) supports only P-256.
     * Reject other curves here so an import cannot succeed for a key that can
     * never be signed with, rather than failing later at sign time. */
...
int wc_ecc_dev_make_key(WC_RNG* rng, int keysize, ecc_key* key, int curve_id)
{
    ...
    if (ret == 0) {
        ...
        /* Only an unsupported-curve / bad-arg result falls back to a software
         * key (CRYPTOCB_UNAVAILABLE -> wc_ecc_make_key SW path). A real
         * provisioning hardware fault (e.g. WC_TIMEOUT_E / WC_HW_E) is surfaced
         * so a key the caller bound to the device is never silently downgraded
         * to a plaintext software key. */
