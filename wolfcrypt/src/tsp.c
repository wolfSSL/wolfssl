    ...
    word32 certSz, void* cm, TspTstInfo* tstInfo)
{
    ...
    /* A verified TSTInfo must have carried content for tstInfo to reference.
     * Enforce the invariant explicitly: success with zero content would skip
     * both the rebasing below and the error clear at the end, handing the
     * caller references into the pkcs7->content that wc_PKCS7_Free releases. */
    if ((ret == 0) && (tstInfo != NULL) && (pkcs7->contentSz == 0)) {
        WOLFSSL_MSG("TSP token verified with no content");
        ret = TSP_VERIFY_E;
    }
