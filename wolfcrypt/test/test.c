...
#endif
{
    ...
    if ( (ret = openssl_pkey0_test()) != 0)
        return err_sys("OPENSSL (PKEY) test failed!\n", ret);
    else
        printf( "OPENSSL (PKEY) passed!\n");
...
#define d2i_X509_fp(a,b) NULL
    ...
    pt = (const void*)msg;
    EVP_SignUpdate(sign, pt, count);
    EVP_SignFinal(sign, sig, &sigSz, prvPkey);
