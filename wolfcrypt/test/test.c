...
/* der key from wolfssl/wolfssl/certs_test.h */
...
int openssl_pkey1_test(void)
{
    ...
    const unsigned char* clikey = client_key_der_2048;
    ...
    printf("testing EPKEY 1\n");

    if ((prvKey = EVP_PKEY_new()) == NULL) {
        printf("error creating new private key\n");
        return -1;
    }
    ...
    f = fopen(cliCert, "rb");
    if (f == NULL) {
        printf("error loading %s cert \"is from certs/ directory\"\n", cliCert);
        return -1;
    }

    x509 = wolfSSL_X509_load_certificate_file(cliCert, SSL_FILETYPE_PEM);
    /* x509 = d2i_X509_fp(f, NULL); not on phase 2 list but used to get key for test */
    if (x509 == NULL) {
        printf("error loading creating x509\n");
        return -1;
    }

    pubKey = X509_get_pubkey(x509);
    if (pubKey == NULL) {
        printf("error loading in public key\n");
        return -1;
    }

    prvKey = d2i_PrivateKey(EVP_PKEY_RSA, NULL, &clikey, (long)sizeof_client_key_der_2048);

    if (prvKey == NULL) {
        printf("error loading in private key\n");
        return -1;
    }

    /* phase 2 API to create EVP_PKEY_CTX and encrypt/decrypt */
    if (EVP_PKEY_bits(prvKey) != 2048) {
        printf("error getting private key bits\n");
        return -1;
    }
    printf("evp private key bits = %d\n", EVP_PKEY_bits(prvKey));

    if (EVP_PKEY_size(prvKey) != 256) {
        printf("error getting private key size\n");
        return -1;
    }
    printf("evp private key size = %d\n", EVP_PKEY_size(prvKey));
    ...
    ret = EVP_PKEY_encrypt(enc, cipher, &outlen, msg, sizeof(msg));
    if (ret < 0) {
        printf("error encrypting msg\n");
        return -1;
    }
    ...
    memset(plain, 0, sizeof(plain));
    ret = EVP_PKEY_decrypt(dec, plain, &outlen, cipher, sizeof(cipher));
    if (ret != 1) {
        printf("error decrypting msg\n");
        return -1;
    }
