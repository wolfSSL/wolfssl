...
static int test_wc_AesCbcEncryptDecrypt (void)
{
    ...
    if (ret == 0) {
        ret = wc_AesCbcEncrypt(&aes, enc, vector, AES_BLOCK_SIZE * 2);
        if (ret == 0) {
            ret = wc_AesSetKey(&aes, key32, AES_BLOCK_SIZE * 2,
                                                    iv, AES_DECRYPTION);
        }
        if (ret == 0) {
            ret = wc_AesCbcDecrypt(&aes, dec, enc, AES_BLOCK_SIZE * 2);
            if (XMEMCMP(vector, dec, sizeof(vector)) != 0) {
                ret = SSL_FATAL_ERROR;
            }
        }
    }
    ...
    #ifdef HAVE_AES_DECRYPT
        if (ret == 0) {
            ret = wc_AesCbcDecryptWithKey(dec2, enc, AES_BLOCK_SIZE * 2,
                                     key32, sizeof(key32)/sizeof(byte), iv);
            if (ret != 0 || XMEMCMP(vector, dec2, sizeof(vector)) != 0) {
                ret = SSL_FATAL_ERROR;
            }
        }
    #endif
    ...
    /* Pass in bad args */
    if (ret == 0) {
        cbcE = wc_AesCbcEncrypt(NULL, enc, vector, AES_BLOCK_SIZE * 2);
        if (cbcE == BAD_FUNC_ARG) {
            cbcE = wc_AesCbcEncrypt(&aes, NULL, vector, AES_BLOCK_SIZE * 2);
        }
        if (cbcE == BAD_FUNC_ARG) {
            cbcE = wc_AesCbcEncrypt(&aes, enc, NULL, AES_BLOCK_SIZE * 2);
        }
        if (cbcE == BAD_FUNC_ARG) {
            cbcE = 0;
        } else {
    ...
    if (ret == 0) {
        cbcD = wc_AesCbcDecrypt(NULL, dec, enc, AES_BLOCK_SIZE * 2);
        if (cbcD == BAD_FUNC_ARG) {
            cbcD = wc_AesCbcDecrypt(&aes, NULL, enc, AES_BLOCK_SIZE * 2);
        }
        if (cbcD == BAD_FUNC_ARG) {
            cbcD = wc_AesCbcDecrypt(&aes, dec, NULL, AES_BLOCK_SIZE * 2);
        }
        if (cbcD == BAD_FUNC_ARG) {
    ...
    printf(resultFmt, cbcD == 0 ? passed : failed);
        ...
        if (ret == 0) {
            cbcDWK = wc_AesCbcDecryptWithKey(NULL, enc, AES_BLOCK_SIZE * 2,
                                     key32, sizeof(key32)/sizeof(byte), iv);
            if (cbcDWK == BAD_FUNC_ARG) {
                cbcDWK = wc_AesCbcDecryptWithKey(dec2, NULL, AES_BLOCK_SIZE * 2,
                                     key32, sizeof(key32)/sizeof(byte), iv);
            }
            if (cbcDWK == BAD_FUNC_ARG) {
                cbcDWK = wc_AesCbcDecryptWithKey(dec2, enc, AES_BLOCK_SIZE * 2,
...
static int test_wc_AesCtrEncryptDecrypt (void)
{
    ...
    ret = wc_AesSetKeyDirect(&aesEnc, key32, AES_BLOCK_SIZE * 2,
                                                    iv, AES_ENCRYPTION);
    if (ret == 0) {
        ...
        if (ret == 0) {
            ...
            ret = wc_AesSetKeyDirect(&aesDec, key32, AES_BLOCK_SIZE * 2,
                                                    iv, AES_ENCRYPTION);
        }
        if (ret == 0) {
            ret = wc_AesCtrEncrypt(&aesDec, dec, enc, sizeof(enc)/sizeof(byte));
            if (XMEMCMP(vector, dec, sizeof(vector))) {
                ret = SSL_FATAL_ERROR;
            }
        }
    }
...
static int test_wc_AesGcmEncryptDecrypt (void)
{
    ...
    ret = wc_AesGcmSetKey(&aes, key32, sizeof(key32)/sizeof(byte));
    if (ret == 0) {
        ret = wc_AesGcmEncrypt(&aes, enc, vector, sizeof(vector)/sizeof(byte),
                                        iv, sizeof(iv)/sizeof(byte), resultT,
                                        sizeof(resultT), a, sizeof(a));
    }
    if (ret == 0) {
        ret = wc_AesGcmDecrypt(&aes, dec, enc, sizeof(enc)/sizeof(byte),
                                        iv, sizeof(iv)/sizeof(byte), resultT,
                                        sizeof(resultT), a, sizeof(a));
    }
    ...
    /* GmacUpdate passes NULL so, no check for in and out variables. */
    if (ret == 0) {
        gcmE = wc_AesGcmEncrypt(NULL, enc, vector, sizeof(vector)/sizeof(byte),
                        iv, sizeof(iv)/sizeof(byte), resultT, sizeof(resultT),
                        a, sizeof(a));
        if (gcmE == BAD_FUNC_ARG) {
            gcmE = wc_AesGcmEncrypt(&aes, enc, vector, sizeof(vector)/sizeof(byte),
                        NULL, sizeof(iv)/sizeof(byte), resultT, sizeof(resultT),
                        a, sizeof(a));
        }
        if (gcmE == BAD_FUNC_ARG) {
            gcmE = wc_AesGcmEncrypt(&aes, enc, vector, sizeof(vector)/sizeof(byte),
                            iv, sizeof(iv)/sizeof(byte), NULL, sizeof(resultT),
                            a, sizeof(a));
        }
        if (gcmE == BAD_FUNC_ARG) {
            gcmE = wc_AesGcmEncrypt(&aes, enc, vector, sizeof(vector)/sizeof(byte),
        ...
        if (gcmE == BAD_FUNC_ARG) {
            gcmE = wc_AesGcmEncrypt(&aes, enc, vector, sizeof(vector)/sizeof(byte),
                            iv, sizeof(iv)/sizeof(byte), resultT,
                            sizeof(resultT) - 5, a, sizeof(a));
        }
        if (gcmE == BAD_FUNC_ARG) {
            /*  MINSZ < iv < MAXLSZ */
            gcmE = wc_AesGcmEncrypt(&aes, enc, vector, sizeof(vector)/sizeof(byte),
                            badIv, sizeof(badIv)/sizeof(byte), resultT,
                            sizeof(resultT), a, sizeof(a));
        }
...
static int test_wc_GmacUpdate (void)
{
   ...
   byte    key32[] =
    ...
    if (ret == 0) {
        ...
        if (ret == BAD_FUNC_ARG) {
            ret = wc_GmacUpdate(&gmac, iv3, sizeof(iv3), authIn3,
                                sizeof(authIn3), NULL, sizeof(tag3) - 5);
        }
        if (ret == BAD_FUNC_ARG) {
            ret = wc_GmacUpdate(&gmac, iv3, sizeof(iv3), authIn3,
