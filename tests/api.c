...
static int test_wc_Chacha_Process (void)
{
    ...
    /* test counter roll over */
    {
        ...
        const byte  expected[128] = {
            
        };
        const byte iv2[] =
        {
        ...
        XMEMSET(rollover, 1, 128);
        ret = wc_Chacha_SetIV(&enc, iv2, 0xffffffff);
        AssertIntEQ(ret, 0);

        ret = wc_Chacha_Process(&enc, cipher, rollover, 128);
        AssertIntEQ(ret, 0);
        ret = XMEMCMP(expected, cipher, 128);
        AssertIntEQ(ret, 0);
    }
