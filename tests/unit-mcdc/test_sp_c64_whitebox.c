...
/*
 * MC/DC white-box supplement for wolfcrypt/src/sp_c64.c.
 *
 * sp_c64.c is the 64-bit *portable C* SP math backend: unlike sp_x86_64.c
 * (or the other asm backends) it has NO cpuid dispatch at all -- every
 * decision in the file is either
 *   - an `err == MP_OKAY` error-propagation guard (needs an earlier failure
 *     to flip, i.e. fault injection -- see the residuals note below), or
 *   - a genuinely data-dependent guard: argument range checks
 *     (mp_count_bits(...) > N), point-at-infinity checks
 *     (sp_<n>_iszero_<n>(...)), or a caller-supplied flag
 *     (e.g. `inMont` in sp_ecc_mulmod_add_<n>/sp_ecc_mulmod_base_add_<n>).
 *
 * There is nothing to force via a cpuid mask here -- every decision this
 * file drives is reached by constructing the right *data* and calling the
 * right entry point, either the public wc_* API (for the general
 * sign/verify/keygen arithmetic) or the sp_ecc_*_<size>()/sp_ecc_is_point_
 * <size>()/sp_ecc_check_key_<size>() entry points directly (these are
 * ordinary global functions in sp_c64.c, not file-static, so no special
 * access trick is needed -- just #include the .c file and call them).
 *
 * This is a coverage-driving supplement, not a known-answer test: the
 * correctness of the arithmetic is already covered by the normal wolfCrypt
 * test suite. The only goal here is to reach each guard with both a true
 * and a false operand vector where that is possible without solving a
 * discrete log (i.e. without needing a *specific* point value that some
 * attacker-hard relationship holds), and without crashing; every operation
 * result is discarded except for the coarse "did it fail outright" checks
 * used to decide whether to WB_NOTE a skip.
 *
 * -------------------------------------------------------------------------
 ...
 * (inMont == 1) side of every one of these decisions is permanently
 * uncovered by the ordinary wolfCrypt test suite (roughly a dozen decisions
 * per curve size across the two functions, times three SP-accelerated
 * curves = the largest single class of gaps in this file). wb_run_mulmod_add
 * below calls both functions directly with a real (on-curve) point pair for
 * every combination of inMont in {0, 1} and map in {0, 1}: inMont == 1
 * mathematically mistreats an ordinary affine point as already being in
 * Montgomery form, which produces a "wrong" but perfectly well-defined
 * result through the same fixed-shape field arithmetic -- exactly the
 * "did it crash" bar this supplement holds itself to.
 *
 * -------------------------------------------------------------------------
 ...
 *      ((privm != NULL) && (mp_count_bits(privm) > N))`
 *     true independently.
 ...
 *
 * -------------------------------------------------------------------------
 * Residuals (documented, not driven -- see file header of
 * test_sp_x86_64_whitebox.c for the same policy applied to the asm backend)
 * -------------------------------------------------------------------------
 * - The `err == MP_OKAY` operand of every `(err == MP_OKAY) && X` decision
 *   in this file (there are dozens): every one of these needs an EARLIER
 *   step in the same function to have already failed. None of the failure
 *   modes involved (MEMORY_E from an allocator that isn't faked in this
 *   harness, or a downstream MP_VAL/ECC_* error from a prior stage) can be
 *   forced without fault injection into sp_c64.c's own allocator/arithmetic,
 *   which is out of scope for a coverage supplement that must not touch the
 *   library's control flow.
 * - `wc_LockMutex(&sp_cache_<n>_lock) != 0` in the ECC point-cache logic:
 *   requires the mutex itself to fail to lock -- fault injection.
 * - `for (i = SP_ECC_MAX_SIG_GEN; err == MP_OKAY && i > 0; i--)` together
 *   with `(err == MP_OKAY) && (!sp_<n>_iszero_<n>(s))` inside
 *   sp_ecc_sign_<n>(): the retry-on-r==0-or-s==0 loop in ECDSA signing.
 *   Hitting r == 0 or s == 0 with a real curve order requires the random
 *   per-signature scalar k to land on one of a vanishingly small set of
 *   values out of ~2^256 (or ~2^384 / ~2^521) -- cryptographically
 *   negligible, not something this supplement forces.
 */
 ...
 * sp_<size>_* point/field math in sp_c64.c (there is no faster/slower
 * path to choose between -- this IS the only path).
 * -------------------------------------------------------------------- */
...
int main(void)
{
    printf("sp_c64.c white-box supplement\n");
#if defined(WOLFSSL_HAVE_SP_ECC) || defined(WOLFSSL_HAVE_SP_RSA) || \
    defined(WOLFSSL_HAVE_SP_DH)
    wb_run_ecc();
    wb_run_rsa();
