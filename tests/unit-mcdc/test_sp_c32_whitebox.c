...
/*
 * MC/DC white-box supplement for wolfcrypt/src/sp_c32.c.
 *
 * sp_c32.c is the 32-bit portable-C SP math backend: unlike sp_x86_64.c (or
 * any of the other asm-accelerated sp_*.c backends) it contains NO cpuid/
 * feature-mask dispatch at all -- it is pure, single-path C, always compiled
 * whenever SP_WORD_SIZE == 32 (the whole file body is wrapped in
 * "#if SP_WORD_SIZE == 32 ... #endif"). Ordinary tests/api traffic already
 * exercises the "everything succeeded, ordinary operands" side of most of
 * its decisions; the residual gaps are almost all the *unlikely-but-not-
 * fault-injected* side of a handful of decision families that public-API
 * traffic essentially never reaches with in-range, valid data:
 *
 *   1. `if ((err == MP_OKAY) && (!inMont))` in sp_ecc_mulmod_add_<n>() and
 *      sp_ecc_mulmod_base_add_<n>() (256/384/521): every real caller in this
 *      tree (see wolfcrypt/src/eccsi.c) always passes inMont == 0, so the
 *      "already in Montgomery form, skip the mod_mul_norm conversion" half
 *      of the decision is never taken. Driven here directly, with valid
 *      scalars/points, across all four (inMont, map) combinations.
 *
 *   2. Point-at-infinity / doubling-collision guards keyed off
 *      sp_<n>_iszero_<n>(z) (and the paired iszero(x) && iszero(y) check)
 *      inside sp_<n>_add_points_<n>() and sp_<n>_calc_vfy_point_<n>().
 *      These only fire when a projective point addition produces z == 0,
 *      i.e. when adding a point to its own negation (true infinity) or to
 *      itself (doubling collision via the general add formula) -- states
 *      that essentially never occur from independently-random ECDSA
 *      sign/verify/ECDH traffic. Driven here by feeding sp_<n>_add_points_<n>
 *      a real point P and its real negation -P (and P and P again), and by
 *      feeding sp_<n>_calc_vfy_point_<n> a zero scalar (0 * point ==
 *      infinity, forcing p->z == 0 through the same code the real verify
 *      path uses).
 *
 *   3. `mp_count_bits(pX) > <curve size>` (and pY, and privm) inside
 *      sp_ecc_check_key_<n>(): normal callers only ever pass ordinates that
 *      already fit the curve, since they come from a parsed/generated key.
 *      Driven here with an explicit 2^<curve size> (one bit too many) fed to
 *      each operand in turn, alongside an all-in-range baseline call.
 *
 * The general Montgomery/point arithmetic itself (mul/sqr/mod/point-add/
 * point-double, the bulk of the file's ~188 decisions) is covered by driving
 * the public ECC sign/verify/ECDH, RSA sign/verify (with key generation),
 * and DH key-agreement entry points below -- no cpuid masking is needed or
 * possible here, so (unlike test_sp_x86_64_whitebox.c) there is only ever
 * one pass through each of these.
 *
 ...
 * instrumented sp_c32.c). NOT part of the wolfSSL build; not registered in
 ...
 *    (e.g. inside sp_ecc_mulmod_add_<n>, sp_ecc_check_key_<n>,
 *    sp_<n>_calc_vfy_point_<n>): `err` only becomes non-MP_OKAY through a
 *    prior allocation failure (SP_ALLOC_VAR/XMALLOC returning NULL) or an
 *    already-reported error from an earlier step in the same function.
 *    Forcing it requires fault-injecting the allocator, which is out of
 *    scope for a coverage-driving supplement operating through the public/
 *    file-static API surface with real data.
 *  - `wc_LockMutex(&sp_cache_<n>_lock) != 0` (the FP-cache mutex-lock
 *    failure check inside sp_ecc_mulmod_<n>()/sp_ecc_mulmod_base_<n>()):
 *    only reachable via a fault-injected mutex implementation.
 *  - The `SP_ECC_MAX_SIG_GEN` retry loop and its paired `!sp_<n>_iszero_<n>
 *    (s)` re-roll check inside sp_ecc_sign_<n>(): looping again only
 *    happens when a freshly generated nonce k produces r == 0 or a
 *    resulting signature s == 0, both cryptographically negligible
 ...
 * for each SP-accelerated curve size compiled in. No cpuid masking is
 * involved -- sp_c32.c is single-path C -- so this is exercised once.
 * -------------------------------------------------------------------- */
...
/* Build -P (same x, y = fieldPrime - y, z = 1) from a real curve point,
 * so a genuine P + (-P) cancellation can be fed to sp_<n>_add_points_<n>()
 * to force its point-at-infinity (z == 0 && x == 0 && y == 0) branch --
 * a state normal sign/verify/ECDH traffic essentially never produces. */
 ...
 * inMont x map combinations, sp_<n>_add_points_<n> point-at-infinity /
 * doubling-collision, sp_<n>_calc_vfy_point_<n> iszero(p->z), and
 * sp_ecc_check_key_<n> mp_count_bits() overflow -- see the file header for
 * the full rationale. One function per curve size since the callee names
 * (word-count suffixes) differ per size in this 32-bit-word backend.
 * ======================================================================= */
#ifndef WOLFSSL_SP_NO_256
static void wb_run_gap_256(void)
{
    ...
    /* --- Target gap 2a: sp_256_add_points_9()'s iszero(z) /
     * (iszero(x) && iszero(y)) branches. --- */
    if (ok && negP != NULL) {
        sp_point_256 pA;
        sp_point_256 pB;
        sp_digit     addTmp[12 * 9];
        ...
        sp_256_point_from_ecc_point_9(&pA, &keyA.pubkey);
        sp_256_point_from_ecc_point_9(&pB, negP);
        sp_256_add_points_9(&pA, &pB, addTmp);

        /* P + P: doubling collision via the general add formula -> z == 0
         * but x/y not both zero, takes the "else" branch. */
        XMEMSET(&pA, 0, sizeof(pA));
        XMEMSET(&pB, 0, sizeof(pB));
        XMEMSET(addTmp, 0, sizeof(addTmp));
        sp_256_point_from_ecc_point_9(&pA, &keyA.pubkey);
        sp_256_point_from_ecc_point_9(&pB, &keyA.pubkey);
        sp_256_add_points_9(&pA, &pB, addTmp);
        ...
        XMEMSET(addTmp, 0, sizeof(addTmp));
        sp_256_point_from_ecc_point_9(&pA, &keyA.pubkey);
        sp_256_point_from_ecc_point_9(&pB, &keyB.pubkey);
        sp_256_add_points_9(&pA, &pB, addTmp);

        WB_NOTE("P-256 add_points infinity/doubling/ordinary exercised");
    }

    /* --- Target gap 2b: sp_256_calc_vfy_point_9()'s
     * sp_256_iszero_9(p1->z) / sp_256_iszero_9(p2->z), forced by a zero
     * scalar (0 * point == infinity), mirroring how sp_ecc_verify_256()
     * itself reaches these checks. --- */
    if (ok) {
        int u1zero, u2zero;
        ...
        for (u1zero = 0; u1zero <= 1; u1zero++) {
            for (u2zero = 0; u2zero <= 1; u2zero++) {
                ...
                sp_256_point_from_ecc_point_9(&p2, &keyB.pubkey);
                s[0] = 7;
                u1[0] = u1zero ? 0 : 5;
                u2[0] = u2zero ? 0 : 5;
                (void)sp_256_calc_vfy_point_9(&p1, &p2, s, u1, u2, tmp,
                    keyA.heap);
            }
        }
    ...
    /* --- Target gap 3: sp_ecc_check_key_256()'s mp_count_bits(pX) > 256
     * (and pY, and privm). 2^256 is 257 bits -- one bit too many for each
     * operand in turn -- plus one all-in-range baseline call. --- */
    if (ok) {
        mp_int big;
...
static void wb_run_gap_384(void)
{
    ...
    if (ok && negP != NULL) {
        sp_point_384 pA;
        sp_point_384 pB;
        sp_digit     addTmp[12 * 15];

        XMEMSET(&pA, 0, sizeof(pA));
        XMEMSET(&pB, 0, sizeof(pB));
        XMEMSET(addTmp, 0, sizeof(addTmp));
        wb_build_neg_point(&keyA.pubkey, ECC_SECP384R1, negP);
        sp_384_point_from_ecc_point_15(&pA, &keyA.pubkey);
        sp_384_point_from_ecc_point_15(&pB, negP);
        sp_384_add_points_15(&pA, &pB, addTmp);

        XMEMSET(&pA, 0, sizeof(pA));
        XMEMSET(&pB, 0, sizeof(pB));
        XMEMSET(addTmp, 0, sizeof(addTmp));
        sp_384_point_from_ecc_point_15(&pA, &keyA.pubkey);
        sp_384_point_from_ecc_point_15(&pB, &keyA.pubkey);
        sp_384_add_points_15(&pA, &pB, addTmp);
        ...
        XMEMSET(addTmp, 0, sizeof(addTmp));
        sp_384_point_from_ecc_point_15(&pA, &keyA.pubkey);
        sp_384_point_from_ecc_point_15(&pB, &keyB.pubkey);
        sp_384_add_points_15(&pA, &pB, addTmp);
    ...
    if (ok) {
        ...
        for (u1zero = 0; u1zero <= 1; u1zero++) {
            for (u2zero = 0; u2zero <= 1; u2zero++) {
                ...
                sp_digit     vbuf[18 * 15];
                sp_digit    *u1  = vbuf;
                sp_digit    *u2  = vbuf + 2 * 15;
                sp_digit    *s   = vbuf + 4 * 15;
                sp_digit    *tmp = vbuf + 6 * 15;

                XMEMSET(&p1, 0, sizeof(p1));
                XMEMSET(&p2, 0, sizeof(p2));
                XMEMSET(vbuf, 0, sizeof(vbuf));
                sp_384_point_from_ecc_point_15(&p2, &keyB.pubkey);
                s[0] = 7;
                u1[0] = u1zero ? 0 : 5;
                u2[0] = u2zero ? 0 : 5;
                (void)sp_384_calc_vfy_point_15(&p1, &p2, s, u1, u2, tmp,
                    keyA.heap);
            }
        }
...
static void wb_run_gap_521(void)
{
    ...
    if (ok && negP != NULL) {
        sp_point_521 pA;
        sp_point_521 pB;
        sp_digit     addTmp[12 * 21];

        XMEMSET(&pA, 0, sizeof(pA));
        XMEMSET(&pB, 0, sizeof(pB));
        XMEMSET(addTmp, 0, sizeof(addTmp));
        wb_build_neg_point(&keyA.pubkey, ECC_SECP521R1, negP);
        sp_521_point_from_ecc_point_21(&pA, &keyA.pubkey);
        sp_521_point_from_ecc_point_21(&pB, negP);
        sp_521_add_points_21(&pA, &pB, addTmp);

        XMEMSET(&pA, 0, sizeof(pA));
        XMEMSET(&pB, 0, sizeof(pB));
        XMEMSET(addTmp, 0, sizeof(addTmp));
        sp_521_point_from_ecc_point_21(&pA, &keyA.pubkey);
        sp_521_point_from_ecc_point_21(&pB, &keyA.pubkey);
        sp_521_add_points_21(&pA, &pB, addTmp);
        ...
        XMEMSET(addTmp, 0, sizeof(addTmp));
        sp_521_point_from_ecc_point_21(&pA, &keyA.pubkey);
        sp_521_point_from_ecc_point_21(&pB, &keyB.pubkey);
        sp_521_add_points_21(&pA, &pB, addTmp);
    ...
    if (ok) {
        ...
        for (u1zero = 0; u1zero <= 1; u1zero++) {
            for (u2zero = 0; u2zero <= 1; u2zero++) {
                ...
                sp_digit     vbuf[18 * 21];
                sp_digit    *u1  = vbuf;
                sp_digit    *u2  = vbuf + 2 * 21;
                sp_digit    *s   = vbuf + 4 * 21;
                sp_digit    *tmp = vbuf + 6 * 21;

                XMEMSET(&p1, 0, sizeof(p1));
                XMEMSET(&p2, 0, sizeof(p2));
                XMEMSET(vbuf, 0, sizeof(vbuf));
                sp_521_point_from_ecc_point_21(&p2, &keyB.pubkey);
                s[0] = 7;
                u1[0] = u1zero ? 0 : 5;
                u2[0] = u2zero ? 0 : 5;
                (void)sp_521_calc_vfy_point_21(&p1, &p2, s, u1, u2, tmp,
                    keyA.heap);
            }
        }
...
int main(void)
{
    printf("sp_c32.c white-box supplement (32-bit portable C, no cpuid "
           "dispatch)\n");
#if defined(WOLFSSL_HAVE_SP_ECC) || defined(WOLFSSL_HAVE_SP_RSA) || \
    defined(WOLFSSL_HAVE_SP_DH)
    wb_run_ecc();
