...
static int test_client_hello_cb_noext(WOLFSSL* ssl, int* al, void* arg)
{
     ...
     * safe.  TLSXT_SERVER_NAME is the extension number itself, which
     * HAVE_TLS_EXTENSIONS defines; the TLSX_ enum member of the same meaning
     * exists only under HAVE_SNI, which this test does not need. */
    if (wolfSSL_client_hello_get0_ext(ssl, TLSXT_SERVER_NAME, &ext,
            &extLen) == 0) {
