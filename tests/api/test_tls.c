 ...
 * test_tls13_record_size_limit(). */
    ...
    /* The same yielding, but with max_fragment_length asked for on the
     * context. wolfSSL_CTX_UseMaxFragment() stores on ctx->extensions and
     * nothing copies that list onto the object - the write path emits both -
     * so a check of ssl->extensions alone does not see it. Getting this wrong
     * makes the client advertise both, which by RFC 8449 Sect. 5 has the
     * server ignore max_fragment_length and leaves the application's
     * requested fragment size silently unapplied. */
    XMEMSET(&test_ctx, 0, sizeof(test_ctx));
    ExpectIntEQ(test_memio_setup(&test_ctx, &ctx_c, &ctx_s, &ssl_c, &ssl_s,
        wolfTLSv1_2_client_method, wolfTLSv1_2_server_method), 0);
    wolfSSL_free(ssl_c); ssl_c = NULL;
    ExpectIntEQ(wolfSSL_CTX_UseMaxFragment(ctx_c, WOLFSSL_MFL_2_11),
        WOLFSSL_SUCCESS);
    ExpectNotNull(ssl_c = wolfSSL_new(ctx_c));
    wolfSSL_SetIOWriteCtx(ssl_c, &test_ctx);
    wolfSSL_SetIOReadCtx(ssl_c, &test_ctx);
    ExpectIntEQ(test_memio_do_handshake(ssl_c, ssl_s, 10, NULL), 0);
    /* No record_size_limit was offered, so the peer learned none and the
     * fragment length the application asked for is the one in force. */
    if (ssl_s != NULL)
        ExpectIntEQ(ssl_s->peerRecordSizeLimit, 0);
    if (ssl_c != NULL)
        ExpectIntEQ(ssl_c->max_fragment, 2048);
    ExpectIntEQ(wolfSSL_GetMaxOutputSize(ssl_c), 2048);
