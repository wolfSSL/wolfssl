...
#if defined(WOLFSSL_DTLS) && defined(WOLFSSL_SESSION_EXPORT) && \
    !defined(WOLFSSL_NO_TLS12)
/* DTLS 1.2 session serialized with export version 6, captured before the
 * (D)TLS 1.3 state chunks were added to the format. Generated with a memio
 * DTLS 1.2 handshake followed by wolfSSL_dtls_export() of the server end
 * (dummy peer callbacks: ip[0]=-1, ipSz=1, port=1, fam=2). */
static byte canned_server_dtls12_session_v6[] = {
    0xA5, 0xA6, 0x01, 0x78, 0x00, 0x42, 0x00, 0x00, 0x00, 0x00,
    ...
    ,
    ...
    ,
};

static int test_dtls_import_v6_set_peer(WOLFSSL* ssl, char* ip, int ipSz,
...
/* Session blobs serialized with export version 6 must still import. */
int test_wolfSSL_dtls_import_v6_canned(void)
{
    ...
    ExpectNotNull(ctx = wolfSSL_CTX_new(wolfDTLSv1_2_server_method()));
    wolfSSL_CTX_SetIOSetPeer(ctx, test_dtls_import_v6_set_peer);
    ExpectNotNull(ssl = wolfSSL_new(ctx));
    ExpectIntEQ(wolfSSL_dtls_import(ssl, canned_server_dtls12_session_v6,
        sizeof(canned_server_dtls12_session_v6)),
        (int)sizeof(canned_server_dtls12_session_v6));
    wolfSSL_free(ssl);
    wolfSSL_CTX_free(ctx);
#endif
    return EXPECT_RESULT();
...
/* Importing a pre-v7 blob into a DTLS 1.3 object must fail with
 * VERSION_ERROR: those formats predate the DTLS 1.3 record state. */
int test_wolfSSL_dtls13_import_reject_v6(void)
{
    ...
    WOLFSSL* ssl12 = NULL;
    WOLFSSL* ssl13 = NULL;
    byte blob[sizeof(canned_server_dtls12_session_v6)];
    byte state[MAX_EXPORT_STATE_BUFFER];
    unsigned int stateSz;
    word16 optLen = 0;
    word32 verOff;

    ExpectNotNull(ctx13 = wolfSSL_CTX_new(wolfDTLSv1_3_server_method()));
    wolfSSL_CTX_SetIOSetPeer(ctx13, test_dtls_import_v6_set_peer);
    ExpectNotNull(ssl13 = wolfSSL_new(ctx13));

    /* patch the protocol version bytes at the end of the Options chunk from
     * DTLS 1.2 to DTLS 1.3 */
    XMEMCPY(blob, canned_server_dtls12_session_v6, sizeof(blob));
    optLen = (word16)((blob[2 * WOLFSSL_EXPORT_LEN] << 8) |
                       blob[2 * WOLFSSL_EXPORT_LEN + 1]);
    ExpectIntEQ(optLen, DTLS_EXPORT_OPT_SZ);
    verOff = 3 * WOLFSSL_EXPORT_LEN + optLen - 2;
    ExpectIntEQ(blob[verOff], DTLS_MAJOR);
    ExpectIntEQ(blob[verOff + 1], DTLSv1_2_MINOR);
    blob[verOff + 1] = DTLSv1_3_MINOR;

    ExpectIntEQ(wolfSSL_dtls_import(ssl13, blob, sizeof(blob)),
        WC_NO_ERR_TRACE(VERSION_ERROR));

    /* the same patched blob into a DTLS 1.2 object is a version mismatch */
    ExpectNotNull(ctx12 = wolfSSL_CTX_new(wolfDTLSv1_2_server_method()));
    wolfSSL_CTX_SetIOSetPeer(ctx12, test_dtls_import_v6_set_peer);
    ExpectNotNull(ssl12 = wolfSSL_new(ctx12));
    ExpectIntEQ(wolfSSL_dtls_import(ssl12, blob, sizeof(blob)),
        WC_NO_ERR_TRACE(VERSION_ERROR));

    /* a pre-v7 state-only blob is also refused on a DTLS 1.3 object */
    wolfSSL_free(ssl12);
    ssl12 = NULL;
    ExpectNotNull(ssl12 = wolfSSL_new(ctx12));
    stateSz = sizeof(state);
    ExpectIntGT(wolfSSL_dtls_export_state_only(ssl12, state, &stateSz), 0);
    /* force the export version nibble to 6 so this stays a pre-v7 blob */
    state[1] = (byte)(((byte)DTLS_EXPORT_STATE_PRO & 0xF0) |
                      ((byte)WOLFSSL_EXPORT_VERSION_6 & 0x0F));
    ExpectIntEQ(wolfSSL_dtls_import(ssl13, state, stateSz),
        WC_NO_ERR_TRACE(VERSION_ERROR));

    wolfSSL_free(ssl12);
    wolfSSL_free(ssl13);
    wolfSSL_CTX_free(ctx12);
