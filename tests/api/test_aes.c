    ...
    byte* iv)
{
    ...
    ExpectIntEQ(wc_AesSetKey(NULL, key , keyLen, iv, AES_ENCRYPTION),
        WC_NO_ERR_TRACE(BAD_FUNC_ARG));
    ExpectIntEQ(wc_AesSetKey(aes , NULL, keyLen, iv, AES_ENCRYPTION),
        WC_NO_ERR_TRACE(BAD_FUNC_ARG));
    ...
    word32 keyLen, byte* expected)
{
    ...
    for (sz = WC_AES_BLOCK_SIZE; sz <= ECB_LEN; sz += WC_AES_BLOCK_SIZE) {
        XMEMSET(cipher, 0x00, sizeof(cipher));
    ...
    ExpectIntEQ(wc_AesSetKey(aes, key, keyLen, NULL, AES_DECRYPTION), 0);
    for (sz = WC_AES_BLOCK_SIZE; sz <= ECB_LEN; sz += WC_AES_BLOCK_SIZE) {
        XMEMSET(decrypted, 0xff, sizeof(decrypted));
        for (cnt = 0; cnt + sz <= ECB_LEN; cnt += sz) {
    ...
    word32 keyLen, byte* expected)
{
    ...
    ExpectIntEQ(wc_AesSetKey(aes, key, keyLen, NULL, AES_ENCRYPTION), 0);
    XMEMCPY(cipher, plain, sizeof(cipher));
...
static int test_wc_AesCbcEncryptDecrypt_WithKey(Aes* aes, byte* key,
    word32 keyLen, byte* iv, byte* vector, byte* vector_enc)
{
    EXPECT_DECLS;
...
#endif
    ExpectIntEQ(wc_AesCbcEncrypt(aes, cipher, vector, WC_AES_BLOCK_SIZE * 2),
        0);
    ExpectBufEQ(cipher, vector_enc, WC_AES_BLOCK_SIZE * 2);
#ifdef WOLFSSL_AES_CBC_LENGTH_CHECKS
    ExpectIntEQ(wc_AesCbcEncrypt(aes, cipher, vector, sizeof(vector) - 1),
    ...
    word32 keyLen, byte* iv)
{
    ...
    byte cipher[CFB_LEN];
#ifdef HAVE_AES_DECRYPT
    byte decrypted[CFB_LEN];
#endif

    XMEMSET(plain, 0x00, sizeof(plain));
    XMEMSET(cipher, 0x00, sizeof(cipher));
    ...
    word32 keyLen, byte* iv, byte* expected)
{
    ...
    XMEMSET(plain, 0xa5, sizeof(plain));
    ...
    byte* iv)
{
    ...
    ExpectIntEQ(wc_AesCtrSetKey(NULL, key , keyLen, iv, AES_ENCRYPTION),
        WC_NO_ERR_TRACE(BAD_FUNC_ARG));
    ExpectIntEQ(wc_AesCtrSetKey(aes , NULL, keyLen, iv, AES_ENCRYPTION),
        WC_NO_ERR_TRACE(BAD_FUNC_ARG));
