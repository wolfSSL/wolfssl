...
int test_wc_AesKeyWrap_Pad(void)
{
    ...
    defined(WOLFSSL_AES_256)
    /* --- Extended coverage for every enabled key size (128/192/256):
     *     boundary-size known-answer vectors independently generated with the
     *     pyca/cryptography library (which reproduces the RFC 5649 Section 6
     *     published vectors), an exhaustive round-trip over all input sizes
     *     1..64, and in-place (in==out) operation.  Plaintext is 0xA0,0xA1,...
     *     The boundary sizes pin the single->multi-block transition (8 vs 9
     *     octets) and the exact-8-multiple zero-padding cases that the 7- and
     *     20-octet vectors never reach. --- */
    {
#ifdef WOLFSSL_AES_128
