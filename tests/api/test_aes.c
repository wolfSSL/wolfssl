...
int test_wc_CryptoCb_AesSetKey(void)
{
    ...
    /* CRITICAL: In TLS builds, verify key WAS copied to devKey for fallback */
    ExpectIntEQ(XMEMCMP(aes->devKey, key, sizeof(key)), 0);

    /* CRITICAL: In TLS builds, verify H table WAS generated for fallback */
    {
        int hTableNonZero = 0;
        int i;
