...
int test_tls13_record_size_limit(void)
{
    ...
    /* And the other direction, which is the half that looks like a bug and
     * is not: the server saw the client's 512 but answered nothing, so
     * RFC 8449 Sect. 4 leaves it unbound - "When the record_size_limit
     * extension is negotiated, an endpoint MUST NOT generate a protected
     * record with plaintext that is larger than the RecordSizeLimit value it
     * receives from its peer", and "If this extension is not negotiated,
     * endpoints can send records of any size permitted by the protocol". The
     * duty follows negotiation, not receipt, so a server that declined to
     * advertise keeps sending full records and does not retain the value.
     * Retaining it would let any client shrink a server that never opted in
     * down to 64-byte records. */
    if (ssl_s != NULL)
        ExpectIntEQ(ssl_s->peerRecordSizeLimit, 0);
    ExpectIntEQ(wolfSSL_GetMaxOutputSize(ssl_s), MAX_RECORD_SIZE);
...
int test_tls13_sct_certificate_msg(void)
{
    ...
    /* RFC 6962 Sect. 3.3: TLS 1.2 answers in the ServerHello, TLS 1.3 in the
     * end entity certificate's CertificateEntry extensions. The TLS 1.2 leg
     * is test_tls13_signed_cert_timestamp(); this is the 1.3 one, which is a
     * different write path entirely. */
    XMEMSET(&test_ctx, 0, sizeof(test_ctx));
    ExpectIntEQ(test_memio_setup(&test_ctx, &ctx_c, &ctx_s, &ssl_c, &ssl_s,
        wolfTLSv1_3_client_method, wolfTLSv1_3_server_method), 0);
    ExpectIntEQ(wolfSSL_set_signed_cert_timestamp_list(ssl_s, sctList,
    ...
    /* No list configured: the entry's extensions field stays empty and the
...
int test_tls13_sct_cert_chain(void)
{
    ...
    /* A chain, not a single certificate: the SCT goes on the end entity
     * certificate only, so every certificate after it must advance to its own
     * empty extensions entry. When the index stopped advancing, each chain
     * certificate reused the leaf's entry - reading a buffer already freed and
     * claiming bytes the message length never budgeted for. Built by hand
     * because test_memio_setup() loads a single certificate, which leaves
     * certChainCnt at zero and cannot reach any of that. */
    ExpectNotNull(ctx_s = wolfSSL_CTX_new(wolfTLSv1_3_server_method()));
    ...
    ExpectIntEQ(test_memio_setup(&test_ctx, &ctx_c, &ctx_s, &ssl_c, &ssl_s,
        wolfTLSv1_3_client_method, wolfTLSv1_3_server_method), 0);
    /* The chain really is more than one certificate, or this proves nothing. */
    if (ssl_s != NULL)
        ExpectIntGT(ssl_s->buffers.certChainCnt, 0);
    ExpectIntEQ(wolfSSL_set_signed_cert_timestamp_list(ssl_s, sctList,
        (unsigned short)sizeof(sctList)), WOLFSSL_SUCCESS);
...
int test_tls13_compressed_certificate_sct(void)
{
    ...
    WOLFSSL     *ssl_c = NULL, *ssl_s = NULL;
    struct test_memio_ctx test_ctx;
    static const byte sctList[] = { 0x00, 0x05, 0x00, 0x03, 0xAA, 0xBB, 0xCC };
    const byte* got = NULL;

    /* The SCT list rides in the leaf's CertificateEntry extensions, and the
     * compressed cache is built with those empty. Compressing here would drop
     * a list the server had already committed to sending, so this handshake
     * has to fall back to the plain Certificate. */
    XMEMSET(&test_ctx, 0, sizeof(test_ctx));
    ExpectIntEQ(test_memio_setup(&test_ctx, &ctx_c, &ctx_s, &ssl_c, &ssl_s,
        wolfTLSv1_3_client_method, wolfTLSv1_3_server_method), 0);
    ExpectIntEQ(wolfSSL_CTX_compress_certs(ctx_s, WOLFSSL_CERT_COMP_ZLIB),
    ...
    /* Without a list to send there is nothing to lose, so compression is
     * still taken - the opt-out must not cost every other handshake. */
    XMEMSET(&test_ctx, 0, sizeof(test_ctx));
    ExpectIntEQ(test_memio_setup(&test_ctx, &ctx_c, &ctx_s, &ssl_c, &ssl_s,
        wolfTLSv1_3_client_method, wolfTLSv1_3_server_method), 0);
    ExpectIntEQ(wolfSSL_CTX_compress_certs(ctx_s, WOLFSSL_CERT_COMP_ZLIB),
...
int test_tls13_compressed_certificate_req_ctx(void)
{
    ...
    /* Answering a CertificateRequest consumes the CertReqCtx node it queued.
     * An in-handshake request queues a zero length one, and that is exactly
     * the case UseCompressedCertificate() allows to compress, so the
     * compressed send path has to release it just as the plain one does.
     * Built like test_tls13_compressed_certificate_client(): the client's own
     * certificate has to be on the context before the cache is made, and
     * before the WOLFSSL is created from it. */
    ExpectNotNull(ctx_c = wolfSSL_CTX_new(wolfTLSv1_3_client_method()));
    ExpectTrue(wolfSSL_CTX_load_verify_locations(ctx_c, caCertFile, 0)
        == WOLFSSL_SUCCESS);
    wolfSSL_SetIORecv(ctx_c, test_memio_read_cb);
    ...
    ExpectIntEQ(test_memio_do_handshake(ssl_c, ssl_s, 10, NULL), 0);
    /* The client really did take the compressed path, so this is testing what
     * it claims to test rather than the plain path by accident. */
    ExpectIntEQ(wolfSSL_get_certificate_compression_used(ssl_s),
        WOLFSSL_CERT_COMP_ZLIB);
    /* Request answered, node released. */
    if (ssl_c != NULL)
        ExpectNull(ssl_c->certReqCtx);
...
int test_tls13_compressed_certificate_server_advertise(void)
{
    ...
    /* certCompAdvertised has to mean "this end put the extension in a message
     * it sent", because that is what RFC 8879 Sect. 4 makes the peer's licence
     * to send a CompressedCertificate. A server only advertises in the
     * CertificateRequest, so a server that never asks for a client
     * certificate has advertised nothing - and must not accept one.
     * test_tls13_compressed_certificate_unsolicited() checks the guard that
     * reads this flag; this checks the flag itself, which the guard is
     * worthless without. */

    /* No client authentication: no CertificateRequest, so nothing offered. */
    XMEMSET(&test_ctx, 0, sizeof(test_ctx));
    ExpectIntEQ(test_memio_setup(&test_ctx, &ctx_c, &ctx_s, &ssl_c, &ssl_s,
        wolfTLSv1_3_client_method, wolfTLSv1_3_server_method), 0);
    ExpectIntEQ(test_memio_do_handshake(ssl_c, ssl_s, 10, NULL), 0);
    if (ssl_s != NULL)
        ExpectIntEQ(ssl_s->certCompAdvertised, 0);
    /* The client did offer, in its ClientHello. */
    if (ssl_c != NULL)
        ExpectIntEQ(ssl_c->certCompAdvertised, 1);
    ...
    /* Client authentication requested: the CertificateRequest carries the
     * extension, so now the server has advertised and may accept one. */
    XMEMSET(&test_ctx, 0, sizeof(test_ctx));
    ExpectIntEQ(test_memio_setup(&test_ctx, &ctx_c, &ctx_s, &ssl_c, &ssl_s,
        wolfTLSv1_3_client_method, wolfTLSv1_3_server_method), 0);
    ExpectTrue(wolfSSL_CTX_load_verify_locations(ctx_s, cliCertFile, 0)
    ...
    ExpectIntEQ(test_memio_do_handshake(ssl_c, ssl_s, 10, NULL), 0);
    if (ssl_s != NULL)
        ExpectIntEQ(ssl_s->certCompAdvertised, 1);
