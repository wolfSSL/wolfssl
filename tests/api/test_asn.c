...
int test_EncodePolicyOID_certgen(void)
{
    ...
    defined(USE_CERT_BUFFERS_2048) && !defined(HAVE_FIPS)
    {
        ...
        /* A malformed policy OID drops that policy but leaves the cert
         * parseable - checked at the cert level, not just on
         * DecodePolicyOID(). Corrupt "2.100.3" ({ 0x81, 0x34, 0x03 })
         * to a non-minimal first arc (leading 0x80); same length, so no
         * other offset shifts. Match the full TLV to avoid a false hit
         * on some unrelated byte. */
        if (EXPECT_SUCCESS()) {
            int i;
            int found = 0;
            int truncated = 0;
            ...
            ExpectIntEQ(wc_ParseCert(&decoded, CERT_TYPE, NO_VERIFY, NULL),
                    0);
            ExpectIntEQ(decoded.extCertPoliciesNb, 0);
            ExpectIntEQ(wc_GetDecodedCertPoliciesTruncated(&decoded,
                        &truncated), 0);
...
/* SSL-layer twin of the DecodedCert check above: the truncated flag must
 * survive CopyDecodedToX509() into a WOLFSSL_X509, in every build that has
 * wolfSSL_X509_d2i(), not only OPENSSL_EXTRA ones. */
int test_wolfSSL_X509_get_certPoliciesTruncated(void)
{
     ...
     defined(OPENSSL_EXTRA) || defined(OPENSSL_EXTRA_X509_SMALL))
    {
        ...
        /* Corrupt the policy OID { 0x81, 0x34, 0x03 } to a non-minimal
         * first arc (leading 0x80), same trick as
         * test_EncodePolicyOID_certgen(): the policy is dropped and the
         * X509 must report that. */
        if (EXPECT_SUCCESS()) {
            int i;
            int found = 0;
            const byte oidBytes[] = { 0x06, 0x03, 0x81, 0x34, 0x03 };
