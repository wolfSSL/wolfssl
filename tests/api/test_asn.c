...
int test_wc_AsnFeatureCoverage(void)
{
    ...
    defined(HAVE_ECC_KEY_EXPORT) && !defined(WOLF_CRYPTO_CB_FIND) && \
    defined(USE_CERT_BUFFERS_256) && !defined(HAVE_FIPS) && \
...
int test_wc_EccPrivateKeyDecode_derive_pub(void)
{
    ...
    !defined(WOLFSSL_FORCE_MALLOC_FAIL_TEST)
    {
        ...
        wc_ecc_free(&privOnlyKey);
        /* Under small-stack the armed window must have allocated something,
         * or the loop below passes vacuously without exercising any OOM
         * path. Other configs (e.g. SP assembly) may legitimately make no
         * heap allocations here, so only the loop's guarantee is checked. */
    ...
    wc_FreeRng(&rng);
        ...
        * !WOLF_CRYPTO_CB_FIND && USE_CERT_BUFFERS_256 && !HAVE_FIPS &&
        * !HAVE_SELFTEST && !WOLF_CRYPTO_CB_ONLY_ECC */
    return EXPECT_RESULT();
}
