...
/* Regression test for Hardware Module Name (RFC 4108 sec 5) OtherName
 * decoding.  The hwType OID and hwSerialNum live inside a SEQUENCE nested in
 * the OtherName value; both must be recovered into the DecodedCert.
 *
 * ECDSA P-256 leaf signed by certs/ca-ecc-cert.pem whose SAN holds a
 * hardwareModuleName (hwType 1.3.6.1.4.1.12345.1, hwSerialNum 01020304)
 * followed by a dNSName "node000.local". */
int test_DecodeOtherName_hwModuleName(void)
{
    EXPECT_DECLS;
#if defined(WOLFSSL_SEP) && defined(HAVE_ECC) && !defined(NO_CERTS) && \
    !defined(NO_ASN) && !defined(NO_FILESYSTEM)
    /* Generated by certs/renewcerts.sh: an ECDSA P-256 leaf whose SAN holds a
     * hardwareModuleName OtherName (RFC 4108) carrying the module type OID
     * and serial number, followed by a dNSName "node000.local". */
    const char* hwnCertFile = "./certs/hwname-cert.der";
    XFILE f = XBADFILE;
    byte hwnCert[2048];
    int bytes = 0;
    ...
    ExpectTrue((f = XFOPEN(hwnCertFile, "rb")) != XBADFILE);
    if (f != XBADFILE) {
        ExpectIntGT(bytes = (int)XFREAD(hwnCert, 1, sizeof(hwnCert), f), 0);
        XFCLOSE(f);
    }
    if (bytes <= 0) {
        return EXPECT_RESULT();
    ...
    wc_FreeDecodedCert(&cert);
#endif /* WOLFSSL_SEP && HAVE_ECC && !NO_CERTS && !NO_ASN &&
        * !NO_FILESYSTEM */
    return EXPECT_RESULT();
}

/* An OtherName value is a CHOICE of value types: the alternative present
 * must be the one its type-id OID calls for, except that a UPN value may be
 * any of the supported string types.  Exercised by mutating
 * certs/bundle-eid-cert.der (bundleEID OtherName + IA5String) and
 * certs/fpki-cert.der (UPN OtherName + UTF8String) in memory. */
int test_DecodeOtherName_typeMismatch(void)
{
    EXPECT_DECLS;
#if defined(WOLFSSL_DTN) && defined(HAVE_ECC) && !defined(NO_CERTS) && \
    !defined(NO_ASN) && !defined(NO_FILESYSTEM)
    /* id-on-bundleEID type-id (1.3.6.1.5.5.7.8.11). */
    static const byte beidOid[] = {
        0x06, 0x08, 0x2b, 0x06, 0x01, 0x05, 0x05, 0x07, 0x08, 0x0b
    };
    /* Offsets into bundle-eid-cert.der of the bundleEID OtherName type-id
     * and of its value's tag after the 2 byte [0] header.  renewcerts.sh
     * regenerates the cert with fixed serial, keys and names, so the
     * offsets are stable; the checks below fail loudly if the layout ever
     * shifts. */
    #define BEID_OID_IDX 561
    #define BEID_TAG_IDX (BEID_OID_IDX + (int)sizeof(beidOid) + 2)
    XFILE f = XBADFILE;
    byte buf[2048];
    byte der[2048];
    int bytes = 0;
    ...
    ExpectTrue((f = XFOPEN("./certs/bundle-eid-cert.der", "rb")) != XBADFILE);
    if (f != XBADFILE) {
        ExpectIntGT(bytes = (int)XFREAD(buf, 1, sizeof(buf), f), 0);
        XFCLOSE(f);
        f = XBADFILE;
    ...
    ExpectIntGT(bytes, BEID_TAG_IDX);
    if (bytes > BEID_TAG_IDX) {
        ExpectBufEQ(buf + BEID_OID_IDX, beidOid, sizeof(beidOid));
        ExpectIntEQ(buf[BEID_TAG_IDX], ASN_IA5_STRING);
    }

    if (EXPECT_SUCCESS()) {
        /* bundleEID type-id whose value is a UTF8String must not parse. */
        XMEMCPY(der, buf, (size_t)bytes);
        der[BEID_TAG_IDX] = ASN_UTF8STRING;
        wc_InitDecodedCert(&cert, der, (word32)bytes, NULL);
        ExpectIntEQ(wc_ParseCert(&cert, CERT_TYPE, NO_VERIFY, NULL),
                    WC_NO_ERR_TRACE(ASN_PARSE_E));
        wc_FreeDecodedCert(&cert);

    #ifdef WOLFSSL_SEP
        /* Hardware Module Name type-id (1.3.6.1.5.5.7.8.4) whose value is
         * an IA5String, not the expected SEQUENCE, must not parse. */
        XMEMCPY(der, buf, (size_t)bytes);
        der[BEID_OID_IDX + (int)sizeof(beidOid) - 1] = 0x04;
        wc_InitDecodedCert(&cert, der, (word32)bytes, NULL);
        ExpectIntEQ(wc_ParseCert(&cert, CERT_TYPE, NO_VERIFY, NULL),
                    WC_NO_ERR_TRACE(ASN_PARSE_E));
        wc_FreeDecodedCert(&cert);
    ...
    #ifdef WOLFSSL_FPKI
        {
            /* FASC-N type-id (2.16.840.1.101.3.6.6), encoded with the same
             * length as the bundleEID type-id. */
            static const byte fascnOid[] = {
                0x06, 0x08, 0x60, 0x86, 0x48, 0x01, 0x65, 0x03, 0x06, 0x06
            };
            /* FASC-N type-id whose value is an IA5String, not the expected
             * OCTET STRING, must not parse. */
            XMEMCPY(der, buf, (size_t)bytes);
            XMEMCPY(der + BEID_OID_IDX, fascnOid, sizeof(fascnOid));
            wc_InitDecodedCert(&cert, der, (word32)bytes, NULL);
            ExpectIntEQ(wc_ParseCert(&cert, CERT_TYPE, NO_VERIFY, NULL),
                        WC_NO_ERR_TRACE(ASN_PARSE_E));
            wc_FreeDecodedCert(&cert);
...
#if defined(WOLFSSL_FPKI) && !defined(NO_RSA)
    /* A UPN value may be a UTF8String or an IA5String: flipping the UPN
     * value of certs/fpki-cert.der to an IA5String must still parse and
     * yield the same entry. */
    {
        /* UPN type-id (1.3.6.1.4.1.311.20.2.3). */
        static const byte upnOid[] = {
            0x06, 0x0a, 0x2b, 0x06, 0x01, 0x04, 0x01, 0x82, 0x37, 0x14,
            0x02, 0x03
        };
        /* Offsets into fpki-cert.der, as above. */
        #define UPN_OID_IDX 825
        #define UPN_TAG_IDX (UPN_OID_IDX + (int)sizeof(upnOid) + 2)
        DNS_entry* dns = NULL;
        int foundUpn = 0;

        bytes = 0;
        ExpectTrue((f = XFOPEN("./certs/fpki-cert.der", "rb")) != XBADFILE);
        if (f != XBADFILE) {
            ExpectIntGT(bytes = (int)XFREAD(buf, 1, sizeof(buf), f), 0);
            XFCLOSE(f);
        }

        ExpectIntGT(bytes, UPN_TAG_IDX);
        if (bytes > UPN_TAG_IDX) {
            ExpectBufEQ(buf + UPN_OID_IDX, upnOid, sizeof(upnOid));
            ExpectIntEQ(buf[UPN_TAG_IDX], ASN_UTF8STRING);
        }
        if (EXPECT_SUCCESS()) {
            XMEMCPY(der, buf, (size_t)bytes);
            der[UPN_TAG_IDX] = ASN_IA5_STRING;
            wc_InitDecodedCert(&cert, der, (word32)bytes, NULL);
            ExpectIntEQ(wc_ParseCert(&cert, CERT_TYPE, NO_VERIFY, NULL), 0);
            for (dns = cert.altNames; dns != NULL; dns = dns->next) {
                if ((dns->type == ASN_OTHER_TYPE) && (dns->len == 17) &&
                        (dns->oidSum == UPN_OID) &&
                        (XMEMCMP(dns->name, "facts@wolfssl.com", 17) == 0)) {
                    foundUpn = 1;
                }
            }
            ExpectIntEQ(foundUpn, 1);
            wc_FreeDecodedCert(&cert);
        }
        #undef UPN_OID_IDX
        #undef UPN_TAG_IDX
    }
#endif /* WOLFSSL_FPKI && !NO_RSA */
    #undef BEID_OID_IDX
    #undef BEID_TAG_IDX
#endif /* WOLFSSL_DTN && HAVE_ECC && !NO_CERTS && !NO_ASN &&
        * !NO_FILESYSTEM */
