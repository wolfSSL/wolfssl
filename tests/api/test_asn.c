...
int test_DecodeAltNames_length_underflow(void)
{
    ...
    /* Minimal self-signed DER certificate with a malformed SAN extension.
     * The SAN SEQUENCE declares 3 bytes of content, but the single DNS
     * entry inside claims 4 bytes of data.  Without a bounds check this
     * causes length to underflow: length(1) -= strLen(4) → -3.
     *
     * This cert is a copy of the one in test_wolfSSL_X509_bad_altname()
     * with byte offset 418 changed from 0x06 to 0x03 (SAN SEQUENCE
     * length shrunk from 6 to 3). */
    static const unsigned char bad_san_cert[] = {
        ...
        /* SAN extension: SEQUENCE length changed from 0x06 to 0x03 */
        0x30, 0x0f, 0x06, 0x03, 0x55, 0x1d, 0x11, 0x04, 0x08, 0x30, 0x03, 0x82,
    ...
    /* Same cert but with correct SAN SEQUENCE length (0x06) for control. */
    ...
    /* Test: the malformed cert with SAN SEQUENCE length (3) smaller than the
     * DNS entry data length (4) must be rejected. Without a bounds check,
     * DecodeAltNames would underflow the length tracker. */
